...
transcript *and* the agent CLI's ``session_id`` (plus which CLI minted it,
ADR-174) that lets a later turn resume it. The conversation is shell state -- the engine has no notion of
...
class ChatHistory:
    def __init__(self):
        ...
        #: Agent-CLI session to resume, or "" for a fresh conversation.
        #: Saved with the transcript, because it belongs to this .blend's
        #: conversation and to no other.
        self.session_id = ""
        #: Which backend the session id belongs to ("claude", "codex"), so a
        #: Codex thread id is never handed to ``claude --resume``. "" on
        #: transcripts saved before providers existed — those are Claude's.
    ...
    def from_json(self, text):
        ...
        if isinstance(data, dict):
            ...
            self.session_provider = str(data.get("session_provider") or "")
