"""One real GPU training run on the product-agent's fresh Lark design, with a
...
This is the Wren experiment (``../wren-fresh/train.py``) with nothing named
after Wren: the model, task and policy outputs are discovered from the CLI's
own envelopes by kind, so the same driver runs on any project whose script
declares one MJCF model, one task and the ``policy_on`` iterate convention
(ADR-192). The observer and the video checker beside the Wren driver are
project-agnostic and are reused by path.
...
to the original Lark creation and is recorded in run.json and each playback review.
...
authored_by = sys.argv[4] if len(sys.argv) > 4 else 'product-agent-authored Lark; accepted creation in one fresh cadex -p turn'
repo = Path.cwd(); ev = p / 'evidence'; run = p / 'runs' / name; train = run / 'train'
generic = Path(__file__).resolve().parent.parent / 'wren-fresh'   # observe.py and check_video.py are project-agnostic
...
assert components == 8, components
base = dict(project_root=p, mode='offboard-checkpoint-experiment', accepted_revision=e['accepted_revision'], digest=e['digest'], params=e['params'], param_specs=m['param_specs'], specs_source='successful CLI export', identity_source='successful params export envelope', requested={'iterations': 240, 'envs': 1024, 'seed': 0, 'checkpoint_every': 20, 'timeout_seconds': 1800, 'authored_by': authored_by}, task_bundle=task_json, task_sha256=sha(task_json), model_xml=model_xml)
write_run_record(run, status='running', snapshot_docs=True, **base)
unit = 'cadex-' + name
cmd = ['systemd-run', '--user', '--scope', '--unit=' + unit, '-p', 'MemoryMax=20G', 'timeout', '--signal=TERM', '--kill-after=20s', '1800', str(Path.home() / 'cadex-train-venv/bin/python'), 'training/cadex_train.py', str(task_json), '--out', str(train / (name + '.cxpolicy')), '--iterations', '240', '--envs', '1024', '--seed', '0', '--checkpoint-every', '20']
