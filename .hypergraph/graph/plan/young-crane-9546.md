...
1. **Deliver the named-plane section CLI (missions 6 and 2; damp-moon-9297).** Both old short units landed, including the separate fresh walk required before promotion; its remaining named leg is sections. One call takes a named axis plane (XY/XZ/YZ) and offset and writes a project-relative section artifact plus accepted revision/digest, plane, units, approximation and limits. Reuse bounded accepted display snapshots and the CPU rendering path where appropriate. Establish the smallest correct geometry contract: exact sections if available through the existing engine contract, otherwise explicitly labeled tessellation cuts with tested clipping and closed contours/caps. Preserve solved placements and cavities; distinguish empty cuts, unsupported/open geometry and errors. Test interior holes, offsets, translated/rotated components and degenerate plane contacts; a projected silhouette is not a section. Inspect real output contents, document the call and run the full built-engine CLI suite, reporting skips. Engine/protocol/payload changes require their additional AGENTS.md gates, at most one full build per unit. Delivery alone does not close review [rec: silver-key-8483] [rec: square-path-1173] [rec: modest-sun-6068] [rec: witty-brook-9419].
2. **Wire delivered sections into walk review (missions 6 and 2; damp-moon-9297).** Conditional on rank 1 landing. Reuse the review session and accepted snapshot; choose and document a reproducible named plane/offset valid for both established mechanisms without mechanism-specific dispatch. Include section availability, accepted revision/digest, plane, approximation/limits and project-relative artifact paths in review.json; commit outputs with project docs. Preserve four named views, catalog inventory and named-pair clearance. Refuse revision mismatch and expose errors without treating retained old files as current success; preserve explicit empty-cut semantics. Update scaffold/project_docs.py, mode-artifact table and CLI/walk docs together. Verify actual section contents and tracked outputs in real arm/carriage walks and local/remote-flag CPU stand-in parity without SSH or GUI. Measure acquisition, section generation and whole-walk costs separately. Run the full built-engine CLI gate and any additional touched-zone gates; keep each toy CPU training run below 15 minutes and 3 GB [rec: square-path-1173] [rec: modest-sun-6068] [rec: witty-brook-9419].
3. **Rehearse complete review on both documented mechanisms (missions 2 and 6; damp-moon-9297).** After wiring lands, run the same documented entry point on fresh hinged-arm and linear-carriage projects, one CPU training iteration/four environments/seed zero in the training venv, each run bounded to 15 minutes and 3 GB. This is the separate lifecycle evidence unit after implementation; any newly failed leg or critic must-fix leads instead. Verify policy witness, reward, accepted revision/digest, four visually inspected named-angle images, meaningful named-plane section, catalog-id inventory and named-pair clearance, all referenced by review and committed with project docs. Exercise section geometry with an interior cut rather than accepting only an empty result. Retain comparable PROGRESS.md numbers, commands, exits, sampled peak memory and explicit timing boundaries; 17.07 s and earlier samples are observations, not a benchmark. Keep the arm's known initial-pose contact and explicit unknown counts; do not claim swept-motion or large-assembly coverage. Run full built-engine CLI gate and report skips; retain evidence without committing checkpoints or rollout traces. Declare what remains or cite complete criterion evidence for maintainer judgement, never mark a charter gap done in the plan [rec: clear-ash-2884] [rec: square-path-1173] [rec: modest-sun-6068] [rec: witty-brook-9419].

The loop reports 15 iterations, 3.2 hours elapsed and 11.8 hours left. Three bounded units are ordered by dependency, not a promise to fill the budget. No new direction or charter-gap retirement is proposed; later criteria stay parked, inherited reduction remains standing work, and GUI/remote execution remains prohibited [rec: witty-brook-9419].
