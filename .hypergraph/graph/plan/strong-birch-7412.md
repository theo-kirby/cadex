...
1. **The lifecycle loop's last unmeasured leg (mission 2).** Every prerequisite the earlier passes named is spent: the decisions tail is retained (ADR-265), the ownership instructions are corrected (ADR-266), the seed spread is measured across four seeds, per-file history repair holds under six failure/retry regressions, and the section eye now cuts where the geometry is (ADR-267). None of it establishes the one thing the criterion is about — **a real provider reading a project's own history and using it to choose an iterate.** That is short unit 1, and it is now the only gap on this rung that a single iteration can close. Nothing else may displace it: no fresh mechanism, no extra seed, no rerun of a completed regression as its own unit, no general context subsystem [rec: steady-chart-0544] [rec: terse-crane-6585] [rec: morning-arrow-5841] [rec: fresh-clover-8521].

2. **What a human still needs in order to tick each seeded criterion.** The four criteria sit at `working`, and the reconciler is explicit that the tick "stays a charter decision rather than a maintainer one." Stated once, so no pass has to re-derive it. **The walk exists and is tested headlessly** — needs a clean end-to-end run on this machine whose design leg was driven by a real provider against project history; short unit 1 is that run. **The agent can see its work without a screen** — needs the four eyes exercised together on a real mechanism since ADR-267, which no run has done; short unit 2 is that, and unit 1 discharges it in passing. **Three modes, one shape** — needs nothing under this run's constraints: the remote limb is scripted through ADR-200/255/268 and the GUI limb documented through ADR-201, and its only honest gap, a real dispatch to a real box, is forbidden here. **The walk holds on a second mechanism** — already carried by three mechanisms walked side by side and by the two offline regressions pinning that the dispatch has no mechanism-specific branch; a fourth rig is worth less than it looks [rec: terse-crane-6585] [rec: western-grotto-7499] [rec: falling-willow-7995].
