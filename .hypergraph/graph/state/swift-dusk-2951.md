...
| two-servo leg scratch project, qualified payload [rec: proud-beacon-8002] | hip-and-knee leg, two revolute in series, two catalog MG90S position servos placed separately, M3 hardware | 539.06 s | 19.13 s | 1.82 s | 2.09 s | 565.72 s | 1.271 GB | -5144.792 over 200 steps (crouch pose -5144.792, effort -0.000227) |

All at 1 iteration × 4 environments (seeds 0, 1, 0, 0 and 0), all `--timeout 600`, all verified against the engine witness (4.48e-09, 3.679e-09, 4.028e-09, 2.882e-09 and 3.758e-09). The second mechanism differed in prompt, joint type (prismatic, not revolute) and actuator type (bounded force `motor`, not a position servo), and the walk's digest edit rewrote its `assembly.policy(...)` call with no refusal [rec: placid-sky-7374].
...
**A fifth mechanism through the unchanged entry point: the two-servo leg (2026-09-08).** Once eligible, the same `cadex walk --prompt` against the qualified staged payload, with the tree clean at `8b8d4155` and no mechanism-specific code, designed and carried a hip-and-knee leg — two catalog MG90S position servos placed as separate components, four independently modeled printed solids, M3 bolts and nyloc nuts — through design, train, declare, verify/rollout and review, four legs exit 0. Its `PROGRESS.md` carries the same rows and metric definitions as the earlier prompt-walk projects, and the two installed recipes retain their comparison at the same 1 × 4 / seed 0 settings (arm -27.1093842209, carriage -24159.1953563 over 50 steps). This is the first walked mechanism whose review touched separately placed catalog parts (6 catalogued instances of 10 components). Reconcile judgement: retain `working`; corroboration on a fifth topology, the rehearsal the previous record left pending now supplied, nothing the criterion still needs. The leg's -5144.792 is a different pose objective, different weights and a 4 s horizon, so it ranks nothing; the mechanism is not mechanically qualified (shin/bolt interference and unattached servo drives are review findings on `damp-moon-9297`) [rec: proud-beacon-8002].
