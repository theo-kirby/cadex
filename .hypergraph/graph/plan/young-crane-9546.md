...
1. **Disable only the standalone Test Tk runner's copy/install row (mission 3; round-glacier-2865).** Follow docs/TEST-TK-AUDIT.md and ADR-230: remove only unittestgui.py from Test_SRCS, retaining its source and every other Test file, TestSources, MainCmd dependency and headless registration. Reconfigure debug/release and verify regenerated copy/install rules omit it; quarantine inventoried stale copies/bytecode from debug, release and install. Use at most one full release build plus install and completed stage before payload readers. Run full engine pytest, packaged lifecycle/licensing, the GUI-denied installed TestApp.TestText('UnitTests') probe, Cadex CTests and inherited CTest comparison against the recorded baseline, reporting failures/skips and inventory differences. Verify the inherited modification notice and manifest against committed HEAD; update ADR, ROADMAP and relevant dated docs, and record source/stage identities and local relocation limits. One coherent disable commit and work record; a newly found dependency or unexplained gate failure blocks deletion and requires replanning. No whole-Test, Qt runner, shell, Windows launcher or servo changes. [rec: humble-tide-6752] [rec: twilight-wolf-7995]
2. **Delete only the verified-disabled unittestgui.py source (mission 3; round-glacier-2865, green-sea-3991).** Conditional on rank 1's separate committed disable, work record and fresh passing gates or explained pre-existing baseline. Delete the audited 399-line source only, retaining all other Test consumers/resources and upstream licensing history. Repeat applicable fresh configure/build/install/stage verification with at most one full build; finish stage before full engine and packaged lifecycle/licensing checks, rerun the GUI-denied retained text path and Cadex/inherited CTest baseline comparison, and prove stale runner copies/bytecode are absent. Update ADR-230, ROADMAP, ledger/dates and manifest if actual inherited differences require it; verify committed-HEAD manifest equality. Report whole-file savings separately from surviving modified-file counts/insertions/deletions against the run baseline. One delete commit plus work record, with actual output and source/payload limitations; this does not close whole-Test, broad GUI-source or broad fork-delta obligations. Replan after this pair rather than repeat stopped searches or invent another removal. [rec: humble-tide-6752] [rec: twilight-wolf-7995]
