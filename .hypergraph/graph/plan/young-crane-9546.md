...
1. **Rerun the documented local toy walk (mission 2; damp-moon-9297).** Old short work is exhausted: clearance wiring included clean public arm/carriage evidence, and the rendering probe landed. Honor the empty-short rule with a separate evidence unit now. Run the documented hinged-arm `cadex script` then `cadex walk` against the built engine, local training venv, one iteration, four environments and seed zero; cap each training run at 15 minutes and 3 GB. Record commands, exits, wall time, peak memory, policy verification, rollout numbers and project-relative artifacts. Assert inventory and clearance are in review and the project commit, including the arm's below-clearance finding and zero unknown pairs. Compare whole-walk cost with 14.5173 s (and the earlier 14.39 s), separately from rebuild timings. Run the CLI zone gate and report skips. Name missing render/section legs; any actual failed leg becomes the next must-fix before implementation. Do not bundle rendering into this evidence commit [rec: clear-ash-2884] [rec: modest-summit-8554] [rec: mellow-beacon-8815].
2. **Deliver the named-angle CPU renderer (mission 6; damp-moon-9297), after the rerun or its repair.** One CLI call writes front/top/right/iso SVGs plus a revision-bearing summary under a documented project path. Follow ADR-239's independently authored accepted-tessellation route, with no Blender rendering dependency or new package. SVG replaces the earlier unproven PNG assumption. Snapshot display buffers before rebuild invalidates them; preserve accepted revision, solved placements exactly once, visibility and exclusion of unposed definition copies. Bound input and image work; refuse malformed or excessive buffers clearly. Resolve the probe's centroid-order occlusion limitation for supported geometry and document tessellation approximation. Test real image contents, angle distinctions, overlapping/depth geometry, translated/rotated components and curved tessellation, plus empty/error and revision fidelity cases. Inspect resulting images, measure acquisition and rendering cost separately, and state unsupported scale honestly. Keep the CLI LGPL boundary, docs, ADR/ROADMAP and full CLI gate; any engine/protocol/payload change also requires its zone and packaged gates. This is one product-call unit; walk wiring is rank 3 [rec: zesty-aspen-6846] [rec: mellow-beacon-8815].
3. **Wire named-angle artifacts into walk review (missions 6 and 2; damp-moon-9297), only after renderer delivery.** Reuse the delivered renderer and existing review inspection session where possible. Include explicit availability, accepted revision, angle names, approximation/limits and project-relative paths in review.json; commit images and summary with project docs and retain inventory/clearance outcomes. Update scaffold/project_docs.py, mode-artifact table and CLI/walk docs together. Real arm/carriage walks assert image contents and tracked reports; extend local/remote-flag CPU stand-in parity without SSH or GUI. Measure whole-walk overhead and run the full built-engine CLI suite with bounded toy CPU training, reporting skips honestly. Keep rendering errors visible and section absence explicit; this unit does not close the whole review criterion [rec: clear-ash-2884] [rec: gilded-basin-9946] [rec: zesty-aspen-6846] [rec: mellow-beacon-8815].

No medium promotion beyond these units until short lands or blocks; critic must-fixes lead. Whenever short empties, the documented local walk rerun leads again. The remaining run signal is 12.6 h after 11 iterations; these are bounded units, not a promise to consume it. Parked Later criteria stay outside dispatch and inherited reduction stays standing work [rec: modest-summit-8554] [rec: mellow-beacon-8815].
