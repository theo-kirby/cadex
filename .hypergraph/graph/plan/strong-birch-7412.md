...
1. **The lifecycle walk's remaining unmeasured shape (mission 2).** The leg that was open here is closed: `runs/uninterrupted-46` was a single `walk --prompt … --resume` invocation, start to review, exit 0 in 103.245 s, whose design turn read `ot4-quill`'s own record, rejected one recorded finding with reasons and took the other as project ADR-008 [rec: sage-glacier-2165]. Two shapes are left, one iteration each and in this order. **From nothing in one invocation, on a mechanism this machine has not built** — short unit 1; the two from-nothing walks that exist here predate ADR-271, ADR-273 and ADR-274, so no walk on this machine has yet exercised the evening's fixes on geometry they did not see. Then **ADR-274's follow-up, live** — it fired zero times in the clean run because the first turn called tools, and only an exit-3-by-silence can exercise it, so it is a thing to watch for in the next walks' `notes` line rather than a unit to schedule. Nothing may displace the first: no extra seed on a walked rig, no rerun of a completed regression as its own unit, no general context subsystem, and no token-free floor taken *in place of* an attempt — though the token-free units below are exactly what an iteration ships when an attempt is refused [rec: swift-orchard-8539] [rec: sage-glacier-2165] [rec: sleepy-mesa-0821].

2. **What a human still needs in order to tick each seeded criterion.** All four sit at `working`, and the reconciler is explicit that the tick "stays a charter decision rather than a maintainer one". Stated once, so no pass re-derives it. **The walk exists and is tested headlessly** — one uninterrupted invocation has now run (`uninterrupted-46`); what is not in *one* run is design-from-nothing plus the current fixes, which short unit 1 supplies. **The agent can see its work without a screen** — the four eyes have run together on live geometry across three rigs, and ADR-273 replaced the section's ranking outright; the honest remainder is not a search but a report, which is short unit 3: the eye still leaves a reader to notice, unaided, that the object it missed was the moving one. **Three modes, one shape** — needs nothing under this run's constraints: the remote limb is scripted (ADR-200/255/268) and the GUI limb documented leg by leg and pinned (ADR-201/269); its only honest gaps, a real dispatch and a real GUI-attached run, are forbidden here. **The walk holds on a second mechanism** — met twice over, and pinned behaviourally by ADR-260's two dispatch regressions; a crank-slider adds joint-kind coverage as a by-product of short unit 1 rather than as a fourth sample [rec: swift-orchard-8539] [rec: sage-glacier-2165] [rec: rising-rain-0117] [rec: cool-eagle-6400] [rec: falling-willow-7995].

3. **Carry `--detach` through the CLI's walk (mission 2).** Unchanged and still demoted, not dropped. `idle-falcon-3004` closed the remote limb's last declared shape difference — a warm start travels and re-points — and named this as the one left: "`--detach` still does not travel through the CLI's walk." Small, self-contained, `cli/` plus `training/`, no engine, protocol, payload or `shell/` diff, testable against the stand-in `ssh`/`rsync` box fixture that record already built. It sits here because it polishes a limb whose remaining honest gap this run's constraints forbid either way, so it must never displace a walk leg or an eye that still misses a part [rec: sleepy-mesa-0821] [rec: idle-falcon-3004] [rec: terse-crane-6585].

4. **Delivered work leaves dispatch.** Seeds 0–3 completed with fixed exported model/task and objective/action identities, training seeds varied and rollout seed 7 held; reward 170.952826823611–175.935971673601, translation 30.078469436328–31.421759939733 mm, rotation zero, every review succeeding and prior artifacts surviving. History updates replace complete files only after successful writing. Recent DECISIONS entries survive the bounded prompt on fresh and resumed turns, and the lifecycle guide no longer promises unconditional repository initialization. The section offset is derived from the accepted snapshot's own bounds and since ADR-273 ranked by objects actually cut, with `cadex section --plane/--offset-mm` unchanged; the remote dispatcher carries and re-points a warm start; and a second walk of a project no longer locks it out (ADR-272). Protection is per-file, not concurrency, multi-document or power-loss safety; context availability is not model understanding [rec: honest-banner-0821] [rec: solar-snow-8543] [rec: western-cliff-6876] [rec: idle-falcon-3004] [rec: rising-rain-0117].
