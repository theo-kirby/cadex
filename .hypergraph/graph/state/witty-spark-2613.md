...
**The domain-note channel is the same in every mode (ADR-245).** A closing `NOTE <subject>:` line in a design turn lands `docs/<subject>.md` beside `ARCHITECTURE.md`, `DECISIONS.md` and `PROGRESS.md`, written by the CLI through the closing-text convention `DECISION:` lines already use, with no shell or engine surface touched. So the three modes keep one shape: headless, GUI-attached and remote all produce the same project documents at the same project-relative paths, and the GUI-attached agent, which has Mesh tools only, still leaves those documents to the CLI or a person. The record also confirms, against an overseer list that asked for them again, that the remote handoff (ADR-200) and the GUI-attached doc (ADR-201) are already shipped and evidenced here and were not reopened [rec: lucid-pebble-4000].
