...
The open gaps in the order they should fall. The order is the charter's: the walk leads this run, the eyes are the walk's own review step, the two documentation legs of three-modes are the fallback when the walk is blocked, and the second mechanism cannot start until the first walk is green [rec: ancient-key-7299].

1. **The walk exists and is tested headlessly (`crisp-reef-5607`; missions 1 and 2).** The one open leg is the dynamics layer. The first walk on this machine built the rig — MG90S from the catalog, ten components, three joints, one project commit, `docs/actuators.md`, `docs/sensors.md` and `docs/rejected.md` — and then gated the whole training layer behind `policy_on=0`, because `assembly.mjcf` and `assembly.dynamics` burn the 300 s worker CPU cap. Units: the declaration bisect, the SIGXCPU legibility repair, and the end-to-end re-run; then, only if the bisect finds a cost that cannot be fixed cheaply, one bounded unit that lets a rig of this size publish a task at all. The criterion's own named leg — toy-scale local CPU training from a prompt-designed mechanism — has never been reached on this machine [rec: open-hollow-2140] [rec: ancient-key-7299].

2. **The agent can see its work without a screen (`damp-moon-9297`; missions 2 and 6).** The four calls exist and were exercised on the previous machine: named-angle renders, a section through a named plane, an assembly inventory with catalog ids, and a clearance check that names offending pairs, with the walk's review leg using them. What is not established here is that they run on this machine's engine and payload; the failed walk exited before its review leg. This gap follows the walk because the walk exercises it, not because it is less important. Standing limits: views and sections describe the initial solved pose, not swept safety; bound agreement is AABB consistency, not OCCT volume validation; SVG wraps a lossless CPU raster, not analytic vectors [rec: silver-key-8483] [rec: southern-otter-5999] [rec: morning-summit-7848].

3. **Three modes, one shape (`witty-spark-2613`; mission 2).** Two units, both documentation and script only, neither executing what the run's constraints forbid: the remote-training handoff, scripted and documented against `training/SETUP.md` and `remote_train.sh` (ADR-089), and the GUI-attached mode, documented against the shell's client while the headless-only constraint holds. These are the only medium units that do not depend on the dynamics stall, which makes them the fallback rung when unit 1 is blocked mid-iteration rather than filler [rec: ancient-key-7299] [rec: forest-shade-2752].

4. **The walk holds on a second mechanism (`swift-dusk-2951`; mission 2).** Blocked on unit 1 by construction: the criterion asks for the same entry point, with no code change specific to the mechanism, taking a second mechanism through the whole loop, with both projects' `PROGRESS.md` carrying comparable numbers. There is no comparable pair to extend while the first walk exits 3. Do not open this by re-rolling design prompts [rec: ancient-key-7299] [rec: fond-star-1809].

5. **The RL training loop and mg-legs (`late-pond-2851`; mission 5), parked.** The loop is proven; what is blocked is gait scale, and a real training run is not a unit of work and never fits in an iteration. The GPU is available to this run and is not this run's target. Toy witness success establishes no learned gait [rec: proud-beacon-8002] [rec: vast-ledge-4610].

6. **Parts library (`brave-stone-9609`; mission 4), parked under Later criteria.** Catalog breadth, manufacturer STEP provenance, L3 motors and compound mechanisms are all in `## Later criteria`; the planner may not target them. Catalog placement has held on two fresh leg designs — six separately placed instances each, 39 catalog pairs, no catalog body entering a printed-solid boolean — while pan-tilt previously fused its hardware, so occurrence ambiguity and cold identity remain open and unselected [rec: early-gate-3510] [rec: morning-summit-7848] [rec: fond-star-1809].

7. **Inherited-tree reduction (`round-glacier-2865`; mission 3), standing only.** nt2 removed 2.79M lines and the charter has retired reduction as a target for this run. Take a removal only when an authorized change makes it obvious and cheap; never as an iteration's target and never as a fresh audit campaign [rec: modest-summit-8554] [rec: autumn-arrow-3125].

Preserved from the previous horizon and not reopened by this pass: the completed worker-identity repair (ADR-244, engine 2085 passed / 52 skipped, CLI 195 passed, packaged lifecycle 15 passed), the closed clearance direction, the completed refusal-metadata work (ADR-247), the first-visit Git, consent, suffix-parity and linked-part qualifications, and ADR-245/246. Their measurements live in `long` and in the records cited there; none of them is a unit now. The durable nt3-leg project stays preserved outside this repository at HEAD `b244f26`, accepted revision `417d0fdf3286`, digest `e6395e3e49c4`, dirty only in recorded runtime metadata — account for that baseline on any authorized visit; do not reset or commit it to manufacture success [rec: lawful-dune-3795] [rec: southern-otter-5999] [rec: ready-otter-2700] [rec: fond-star-1809] [rec: sunny-walrus-5847].
