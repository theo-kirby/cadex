...
1. **Wire the clearance check into the walk's review step (mission 6; damp-moon-9297).** After short rank 3 lands with its record [rec: restless-star-0524]. Same shape as the inventory wiring: the review leg runs the reader, `docs/clearance.md` lands in the project, `review.json` gains a `clearance` block (pairs checked, offending pairs by label and catalog id, the thresholds used), `PROGRESS.md`'s row carries the offending-pair count so a design change that introduces an interference is visible in the comparison; `project_docs.py`, the scaffold and `docs/CLI.md` in one commit with a test [rec: wild-marsh-9611]. A walk does not fail on an offending pair: it reports it, because the walk is the review, not the gate.
2. **Headless review, render from named angles (mission 6; damp-moon-9297).** One CLI call, PNGs under a documented project path, from a fixed set of named angles (front, top, right, iso at least) of the accepted revision. First step is a probe, recorded either way: whether the shell's blueprint renderer runs under the bundle's Blender in background mode as an arm's-length subprocess, the way CalculiX is driven (ADR-142), inside the charter's no-GUI constraint; the CLI is LGPL and copies nothing from `shell/`, so the route is a subprocess on the bundle or a render from the accepted BREP's tessellation with no Blender at all, and the unit says which and why. The output is a project document in the ADR-193 sense, indexed the way blueprints are (ADR-150) if the store already has the slot. Then wire it, as its own unit [rec: wild-marsh-9611]. [rec: flat-river-8853]
3. **Headless review, section through a named plane (mission 6; damp-moon-9297).** The same call shape as rank 2 with a plane named by axis and offset, or by an accepted output's face; the engine already cuts sections for the cage overlay (ADR-127), so the cut is engine-side and only the picture is the question rank 2 answers. Then wire it. With ranks 1 to 3 landed and short rank 3, all four calls exist and the review step uses them, which is the criterion's exact wording; the record of the last one says whether `damp-moon-9297` can be ticked. [rec: flat-river-8853]
4. **The one thing three-modes still lacks, if short rank 1 names one (mission 2; witty-spark-2613).** Conditional: exists only if the audit finds the criterion's wording unmet. A warm start under `--remote` is refused before any leg [rec: green-delta-7130] and stays refused; documenting it is not a gap. Never a dispatch, never a GUI launch. [rec: restless-star-0524]

Withdrawn this pass: the conditional unit for a named design-leg cause, because both prompt re-runs ran clean and the envelope that would name a cause is test-pinned [rec: placid-sky-7374] [rec: ancient-wind-0117]; and the remote-handoff unit, which short rank 1 now carries as part of the three-modes audit [rec: restless-star-0524].

Operator re-plan before nt3 (2026-09-07) [rec: modest-summit-8554]: the fork-delta, manufacturer-geometry, catalog-breadth, residual-Phase-8 and L3-motors units that filled this rung are withdrawn. Their criteria are parked under `## Later criteria`; their negative knowledge below stands and must not be relearned. Inherited-tree reduction is standing work on the long rung now, never the unit of an iteration.
