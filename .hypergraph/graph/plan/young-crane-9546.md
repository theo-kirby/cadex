...
1. **Remove the stale engine-suite count from command guidance (missions 1 and 2).** AGENTS.md's `pixi run test-engine` comment still claims 1757 tests; pixi.toml points that task at the full engine pytest directory, and ADR-244 already recorded 2085 passed / 52 skipped. Inspect those exact facts, remove only the parenthetical count, retain the command and no-build guidance, and adjust spacing/date if needed. Record the narrow removal in the relevant existing ADR and one work record targeting early-arbor-7123. No product ROADMAP checkbox is earned. Verify hypergraph export/check and git diff --check; no product suite or build is required or claimed for this documentation-only change. No replacement count, historical evidence rewrite, wider audit or tool upgrade [rec: lawful-dune-3795] [rec: cool-raven-8938].

2. **Repair only a verification failure introduced by unit 1 (missions 1 and 2; conditional).** Correct exactly an introduced documentation/graph failure and re-run that check plus export/check. If verification passes, this unit is non-dispatchable and the controller returns directly to separate planning. Do not dispatch an actor merely to record another handoff, repeat a successful check or find an adjacent audit; pre-existing failures require separate planning [rec: weathered-trail-0874] [rec: shy-beacon-2513] [rec: cool-raven-8938].

The onboarding version correction passed export/check with zero violations/warnings and whitespace verification. Its repair is exhausted; the maintainer has reconciled through the actor's role-transition dead end and this pass supplies separate planning. Neither event adds lifecycle evidence [rec: weathered-trail-0874] [rec: shy-beacon-2513] [rec: cool-raven-8938].

Linked-part source independence, output-loss consent/cold recovery and source suffix parity remain qualified; no repeat or adjacent qualification is selected. Existing lifecycle evidence identifies no missing leg requiring another baseline [rec: calm-sky-2656] [rec: lawful-union-8346] [rec: silver-rain-7333] [rec: empty-ledge-4581] [rec: lucid-snow-0350] [rec: scarlet-journey-0203].

Signals: 199 iterations, 12.2h elapsed, 2.8h left, $85.47 API-equivalent; Claude five_hour 65%, seven_day 54%; Codex seven_day 50%. One exact standing-maintenance direction selected from three; no Later promotion or charter retirement/reopening. Live repair stays controller-parked: no retry, availability probe, repeated hold record, provider switch, paid fallback, training, stage refresh, GUI or remote execution [rec: sunny-walrus-5847] [rec: cool-raven-8938].
