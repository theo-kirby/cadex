...
1. **Qualify the payload schema check's actual boundary (missions 1, 2 and 6).** Use a disposable fixture following existing engine-resolution tests: a nonempty schema mismatch must refuse resolution; matching schema with an absent unrelated worker/API module should still resolve. Record fixture, commands and results and run existing engine-resolution tests. This qualifies resolution only, never worker execution or import closure. Do not alter the real bundle, commit new tests, build or train. A contradictory result ends the direction and withdraws rank 2. Protect working walk/review criteria crisp-reef-5607 and damp-moon-9297; no runtime defect is alleged [rec: spring-wolf-7431] [rec: northern-hill-9362].
2. **Conditional removal of the payload-coherence overclaim (missions 1, 2 and 6).** After rank 1 confirms the boundary, replace docs/CLI.md section 6's claim that a payload assembled from two trees fails before a frame with the actual schema-mismatch check and its limits. Correct only the matching engine.py comment if needed; change no runtime behavior. Net-subtractive prose, doc date and removal ADR; apply ROADMAP conventions without reopening criteria. Check scaffold consistency, run full CLI gates if touching cli/ with the refreshed engine and bounded CPU settings, and report selection/skips honestly. No broad audit, fingerprint mechanism or replay walk. Clean correction exhausts this direction [rec: spring-wolf-7431].

The ordinary bundle refusal and supported refresh both landed: complete installed-engine walk, 15 packaged lifecycle and 195 CLI passes, plus matching built-bundle background shell gate. Neither packaging unit remains dispatchable. Signals: 29 iterations, 4.7h elapsed, 10.3h left; only the two bounded guidance units are selected. GUI stays unlaunched and Later criteria parked [rec: northern-hill-9362] [rec: strong-raven-3067] [rec: spring-wolf-7431].
