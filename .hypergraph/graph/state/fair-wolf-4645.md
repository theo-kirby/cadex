...
**D11 now has a current 90 mm `wren71-final` comparison on the persistent Wren dashboard.** Same-pose/camera viewport/capture PNGs are byte-identical; decoded-video mean absolute RGB error is 1.50058/255 (threshold 3). Identified neural-whoop reference modules at `31caeb28…` were restaged with Wren geometry at fit/close/wide cameras, alongside an actual shipped dark-reference frame. Visual inspection found matching grey grid, fog/sky fade, palette, lighting and grounded shadows; the slightly crisper close shadow is consistent with ADR-301's fitted frustum/bias and demonstrates no mismatch requiring correction. Real orbit/zoom and underside inspection show no stage edge or seam; the finite blue plate is model geometry. Current and historical video playback/download survive polling and return to current; the same server remains active with 18 runs. Later decoded frames show a bent standing pose, not walking. This resolves the historical-110-mm-only comparison limit; evidence remains same-machine private-address only [rec: terse-walrus-5414].

Earlier 110 mm Wren comparison retained byte-identical PNGs and decoded error 1.5044/255, with 23 evidence tests and no full-suite claim for its late backfill [rec: kind-oak-1484].
