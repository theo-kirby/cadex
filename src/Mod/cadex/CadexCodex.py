...
Cadex never reads or handles ChatGPT OAuth tokens.  The pinned Codex
...
CODEX_APP_SERVER_ENV = "CADEX_CODEX_APP_SERVER"
CODEX_HOME_ENV = "CADEX_CODEX_HOME"
...
def codex_home() -> Path:
    ...
    if sys.platform == "win32":
        ...
        return root / "Cadex" / "Codex"
    if sys.platform == "darwin":
        return Path.home() / "Library" / "Application Support" / "Cadex" / "Codex"
...
def cadex_thread_config(
    *,
    web_search_enabled: bool = False,
    skills_enabled: bool = False,
) -> dict[str, Any]:
    """Build the narrow Codex capability configuration for one Cadex turn."""
    config: dict[str, Any] = {
...
def resolve_runtime_command() -> CodexRuntimeCommand:
    ...
    if not executable.is_file():
        raise CodexAppServerError(
            "The bundled Codex app-server runtime is missing. Reinstall Cadex "
            "with ChatGPT subscription support."
        )
    version = _manifest_version(root)
    if version != CODEX_APP_SERVER_VERSION:
        raise CodexAppServerError(
            "The bundled Codex app-server version does not match Cadex: "
    ...
    def start(self) -> None:
        with self._state_lock:
            ...
            self._reader_thread = threading.Thread(
                ...
                name="Cadex-Codex-stdout",
                daemon=True,
            )
            self._stderr_thread = threading.Thread(
                target=self._read_stderr,
                name="Cadex-Codex-stderr",
                daemon=True,
            )
            ...
            self.request(
                "initialize",
                {
                    "clientInfo": {
                        ...
                        "title": "Cadex",
                        "version": str(
                            os.environ.get("CADEX_VERSION") or "development"
                        ),
                    },
    ...
    def _dispatch_message(self, message: dict[str, Any]) -> None:
        ...
        if request_id is not None and method:
            thread = threading.Thread(
                ...
                name=f"Cadex-Codex-request-{method}",
                daemon=True,
            )
            thread.start()
            return
            ...
            self._write_message(
                {
                    ...
                    "error": {
                        "code": -32601,
                        "message": f"Cadex does not handle {method}.",
                    },
                }
            )
            return
...
def load_codex_skill_catalog(
    ...
    *,
    ...
    cadex_root = codex_home() / "skills"
    if personal_root.is_dir():
        personal_resolved = personal_root.resolve()
        cadex_resolved = cadex_root.resolve()
        if personal_resolved != cadex_resolved:
