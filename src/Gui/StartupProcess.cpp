...
void StartupPostProcess::activateWorkbench()
{
    ...
    migrateCadexBackgroundAutoload(wb);
...
void StartupPostProcess::migrateCadexBackgroundAutoload(const QStringList& workbenches)
{
    constexpr auto migrationKey = "CadexBackgroundAutoloadModules2026";
    ...
    // The original Cadex preference pack enabled every then-supported modeling
    ...
    if (configured == legacyPackModules
        ...
        Base::Console().message(
            "Removed Cadex's obsolete all-workbench background autoload preference.\n"
        );
    }
