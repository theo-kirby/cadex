...
**Coverage baseline audited (ADR-212):** the existing completed payload passes 90 lifecycle/library tests without skips. All four L3 values publish in canonical and placed form, but N20 has publication-only kernel coverage rather than the dedicated interface probes present for BLDC/L12/joint. N20 interface probes are the smallest evidence-only follow-up. `docs/L3-COVERAGE.md` separates nominal geometry and source ratings from coupling fit, S-switch powered reachability and physical inertia; the actuator helper belongs to `ServoPart`, not ordinary L3 `LibraryPart` values. This audit reran neither the full engine suite nor build/staging, and establishes no portable-release claim [rec: steady-rain-3009].
