...
1. **The lifecycle loop's last unmeasured leg (mission 2).** Every prerequisite the earlier passes named is spent: the decisions tail is retained (ADR-265), the ownership instructions are corrected (ADR-266), the seed spread is measured across four seeds, per-file history repair holds under six failure/retry regressions, and the section eye now cuts the moving part in a live `review.json` (ADR-267 corrected by ADR-270). None of it establishes the one thing the criterion is about — **a real provider reading a project's own history and using it to choose an iterate.** That is short unit 1; it is still the only gap on this rung that a single iteration can close, and four accepted token-free units have now gone by without it being attempted. Nothing may displace it: no fresh mechanism, no extra seed, no rerun of a completed regression as its own unit, no general context subsystem, and no token-free floor taken in place of an attempt [rec: candid-fern-3092] [rec: steady-chart-0544] [rec: terse-crane-6585] [rec: morning-arrow-5841] [rec: fresh-clover-8521].

2. **What a human still needs in order to tick each seeded criterion.** The four criteria sit at `working`, and the reconciler is explicit that the tick "stays a charter decision rather than a maintainer one." Stated once, so no pass has to re-derive it. **The walk exists and is tested headlessly** — needs a clean end-to-end run on this machine whose design leg was driven by a real provider against project history; short unit 1 is that run. **The agent can see its work without a screen** — the four eyes have now been exercised together on a live mechanism (`copper-canyon-0231`: quill `empty` → `ok`, housing 1 → 3 loops, render / inventory / clearance unchanged), so what is left is narrower than it was: ADR-267's swing-arm and carriage numbers still rest on box stand-ins and have never cut live geometry. That is short unit 2, and it is token-free. **Three modes, one shape** — needs nothing under this run's constraints: the remote limb is scripted through ADR-200/255/268 and the GUI limb documented leg by leg and pinned through ADR-201/269, and its only honest gap, a real dispatch to a real box, is forbidden here. **The walk holds on a second mechanism** — already carried by three mechanisms walked side by side and by the two offline regressions pinning that the dispatch has no mechanism-specific branch; a fourth rig is worth less than it looks, and short unit 2 feeds `swift-dusk-2951` for free without building one [rec: candid-fern-3092] [rec: copper-canyon-0231] [rec: cool-eagle-6400] [rec: terse-crane-6585] [rec: western-grotto-7499] [rec: falling-willow-7995].

3. **Delivered work leaves dispatch.** Seeds 0–3 completed with fixed exported model/task and objective/action identities, training seeds varied and rollout seed 7 held; reward 170.952826823611–175.935971673601, translation 30.078469436328–31.421759939733 mm, rotation zero, every review succeeding and prior artifacts surviving. History updates replace complete files only after successful writing. Recent DECISIONS entries survive the bounded prompt on fresh and resumed turns, and the lifecycle guide no longer promises unconditional repository initialization. The section offset is derived from the accepted snapshot's own bounds with `cadex section --plane/--offset-mm` unchanged, and the remote dispatcher now carries and re-points a warm start. Protection is per-file, not concurrency, multi-document or power-loss safety; context availability is not model understanding; derived coverage was computed from stored summaries, not a live cut. No additional failure-hardening or context campaign follows [rec: honest-banner-0821] [rec: solar-snow-8543] [rec: western-cliff-6876] [rec: idle-falcon-3004].

4. **Keep comparison and retention limits.** Four-seed ranges are descriptive; the 30 mm action midpoint remains a confound and neither matching objectives nor favorable travel proves equivalent control difficulty or learned improvement. Root review exclusions and policy exclusions after default negations govern untracked files; existing tracked/staged paths retain ordinary Git semantics. Legacy identity rows remain unavailable and numeric comparison references may differ from same-kind identity references. A derived section offset changes the artifact path for future runs and proves no motion clearance. No automatic migration, untracking or standalone-rollout reporting work follows [rec: honest-banner-0821] [rec: easy-beacon-3803] [rec: modest-prairie-3356] [rec: steady-chart-0544].

5. **Keep the charter boundary.** Four seeded criteria are working in the projection. Remote handoff remains scripted/documented and GUI attachment documented under execution constraints. Keep mode rehearsals, inherited-removal campaigns, catalog, fleet, report rendering and gait training parked. Kernel rollout clearance remains unselected on long; quill overlap is deliberate and initial-pose review does not prove swept fit. No charter gap is deleted, retired or marked done by this pass [rec: restless-cabin-9629] [rec: western-grotto-7499] [rec: sleepy-hollow-9498] [rec: chilly-crest-2100].
