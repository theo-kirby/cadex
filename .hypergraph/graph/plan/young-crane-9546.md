...
1. **Fresh hinged-arm complete-review rehearsal (missions 2 and 6; damp-moon-9297).** Section delivery and shared-snapshot integration landed; perform the separate evidence run now. Use the documented public entry point on a fresh project, one local CPU training iteration/four environments/seed zero in the training venv, bounded to 15 minutes and 3 GB. Verify policy witness and reward; accepted revision/digest agreement across rollout, review, render and section; four visually inspected named-angle images; a meaningful XZ section at Y = 3.125 mm; inventory catalog-id fields with truthful uncatalogued synthetic components; and named-pair clearance retaining the arm's known initial contact and explicit unknown counts. Verify actual contour contents and clean committed project docs/artifact bytes. Record reproducible commands, exits, sampled peak process-tree memory, comparable PROGRESS.md numbers and separate acquisition/render/section/whole-command timing boundaries. Run the full built-engine CLI suite and report skips. Retain compact evidence without checkpoints, rollout traces or machine paths. This advances review evidence but the second mechanism remains required [rec: placid-lily-5624] [rec: copper-timber-8947] [rec: sage-crow-3224].
2. **Fresh linear-carriage rehearsal and comparison (missions 2 and 6; damp-moon-9297).** After rank 1 lands, use the identical documented entry point with the existing carriage script and no mechanism-specific code change. Apply the same CPU bounds, seed, environments, artifact inspection, revision checks and full built-engine CLI gate. Visually inspect all four named views and the meaningful XZ Y = 3.125 mm section; verify inventory and named-pair clearance, preserving the expected separated pair and truthful unknowns. Both projects' committed PROGRESS.md must carry comparable reward/witness/review numbers, with units and measurement boundaries; compare to the fresh arm evidence rather than treating older timings as a benchmark. Cite the combined evidence against every headless-review clause and preserve documented-only GUI mode and scripted remote handoff, citing existing local CPU stand-in parity. Declare any remaining gap or complete evidence for maintainer judgement; never mark a charter gap done in the plan. Any newly failed leg or critic must-fix leads instead [rec: clear-ash-2884] [rec: copper-timber-8947] [rec: sage-crow-3224].

The loop reports 17 iterations, 3.5 hours elapsed and 11.5 hours left. These two units split the previously planned two-mechanism rehearsal; no new direction, medium implementation promotion or charter-gap retirement is proposed. No GUI, SSH, provisioning or GPU dispatch. At most one full build per unit, only if its touched zone requires it; source CLI evidence does not qualify an ordinary bundle. If both units land or block, re-plan from their evidence without promoting Later criteria [rec: modest-summit-8554] [rec: sage-crow-3224].
