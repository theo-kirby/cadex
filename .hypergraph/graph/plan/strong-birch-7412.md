...
1. **Wire clearance into the walk's review step (mission 6 and 2; damp-moon-9297).** Promote only after the two outstanding short units land or block and the clearance call has accepted evidence [rec: crimson-canyon-5993]. The review reader lands `docs/clearance.md`; `review.json` carries pairs checked, offending pairs by label/catalog id and thresholds; `PROGRESS.md` carries offending-pair count for comparison. An offending pair is a review finding, not an automatic walk failure; distinguish measurement failure from a clear result. Keep `project_docs.py`, the scaffold, shared mode-artifact table and CLI docs in one tested commit, and preserve whole-walk stand-in parity [rec: wild-marsh-9611] [rec: gilded-basin-9946]. This follows the already-available inventory wiring so each call enters the product walk as it lands [rec: modest-summit-8554].
2. **Headless review, render from named angles (mission 6; damp-moon-9297).** One CLI call, PNGs under a documented project path, front/top/right/iso at least, of the accepted revision. Start with a bounded probe: can the existing blueprint renderer run in the bundle's Blender background process under the no-GUI constraint, without copying GPL shell code into LGPL CLI? Otherwise assess rendering accepted tessellation with no Blender. Record the selected route and its evidence before implementation; reuse the project-document store where it already supports the artifacts. Then wire the call into the walk in a separate unit with scaffold/docs and gate evidence. No replacement shell or engine. [rec: flat-river-8853] [rec: wild-marsh-9611]
3. **Headless review, section through a named plane (mission 6; damp-moon-9297).** Reuse the rendering route proven at rank 2; engine-side section geometry, plane named by axis and offset or an accepted output's face, one CLI call and a project-relative output. Then wire it as a separate unit. The final evidence unit checks all four calls and their use in the walk on the two established mechanisms, and declares precisely what remains before damp-moon-9297 is met; the planner does not mark the criterion done. [rec: flat-river-8853] [rec: placid-sky-7374] [rec: crimson-canyon-5993]

The conditional three-mode repair is no longer needed: the audit found no missing item under the exact charter wording and supplied real-engine stand-in artifact parity. This removes a conditional dispatch, not a charter-derived gap; remote transport and concurrent GUI mutation remain unexercised by design [rec: gilded-basin-9946]. No medium work is promoted while inventory wiring and clearance remain outstanding in short [rec: crimson-canyon-5993].

Parked catalog, fleet, outside-source, report and reduction criteria remain outside dispatch; retain their negative knowledge rather than repeating prior audits [rec: modest-summit-8554].
