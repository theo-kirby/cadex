...
def render_blueprint(theme="blueprint", max_size=1024, views=None,
    ...
    *current presentation* as a sheet, per-view overrides layered on top of
    ...
    callouts = []
    dimensions = []
    # The declared measurements, resolved once against this response's
    # display block — the viewport overlay's own reader, so a sheet and the
    # live view can never disagree about what a part measures.
    from . import cadex_dimension

    measurements = cadex_dimension.records_from_display(display)
    ...
    try:
        for index, (spec, rect) in enumerate(zip(specs, rects)):
            ...
            dimensioned = (cadex_sheet.dimensions_active(spec)
                           and bool(measurements))
            # The wider fit buys the label band; a cell too narrow to
            # carry one gets the normal fit and the drop note below. A
            # dimensioned cell buys a narrower band of its own — the
            # extension lines and the numbers live outside the part.
            roomy = named and w >= cadex_sheet.CALLOUT_MIN_WIDTH
            fit_margin = MARGIN
            if roomy:
                fit_margin = cadex_sheet.CALLOUT_FIT_MARGIN
            elif dimensioned:
                fit_margin = cadex_sheet.DIMENSION_FIT_MARGIN
            ...
            if dimensioned:
                jobs = cadex_sheet.dimension_jobs(
                    measurements, fitted, w, h, hidden=spec["hide"])
                for job in jobs:
                    job = dict(job)
                    job["points"] = tuple(
                        (margin + x + px, margin + y + py)
                        for px, py in job["points"])
                    dimensions.append(job)
                if jobs:
                    notes.append("views[{:d}]: {:d} dimension(s) "
                                 "drawn".format(index, len(jobs)))
            elif (spec.get("dimensions") is True and not measurements):
                notes.append("views[{:d}] asks for dimensions, but the "
                             "script declares none; add part.measurement"
                             "(...) and rebuild".format(index))
    finally:
    ...
    pixels, width, height = cadex_sheet._dress_sheet(
        ...
        background=ground, callouts=callouts, dimensions=dimensions)
    ...
    return {
        ...
        # The margin band the dress pass added around the field: the rects
        # above are field-relative, so a hit test against the finished sheet
        # must offset by this (the draft editor's cell_rects does).
        "margin": margin,
        "size": [width, height],
        "version": version,
        "project": project,
