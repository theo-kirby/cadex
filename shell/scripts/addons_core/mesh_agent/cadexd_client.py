...
MODELING_OPS = frozenset(
    {"open_project", "write_script", "edit_script", "set_params", "rebuild",
     "put_asset", "link_part", "put_blueprint"}
)
...
def preflight(explicit="", bundle_roots=()):
    ...
    if not freecadcmd:
        return (False,
                ...
                "Mesh normally uses the engine bundled with it; to use a "
                "different build, set the engine path in the Mesh Agent "
                "add-on preferences or the MESH_FREECADCMD environment "
                "variable.")
