...
**F6 is the run's next design turn: the frozen Robin create prompt on fresh `ot7-robin-b`, dispatched after this reconcile pass and only when the runner's window probe reads under 45 %, on a product one change newer than F5's, whose build replies now carry an advisory `inventory` block beside `fit` (ADR-362, commit `fd3b3643`) [rec: sunny-chart-5873] [rec: lawful-grotto-1291].** F5 exhausted on 2026-09-16 with every count of its bar met except catalog hardware, after four turns in which the agent claimed all twelve purchased parts catalog while the published inventory listed both servos and both horns uncatalogued, because nothing in its fit summary named catalog identity [rec: sunny-chart-5873]. At the critic's direction the bridge now reads `inspect scope=inventory` under the same lock as the fit read and puts component, catalogued and uncatalogued counts, the catalog roll-up and every uncatalogued source name into the reply, the progress line, the `--json` envelope and the prose report, with no verdict and nothing refused, and the system prompt tells the agent that catalog identity is measured too: read the block before claiming catalog hardware, and if a purchased part is listed, place the untouched catalog body and put the cut in the printed part (detail on `chilly-union-8972`) [rec: lawful-grotto-1291]. Robin's frozen prompts are unchanged, so a catalog-count difference from F5 is a difference across that one product change, and REPORT.md's F10 section says so [rec: lawful-grotto-1291]. The dispatch is `run.py robin "$PROJECTS/ot7-robin-b" --model claude-fable-5 --turns 1`; the window read 41 % after F5's last turn and a create costs about half a window, so the create waits for the 18:50 UTC reset on 2026-09-16, and the create-plus-three-continuations schedule spans several five-hour windows; F7 on `ot7-plover-b` follows [rec: sunny-chart-5873]. The gate F4's first turn named below was met in code before F5 ran (the per-message cap, the runner's recorded `medium` effort, streams captured as they arrive): F5's four turns on `ot7-heron-c` completed on their own with their streams retained [rec: sunny-chart-5873].
...
Reconcile judgement: retain `open`; a void call is neither a pass nor an attempt, and no design turn has yet been dispatched for F6 [rec: keen-wing-6569] [rec: narrow-wave-7452] [rec: sunny-chart-5873].
...
- sunny-chart-5873 — F5 exhausted with catalog hardware its one failing count; F6's create on `ot7-robin-b` named next, after the 18:50 UTC reset and under 45 %
- lawful-grotto-1291 — ADR-362: F6 runs on a product whose build replies carry the advisory inventory block; frozen prompts unchanged; dispatch after the reconcile pass
