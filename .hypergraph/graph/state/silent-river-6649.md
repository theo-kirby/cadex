...
**The fresh biped now has a successful 240-iteration baseline with verified intermediate and final playable videos; review-driven redesign and retraining remain open.** The default product agent created Reed without old mechanism/checkpoint/history imports. Its first walk trained but failed declaration because the script lacked `policy_on` [rec: lucid-journey-6875]. After quota-refused agent repairs, actor recovery added a playback-only switch and separately retained the verified probe1 playback; this was not a successful product-agent repair or physical design change [rec: amber-gate-7498].

Probe3 trains the unchanged model/task on GPU, exits 0 and retains real assembled training inputs plus browser-verified active checkpoint and final-policy videos. For the same seed-0 eight-second task, checkpoint20 survives eight seconds with 45.94 mm forward displacement, while the final policy falls at 0.52 seconds after 210.54 mm. Rising reward did not improve survival; successful training does not establish walking. The experiment uses the retention helper and offboard trainer directly, not a completed `cadex walk` orchestration [rec: light-brook-2640].

Keep `open`: review-driven physical design change, retraining and common seed-set displacement/survival/fall comparison remain unmeasured; seeds 1–9 are not claimed. This baseline supports that comparison but does not complete the lifecycle [rec: light-brook-2640].
