...
"""Workbench/domain read-tool implementations outside ``CadexCore``."""
...
def partdesign_operation_for_feature(feature: Any) -> str | None:
    ...
    if (
        ...
        and str(getattr(feature, "CadexFeatureType", "")) == "thin_loft"
    ):
        return "thin_loft"
...
def assembly_solver_verification(result: dict[str, Any]) -> dict[str, Any]:
    """Solver-outcome verification shared by the joint and solve tools.

    Redundant constraints (code -2 or the redundancy diagnostic flags) are a
    warning, not a failure: FreeCAD solves and keeps redundantly-constrained
    assemblies, and a fixed joint on an already-positioned component is
    routinely redundant. Only conflicts, malformed constraints, and hard
    solver errors roll back the transaction.
    """
    diagnostics = result.get("solver_diagnostics") or {}
    code = int(result.get("solver_code", -1))
    failures: list[str] = []
    if code not in (0, -2):
        failures.append(f"solver returned {code} ({assembly_solver_verdict(code)})")
    if diagnostics.get("has_conflicts"):
        failures.append(
            f"conflicting joints: {diagnostics.get('conflicting_joints')}"
        )
    if diagnostics.get("has_malformed_constraints"):
        failures.append(f"malformed joints: {diagnostics.get('malformed_joints')}")
    diagnostics_available = diagnostics.get("available") is True
    checks = [
        {
            ...
            else "Native assembly solver diagnostics are unavailable.",
            "actual": diagnostics,
        },
        {
            "name": "solver_result",
            "ok": not failures,
            "message": "; ".join(failures) or None,
    ...
    if failures:
        verification["error"] = "Assembly solver failed: " + "; ".join(failures)
    elif not diagnostics_available:
        verification["error"] = (
            "Native assembly solver diagnostics are unavailable."
        )
    if (
        code == -2
        or diagnostics.get("has_redundancies")
        or diagnostics.get("has_partial_redundancies")
    ):
        verification["warnings"] = [
            "Solver reports redundant constraints; the assembly still solves. "
            "Simplify joints only if component placements are actually wrong."
        ]
    return verification
