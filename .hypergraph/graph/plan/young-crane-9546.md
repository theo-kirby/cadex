...
1. **Remove the stale onboarding version assertion (missions 1 and 2).** The Getting the tool block in .hypergraph/AGENTS.md says the project is on 0.0.7; config and the installed CLI both report 0.0.13. Confirm these read-only facts, then replace the duplicated version claim with a reference to .hypergraph/config.yml as the project-copy version source. Retain the distinction between upgrading the CLI and refreshing committed copies; update the verification date if needed. Record the narrow removal in the relevant existing ADR and a work record with real state impacts. No product ROADMAP checkbox is earned. Run hypergraph export/check and git diff --check; no product gate/build is claimed for documentation-only work. No installation, upgrade, network access, generated-skill rewrite or broader doc audit [rec: green-shade-8828].

2. **Repair only an introduced documentation verification failure (missions 1 and 2; conditional).** If unit 1 causes a graph or documentation check to fail, correct exactly that introduced failure and re-run the failed check plus export/check. A pre-existing mismatch or tool-version warning is evidence for separate planning, not authorization for upgrade or protocol changes. If unit 1 passes, this unit is non-dispatchable: close dispatch and return to separate planning without an adjacent audit or administrative repeat [rec: green-shade-8828].

Linked-part source independence is qualified: four staged CLI exits 0/0/0/0, exact BREP and accepted identity preserved with the source path absent, protected project docs, and 25 focused tests passed. No conditional repair is warranted; fold the actor handoff and do not repeat or expand into refresh/path-repair/catalog identity [rec: calm-sky-2656] [rec: lawful-union-8346]. Consent/cold recovery and seven-suffix parity are likewise complete, with no new runtime Save-As claim [rec: silver-rain-7333] [rec: empty-ledge-4581].

Signals: 197 iterations, 12.0h elapsed, 3.0h left, $85.47 API-equivalent; Claude five_hour 65%, seven_day 54%; Codex seven_day 49%. One concrete standing-maintenance direction selected from three. No missing lifecycle leg warrants another baseline; no Later promotion, charter retirement or reopening. Live repair remains controller-parked: no retry, availability probe, repeated hold record, provider switch, paid fallback, training, stage refresh, GUI or remote execution [rec: lucid-snow-0350] [rec: scarlet-journey-0203] [rec: sunny-walrus-5847] [rec: green-shade-8828].
