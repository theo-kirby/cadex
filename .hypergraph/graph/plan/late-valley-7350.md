...
1. **North star and parked rungs (mission 2).** The robot prompt, print-ready export, G-code, rollout video, fleet installation and gait-scale training remain long directions requiring human promotion. None is selected by this pass [rec: modest-summit-8554] [rec: frosty-wolf-4770].

2. **Standing lifecycle maintenance (missions 1/2/6).** History and context fixes, the live history iterate, the uninterrupted resumed walk, section ranking, standalone derived sections, model persistence and missed-moving-object reporting have all landed. **This run's frontier criteria are now all evidenced**: the walk completed every leg from nothing [rec: chilly-basin-7378], the second mechanism holds [rec: rare-cliff-9595], all four eyes have run inside a completed walk [rec: idle-pond-4961], and the three modes are exercised/scripted/documented as their own wording allows [rec: idle-falcon-3004] [rec: cool-eagle-6400]. Short now carries the last named opening (detach through the walk), the one new direction below, and a small budget defect. Preserve project history and local evidence; the human ticks the boxes [rec: humble-star-3313] [rec: sage-glacier-2165] [rec: rising-rain-0117] [rec: neat-grotto-9232] [rec: silent-anchor-7513] [rec: square-crest-8869].

3. **This machine's evidence.** The swing-arm prompt walk exited 0 in 17:43; the prismatic carriage prompt walk in 359.8 s; the fresh crank-slider walk in 1680.78 s through every leg and all four eyes [rec: chilly-basin-7378]. The model-free carriage width change 70→80 mm completed in 20.89 s at peak RSS 1,760,116 KiB, preserved 21 baseline files and automatically recorded reward 3.296298→2.760187 over equal 200-step rollouts. This is toy pipeline and **initial-pose** evidence, not motion safety, printable fit, general design ranking or learned gait — a distinction item 7 below now acts on. The former fixed-plane misses are historical: section derivation and coverage ranking have shipped, standalone derivation measures 6/10 objects cut on `ot4-swing2`, and the report join names missed moving objects without adding another cut or swept coverage [rec: idle-pond-4961] [rec: wandering-jasper-6102] [rec: rare-cliff-9595] [rec: mellow-quartz-8093] [rec: rising-rain-0117] [rec: neat-grotto-9232].

4. **Runtime reporting is delivered.** ADR-251 reports match/different/unavailable before the first leg, in JSON and stderr. The fresh walk's own comparison reported `match` across 56 files **after** a rebuild that the comparison itself justified — the checkout's `CadexDynamics.py` differed from the install by exactly the previous unit's commit [rec: chilly-basin-7378]. Neither matching bytes nor a successful review establishes binary or loaded-module provenance. Latest recorded CLI gate: 289 passed [rec: idle-pond-4961] [rec: solar-snow-8543] [rec: modest-prairie-3356] [rec: narrow-wing-0418] [rec: northern-quartz-8401].

5. **Mode and environment limits.** GUI attachment remains documented and unexercised; remote dispatch remains scripted with local CPU stand-in parity tests, without touching another machine. CLI `CADEX_MODEL` does not select the shell model. This machine has no usable shell bundle; old installed-bundle measurements remain historical evidence on their original machine, not current shell verification. OpenBLAS worker reservation and worker snapshot repairs are delivered, not targets [rec: early-quill-3654] [rec: western-reef-4119] [rec: lawful-wolf-9205] [rec: lawful-dune-3795] [rec: frosty-wolf-4770].

6. **Reduction and wider ambitions.** Inherited-tree reduction stays standing-only: take a cheap necessary removal during authorized work under its protocol, never a fresh campaign. Catalog, manufacturer STEP, compound mechanisms, outside-source work, studies, reports and mg-legs remain parked. Keep gates green, docs and manifest honest and the frontier short without inventing a bookkeeping actor iteration [rec: autumn-arrow-3125] [rec: modest-summit-8554] [rec: frosty-wolf-4770].

7. **Kernel clearance over the dynamics rollout — selected, in its smallest form, on the justification this item asked for (missions 2 and 6).** This item stood unselected because its promotion needed a separate justified bet. The bet exists now, and so does the measurement: the very run that closed the walk criterion reported `1 intersection: frame ∩ slider, 648.0 mm³` **at the initial solved pose**, then rolled 151 solved frames with 74.62° of crank rotation and 20.32 mm of coupler travel with clearance measured at none of them [rec: chilly-basin-7378]. The engine already measures a swept clearance — `_clearance_prepare` and `_clearance_at_frame`, re-placing each prepared shape from its component's live placement and reporting the worst approach over a whole travel (ADR-130, ADR-242) — but only inside `assembly.simulation`'s kinematic trace; `assembly.rollout`, which is the motion the walk reviews, has none, and `core.inspect` scope `clearance` reads exactly one pose. **What is selected is the reuse, not a new engine**: those functions against poses the rollout trace already carries, reporting the worst pair and its frame. It answers the refusal question the same way this run already settled it — **report, never refuse** — because `assembly.simulation` refuses on breach and inheriting that by accident would turn a review finding into a walk failure. Engine-zone work with the packaged gate behind it (ADR-023), no `mjx`/`jax` anywhere near it (ADR-084), and `OP_ARG_SPECS`/`docs/INTEGRATION.md`/the shell client moving together if a result shape changes. **The CLI-side bounding-box screen was measured and retired and is not resurrected** [rec: strong-falcon-1463] [rec: morning-summit-7848]. If the trace does not carry re-placeable poses, this becomes a measurement record saying so and is dropped rather than grown; a swept-volume kernel, a new trace format or a new dependency is a different direction and would need its own bet [rec: sleepy-hollow-9498] [rec: square-light-7067] [rec: chilly-basin-7378] [rec: square-crest-8869].

## Negative knowledge

- [scope: what item 7's promotion does and does not authorise | confidence: high | evidence: square-crest-8869] Selecting kernel clearance over the rollout authorises **one reuse of an existing per-frame measurement against poses an existing trace already carries**, reported and never refused. It does not authorise a swept-volume kernel, a new trace format, a new dependency, a second clearance surface beside `assembly.simulation`'s, a refusal semantic on the walk, an `mjx`/`jax` import, or the resurrection of the retired CLI-side bounding-box screen [rec: strong-falcon-1463]. It is not a claim about printability, fit or gait, and a clean moving-clearance block would prove no more about a mechanism than the clean static one did about `ot4-mix55`, which had a 648.0 mm³ intersection at the pose it was measured in [rec: chilly-basin-7378]. If the poses are not re-placeable from the trace, the honest output is a measurement record and the direction is dropped [rec: square-crest-8869].
...
- [scope: what is still selected on short | confidence: high | evidence: candid-fern-3092] The derived section offset has landed and leaves dispatch; entries above and below naming it as pending short work are historical. The lifecycle iterate is unattempted, and the cause measured this pass is ranking rather than capacity — `claude` on PATH, engine binaries present, five-hour usage 35%. No provider probe, retry or wait unit follows from that, and no long-rung direction is promoted by it [rec: copper-canyon-0231] [rec: terse-crane-6585].

- [scope: exhausted context direction and the reopened iterate | confidence: high | evidence: steady-chart-0544] Offline lifecycle context maintenance is complete; entries below selecting it, and those recording Claude capacity as blocked, are historical. A supplied five-hour usage signal reopens exactly one dispatched provider attempt and licenses no probe, retry or wait unit. The empty quill sections trace to a constant tuned to two other mechanisms; deriving it is bounded CLI work and settles nothing about motion clearance [rec: morning-arrow-5841] [rec: fresh-clover-8521] [rec: honest-banner-0821].
...
- strong-falcon-1463 — record the engine-side kernel swept check as the successor direction the CLI screen must justify first
...
- steady-chart-0544 — record context maintenance as spent and the fixed section constant as a measured limit
- candid-fern-3092 — record the derived section as delivered and keep the unattempted iterate as the only thing selected on short
...
- square-crest-8869 — promote item 7 (kernel clearance over assembly.rollout) from unselected to selected in its bounded reuse-only form, on the initial-pose measurement the fresh walk produced
