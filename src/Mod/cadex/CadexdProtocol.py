...
OP_ARG_SPECS: dict[str, tuple[dict[str, type], dict[str, type]]] = {
    ...
    # The deliverable: one STL per named part, at its own origin, into
    # `print/`. `printable` is the whole job — the output names the caller
    # means to print, named on the call (ADR-158). There is no op that
    # *stores* a mark and there is no state behind this one: which parts a
    # person means to print is a decision about a view of the model, so the
    # party drawing that view keeps it and says so here.
    #
    # `conflict` is absent on the first call **on purpose**: the refusal is
    # what carries the list of files already there, which is `link_part`'s
    # arrangement exactly — ask with nothing chosen, and the refusal is the
    # question. Then "overwrite" or "keep_both". `deflection` is the
    # tessellation chord height for a BREP output, in millimetres; left out,
    # the display default sizes it off the shape.
    "export_printable": (
        {"printable": list},
        {"conflict": str, "deflection": float},
    ),
        ...
        frozenset(
            {
                ...
                "cages",
                # ...and the printable roster (ADR-156, ADR-158): which of
                # the accepted revision's outputs could become an STL. Not
                # which are ticked — the ticks belong to whoever draws the
                # checkboxes, and this is the list they may draw.
                "printable",
            }
        ),
        frozenset(),
