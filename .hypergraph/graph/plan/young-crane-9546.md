...
1. **Wire delivered named-angle artifacts into walk review (missions 6 and 2; damp-moon-9297).** The prior rerun and renderer delivery landed; this is the remaining original short unit. Reuse the delivered renderer and existing review inspection session where possible, snapshotting valid display buffers before later requests invalidate them. Include availability, accepted revision/digest, front/top/right/iso names, approximation/limits and project-relative paths in review.json. Commit images and summary with project docs; retain inventory/clearance outcomes and revision fidelity. Update scaffold/project_docs.py, mode-artifact table and CLI/walk docs together. Real arm/carriage walks assert actual nonblank image contents and tracked reports; extend local/remote-flag CPU stand-in parity without SSH or GUI. Keep rendering errors visible, avoid treating retained old files as current success, and leave section absence explicit. Measure whole-walk overhead separately from acquisition and rendering. Run the full built-engine CLI suite with local toy CPU training bounded to 15 minutes and 3 GB per run, reporting skips. Engine/protocol/payload changes require their additional gates. This does not close the review criterion [rec: silver-key-8483] [rec: clear-ash-2884] [rec: gilded-basin-9946] [rec: nimble-beacon-7598].
2. **Rerun the documented local toy walk after integration (mission 2; damp-moon-9297).** Once rank 1 lands, the old short is exhausted; honor the empty-short rule with a separate evidence unit before promoting sections. Any failed leg from integration leads instead. Run the documented hinged-arm script and walk in a fresh project, built engine and training venv, one iteration/four environments/seed zero, within the same 15-minute/3-GB bounds. Record commands, exits, wall time, peak memory, policy verification, reward and project-relative artifacts. Inspect the four images, verify accepted revision and committed summary/review/docs, and retain the arm's below-clearance finding with zero unknown pairs. Compare whole-walk cost with 15.2430 s and 14.5173 s as individual samples, not a regression benchmark; separate rendering/acquisition timings. Run the full CLI gate and report skips. Name the remaining section leg; do not bundle section implementation or claim complete review [rec: brave-delta-4193] [rec: clear-ash-2884] [rec: silver-key-8483] [rec: nimble-beacon-7598].

No medium promotion until these short units land or block; critic must-fixes lead. Latest loop signals are 13 iterations and 12.1 hours remaining after 2.9 elapsed; two bounded units are not a promise to consume the budget. No new direction or charter-gap retirement is proposed. Later criteria stay parked and inherited reduction remains standing work [rec: nimble-beacon-7598].
