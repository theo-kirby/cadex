...
1. **One selected finite direction: explicit toy CPU verification (missions 1/2/6).** The final recovery suite passes, but its preceding focused run selected CUDA and failed in cuSolver until CPU was pinned. The remaining setup prose and unpinned real train tests make the intended backend depend on the discovered venv. Short first rehearses and corrects the CPU instructions, then consolidates test selection and actual-device assertions. This is maintenance of the headless-walk criterion, not a CUDA fix or a new product option [rec: placid-ember-6741] [rec: dry-grove-2638].
2. **Recovery is delivered.** Public retry preserved 46 old files and prior progress; durable coverage now verifies the retained sweep, new policy, complete review and comparison against last-success rows. Final CLI gate: 223 passed, no skips. Remove both spent recovery dispatches; no further recovery leg or failure matrix is selected [rec: dusty-vale-2809] [rec: placid-ember-6741].
3. **Three candidates and budget.** Select CPU consistency from the newly observed failure; reject repeated recovery and reject Later catalog/fleet expansion. Signals: 13 iterations, 4.5h elapsed, 43.5h left; seven iterations without frontier movement, Claude five_hour 100%, Codex seven_day 71%. Two model-free units fit; quota and remaining time create no work or clock gate. An empty actor bookkeeping dispatch is not a fallback [rec: careful-union-7585] [rec: dry-grove-2638].
4. **Evidence and scope limits remain.** Both mechanisms prove the local toy pipeline; neither the width iterate nor changed-weight recovery proves policy improvement. GUI remains documented/unexercised, remote parity a local stand-in, runtime comparison Python bytes only. Reduction stays standing-only; catalog breadth, gait scale, fleet, reports, variant studies and outside-source mechanisms stay parked. No charter gap is retired or checkbox edited [rec: mellow-quartz-8093] [rec: early-quill-3654] [rec: narrow-wing-0418] [rec: placid-ember-6741] [rec: dry-grove-2638].
