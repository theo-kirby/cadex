...
**Robin has been through the full recorded lifecycle: designed from a prompt, accepted with fits, reopened, trained once (bounded), evaluated over ten seeds, with checkpoint and final videos on the persistent dashboard.** D7 is held `open` only because no record declared it ticked and the human owns the checkbox; the records below name the evidence for every clause of the criterion, and one design decision is left open for the next turn on Robin [rec: candid-delta-9314].
...
**Dashboard.** Verified at every experiment start and end: fresh headless browsers at 1400×900 and touch 400×850, no horizontal overflow, real solids by default and nine proxy outlines under the labelled toggle. It now serves `ot6-robin` with five runs and a fresh visit selects `robin2-final` (24 components, 114 344 triangles) [rec: candid-delta-9314].
