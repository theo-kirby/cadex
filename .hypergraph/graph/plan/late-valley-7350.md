...
2. **Standing lifecycle maintenance with one finite recovery direction (missions 1/2/6).** The failed-retraining regression preserves policies and history while retaining the changed sweep with policy_on=0; its revisit stops at export. Short now rehearses recovery and replaces terminal coverage with a complete retry, consolidating existing tests and guidance. This is unproved continuation, not a production bug claim. No failure matrix, rollback layer or automatic repeat follows [rec: dawn-sky-3891] [rec: northern-sage-7087]. The prior empty actor dispatch was rejected and is not progress [rec: careful-union-7585].
