...
2. **Standing lifecycle maintenance (missions 1/2/6).** CPU setup and shared regression selection are delivered: the public rehearsal passed in 14.6 s / 1.54 GB and all 223 CLI tests passed without an outer CPU override. Recovery remains covered. The only selected continuation retains existing named inventory evidence in historical walk reviews, first measuring the latest-document boundary, then replacing the lossy projection. Git history is retained today; no data-loss claim, catalog inference, archival subsystem or automatic adjacent campaign follows. Empty bookkeeping actor dispatch remains invalid [rec: floral-arrow-7365] [rec: keen-field-4379] [rec: careful-union-7585] [rec: scarlet-ocean-2920].
