...
def script_printable(scene):
    ...
    """
    block = _script_block(scene, "printable")
    if block is None:
        return None
    return [dict(entry) for entry in list(block.get("outputs") or [])
            if isinstance(entry, dict)]
...
def api_overview(payload):
    ...
    if isinstance(library, dict):
        # The parts library (ADR-181) is not a capability domain but the
        # model shops it the same way: list what exists here, serve the
        # full catalog through describe_cad_api domain="lib".
        part_numbers = {}
        for family, block in sorted(dict(library.get("catalog") or {}).items()):
            if not isinstance(block, dict):
                continue
            ids = (block.get("sizes") or block.get("codes")
                   or block.get("skus") or [])
            part_numbers[family] = list(ids)
        domains["lib"] = {
            "api_global": library.get("api_global", "lib"),
            "functions": sorted(
                str(item.get("name") or "")
                for item in list(library.get("exports") or [])
                if isinstance(item, dict) and item.get("name")),
    ...
    overview["how_to_read_this"] = (
        ...
        "you are unsure of. 'lib' is the parts library: catalogued "
        "fasteners, bearings and servos as ready part values -- prefer it "
        "to modelling standard hardware by hand.")
    return overview
...
def api_domain(payload, domain):
    ...
    block = domains.get(wanted)
    if block is None and wanted in ("lib", "library"):
        block = payload.get("library")
    if block is None:
        return False, ("Unknown domain {!r}. The engine serves: {:s}.".format(
            domain, ", ".join(sorted(list(domains) + ["lib"]))))
    return True, block
...
def compact_domain(block):
    ...
    if notes:
        compact["notes"] = notes
    # The library block carries its browsable catalog, and the catalog is
    # the point of asking for it: part numbers and deciding specs, no prose.
    catalog = block.get("catalog")
    if isinstance(catalog, dict):
        compact["catalog"] = catalog
