...
**Current Wren saved-project reopen and persistent dashboard restart pass on retained real artifacts.** Two fresh engine processes preserve accepted revision `de9692bd4ee5…`, digest `cd99e3be1555…`, contract, attempt and accepted bytes. Of 2,004 inventoried files, only normal candidate rotation (42 added/42 pruned) and manifest timestamp change; dashboard restart changes no project file. All 15 run views plus ACCEPTED NOW retain identities, specs/decisions and three histories; current and historical videos play/download with recorded hashes. The open playing page recovers without navigation, and fresh visits select `wren66-final` with 90 mm feet and 240 points per history. No real trainer was active: this is completed-artifact evidence, supplemented by synthetic telemetry concurrency, not a new GPU-training restart claim. Same-machine private-address and headless persisted-save scope only [rec: sunny-canyon-4438].
