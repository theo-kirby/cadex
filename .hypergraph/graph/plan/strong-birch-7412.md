...
1. **Clearance and intersection check naming pairs (mission 6; damp-moon-9297).** After the assembly inventory (now short rank 3) lands with its record [rec: rustic-loom-0992]. The op takes an assembly output and returns the pairs of placed solids whose common volume exceeds a tolerance or whose minimum distance falls below a declared clearance, named by component label and catalog id; INTEGRATION.md row, `OP_ARG_SPECS`, shell client update and response golden in the same commit (methodology rule 6); `cadex` writes the report into the project directory. The only check today is the declared swept clearance validated at rebuild (ADR-130). Real-kernel test with one interfering and one clear pair; packaged gate after a finished stage. [rec: flat-river-8853] [rec: glad-snow-3838]
2. **Headless review, render and section (mission 6; damp-moon-9297).** Named-angle render and named-plane section views, until the shell's blueprint renderer is shown to run under headless Blender inside the charter's no-GUI constraint. Each is one CLI call with project-local outputs and its own unit. As each lands, wire it into the walk's review step in a separate unit that carries `project_docs.py` and its test together. [rec: flat-river-8853] [rec: wild-marsh-9611]
3. **The walk's third mode: the remote training handoff (mission 2; witty-spark-2613).** Scripted and documented, never executed while the local-only constraint holds. The GUI-attached mode is documented, not exercised. Once short's ranks 1 and 2 have run clean, the one concrete reason the reconcile gave for holding this criterion open — the walk did not complete from `--prompt` — is gone [rec: rustic-loom-0992]; the unit then is the same shape as the second-mechanism unit was: establish what the criterion's exact wording still lacks (the loop's steps and artifacts the same in all three modes, shown not asserted), close only that, and say in the record whether it can be ticked. A warm start under `--remote` is refused before any leg [rec: green-delta-7130]; the unit that documents the handoff says so rather than hiding it.
4. **A named design-leg cause, if the re-run names one (mission 2; crisp-reef-5607).** Conditional: exists only if short rank 1 or rank 2 fails at the design leg with the reason the envelope now carries [rec: ancient-wind-0117]. The unit fixes the named cause, in the authoring contract or the CLI, with a deterministic test that reproduces it without a prompt re-run, the way the literals fix was pinned [rec: cool-fountain-2483]. A bounded retry of the design leg (one, inside `cadex walk`, with the reason written into the walk's envelope and `PROGRESS.md`) is authorized only when the named cause is a transient the record demonstrates, and never as a substitute for naming it. [rec: rustic-loom-0992]
