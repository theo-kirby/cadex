...
| **`assembly_mjcf_xml`** | a self-contained MJCF model file (`MJC` only) | ADR-081 |
| **`assembly_training_task_json`** | a `cadex-training-task-v1` bundle (`MJC` only) | ADR-083 |
| **`assembly_policy_receipt_json`** | the engine's receipt for a verified policy (`MJC` only) | ADR-084 |
