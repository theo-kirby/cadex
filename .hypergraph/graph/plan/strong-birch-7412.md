...
1. **Catalog-part integrity in review (missions 2 and 6).** Bounded discovery is complete: three real-kernel scratch acceptances distinguish direct, transformed-fuse, cut-tool and discarded paths while preserving placed counts. The pan-tilt retains fourteen outputs and zero catalog stamps; equal definitions lose occurrence provenance, and cold reports cannot supply those identities. Fresh compliance is now measured once: on the two-servo leg the agent published one servo, one bolt and one nut body and placed each twice as separate components, six catalogued instances, 39 catalog pairs in the bound checker, and no catalog body fed a printed-solid boolean. That is one run under the shipped guidance, not a guarantee; the pan-tilt rerun had fused. Short's iterate re-measures compliance under a modification prompt. No diagnostic implementation, persistent identity contract or repeat experiment is selected; purchases and surviving material remain unproved by dependency reachability. No catalog breadth promotion [rec: early-gate-3510] [rec: misty-tide-6394] [rec: clear-crest-9910] [rec: proud-beacon-8002].
2. **Preserve completed lifecycle and review criteria (missions 1, 2 and 6).** Five clean prompt walks (pendulum rig, linear carriage, pan-tilt twice, two-servo leg) and cold persistence are the successful baseline; the leg completed all legs in 565.7 s at 1.27 GB peak on the ADR-244 payload with no worker error, and its review named real findings. Short now iterates on those findings; the walk is not rerun for its own sake. Keep source, fresh payload and ordinary installed bundle evidence distinct: the installed application predates ADR-243/244, GUI-attached stays documented-only and remote stays scripted-only [rec: first-branch-9614] [rec: quiet-vine-3426] [rec: morning-summit-7848] [rec: proud-beacon-8002] [rec: weathered-hill-5955].
3. **Worker identity repair is qualified (missions 1 and 2).** ADR-244 detaches worker snapshots, hashes the bytes published and validates cached identity including publication fallback. Full engine 2085 passed/52 skipped, built-engine CLI 195 passed/no skips, fresh packaged lifecycle 15 passed; supported build/install and staging completed, and a real worker acceptance/rebuild retained its digest. Twenty warm lookups had median 2.493 ms, maximum 6.130 ms, local observations only. The leg walk then ran four legs against that payload cleanly. General concurrent-writer safety, whole-install snapshot consistency and historical writer identity are unproved; no cache redesign or further race campaign is selected. The fresh stage is locally linked, not portable-release evidence [rec: nimble-basin-8423] [rec: lawful-dune-3795] [rec: proud-beacon-8002].
4. **The clearance direction remains closed (missions 1, 2 and 6).** ADR-241 and ADR-242 have measured initial and swept frame corrections; the installed bundle's catalog assembly passed twenty bound comparisons with a failing negative control, and the leg passed ninety. The walk reviews initial pose; swept rollout review integration remains unselected. The leg's shin/bolt and servo/link findings are handled by short's iterate as a project change, not as a review-tool change; existing pan-tilt contacts remain project design findings [rec: southern-otter-5999] [rec: morning-summit-7848] [rec: proud-beacon-8002].

No charter-gap retirement or Later promotion. GUI-attached lifecycle remains documented-only and remote remains scripted-only; toy policy witness success establishes no gait performance. The completed rehearsal grants no runtime implementation authority beyond the conditional domain-doc correction in short [rec: modest-summit-8554] [rec: weathered-hill-5955].
