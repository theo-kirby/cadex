...
1. **Remove the machine path from the walk's recorded output label (missions 1 and 2).** Fix only the walk branch of the progress subject formatter, shared by PROGRESS.md and the project commit subject. Prefer a project-relative output label for an output inside the project, with a basename fallback outside it; never record an absolute home path. Add a regression exercising the recorded row and commit subject with an absolute --out and preserving useful run identity. Update docs/CLI.md and the project-doc scaffold in the same commit, add the ADR line and ROADMAP checkbox, run `pixi run python -m pytest cli/tests` and report engine skips honestly. No historical project/repository commit rewrite, general path scrub or unrelated export/link correction. This is the concrete defect the durable walk named; the next unit verifies its fix live [rec: fond-star-1809] [rec: forest-shade-2752] [rec: wild-marsh-9611].

2. **Run the resumed repair turn on the durable leg (missions 2 and 6).** After unit 1 lands or is explicitly blocked, run the documented `cadex walk --resume` on the existing durable project basename `nt3-leg` under the operator's `cadex-projects` directory, outside this repository. First read its current revision and baseline documents; do not recreate it or substitute the reaped temporary leg. Use a new --out under runs/, the qualified payload `build/engine/cadex-engine-0.0.0-macos-arm64` (CadexScriptedRuntime.py SHA256 602164e85c399ad203517eb269ec81bca549dc07b72d303659c1cffffe1dc6df), the training venv, JAX_PLATFORMS=cpu, --iterations 1 --envs 4 --seed 0 --timeout 600, and the existing 2.9 GiB / 850 s process-tree guard. One prompt asks the agent to read its own clearance, inventory, actuator and sensor notes, choose one justified geometry correction from that review, explain the choice in DECISIONS.md, preserve separately placed catalog hardware and preserve task/reward, observation/action definitions and rollout horizon for comparison. Name the documents, not the measured answers; do not require elimination of intentional face contacts or claim the boss is print-fit qualified. No operator script edits. Run design → training → declaration → verification/rollout → review through the same entry point. Check whether the actual stored conversation resumed: the product can automatically fall back to fresh with a report note; record that as a resume finding, never as proof of same-session repair, and do not launch another design attempt. Report each leg clean, guess/person, or unreached with exact errors. Compare total_reward against 0.8530453495479725 only if objective and horizon truly stayed equal; otherwise explain non-comparability. Compare clearance against 45 pairs / 9 offending zero-volume contacts / 0 unknown; inspect four views and the XZ section, tally six catalog instances or an explained change, and run the bound-agreement checker with its 1e-3 mm tolerance. Verify the new PROGRESS row and project commit subject contain no absolute output path, notes survive, and project status is clean; no checkpoints, rollout or machine paths enter this repository. A quota refusal or guard cutoff is one recorded finding and returns scheduling to the controller, with no retry, wait, provider switch or paid fallback. Any fused/dropped catalog body or inability to choose a justified correction is evidence for the next planner, not permission for a source repair in this unit [rec: fond-star-1809] [rec: lucid-pebble-4000] [rec: forest-shade-2752] [rec: lawful-dune-3795].

Signals: 185 iterations, 10.7h elapsed, 4.3h left, $82.49 API-equivalent; Claude five_hour 74%, seven_day 54%, overage-included 99%; Codex seven_day 32%. Two bounded units, no medium or Later promotion, no charter-gap retirement. This continues the selected review-driven iterate; no new direction. Availability is not inferred from usage percentages. No GUI, remote execution, bundle refresh, repeat worker qualification, dependency discovery or catalog diagnostic [rec: forest-shade-2752].
