...
1. **Preserve completed lifecycle and review criteria (missions 1, 2 and 6).** Three fresh mechanisms — the hinged arm, the carriage and the pan-tilt head on two catalog MG90S — and cold persistence remain working; the ordinary installed-engine route passes the complete arm walk and same-root packaged/CLI gates after supported refresh, and the engine-selection guidance states only the verified schema check. The pan-tilt walk was the first whose inventory carried catalog ids and the first to exercise clearance on `lib.*`-placed parts, which is how the frame defect surfaced; pair clearance now composes the component and shape placements. No packaging repair, training replay or same-mechanism repeat beyond the selected rerun is selected [rec: first-branch-9614] [rec: quiet-vine-3426] [rec: fair-cedar-7455] [rec: strong-raven-3067] [rec: empty-banner-7438] [rec: light-timber-5868].
2. **Finish the clearance thread, then the direction ends (missions 1, 2 and 6).** Short holds the pan-tilt rerun against a refreshed bundle carrying ADR-241 and the bounded swept-clearance frame fix. When both land, the review surface no longer publishes a number that is false for catalog parts at either the initial pose or along the trace, and the installed application agrees with the dev tree; the standing rule for the next empty short rung is again the charter ladder's walk. The three real contacts the corrected clearance revealed on the pan-tilt project (pan servo in the yoke, tilt servo in the yoke arm and head plate) are that project's design business and are not selected: the iterate criterion is ticked and a design turn there closes no criterion [rec: simple-raven-2485] [rec: light-timber-5868].

No new charter gaps, feature checkboxes or Later promotions are selected. The payload-identity guidance direction is exhausted; no broader documentation audit follows. The refreshed application remains a local unrelocated deployment; GUI-attached lifecycle and remote execution remain unexercised. Policy witness success is toy execution evidence, pair clearance is initial-pose and sections are tessellated [rec: strong-raven-3067] [rec: modest-summit-8554] [rec: keen-pebble-3574].
