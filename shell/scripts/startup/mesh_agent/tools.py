...
TOOL_DEFS = [
            ...
            "properties": {
                ...
                "target": {"type": "string",
                           "description": "Object name for scope=object; "
                                          ...
                                          "scope=history; sheet name or "
                                          "ordinal for scope=blueprint."},
                "path": {"type": "string",
    ...
    },
    {
        "name": "blueprint_view",
        "description": (
            "Draw the model as clean outlines on a blueprint-blue, "
            "cutting-mat-green, grey or drawing-paper-white (theme "
            "'technical': black lines) background, live in the viewport, "
            ...
            "properties": {
                ...
                "theme": {
                    ...
                    "enum": ["blueprint", "cutting_mat", "grey",
                             "technical"],
                    "description": ("The background the lines are drawn on "
                                    "(default blueprint; 'technical' is "
                                    "black-on-white)."),
                },
                "grid": {
                    "type": "boolean",
    ...
    },
    {
        "name": "make_blueprint",
        "description": (
            "Draw a blueprint sheet of the model as the live DRAFT on the "
            "user's screen -- white lines on a blueprint-blue, "
            "cutting-mat-green or grey ground, or a true "
            "TECHNICAL DRAWING with theme 'technical' (black lines on "
            "drawing-paper white), dressed as "
            "a drawing sheet (zone grid, project name, version block). "
            "NOTHING IS STORED by this call: the draft renders live in "
            "the user's Blueprint Editor window, re-renders itself when "
            "the model rebuilds, and "
            "you iterate on it with the user -- render again with "
            "different cells until they are happy, then save_blueprint "
            "stores the version they approved. The user can CLICK a view "
            "cell of the draft to tag it: the tag arrives in their next "
            "message as @cell-N with that cell's spec, and it is ground "
            "truth for which section they mean. "
            "Measurements the script declares with "
            "part.measurement (lengths, diameters, radii, angles) are "
            "drawn as proper dimensions -- extension lines, a broken "
            "dimension line, the number in the gap -- on every "
            "orthographic cell by default (dimensions true/false "
            "overrides that per cell), which is how a sheet becomes a "
            "functional technical drawing: declare the measurements, then "
            "render with theme 'technical'. You CURATE it for what was "
            "built, cell by "
            ...
            "NAME the sheet and it is revisable once saved: name is its "
            "identity, so saving again under the same name stores the "
            "next version, and based_on='<that name>' loads a STORED "
            "sheet's recipe into the draft -- pass only what changes (a "
            "different theme, one edited views list) and everything else "
            "is inherited. inspect_model scope=blueprint lists the stored "
            "names, versions and recipes, so a sheet from last week can "
            "be revised in this session. "
            ...
            "in the reply too, so look at it and iterate; drafts are "
            "free -- nothing lands in the project until save_blueprint. "
            "Unavailable when Blender runs headless."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
                "name": {
                    ...
                    "description": ("What this drawing is called, e.g. "
                                    "'gearbox overview'. It is the "
                                    "sheet's identity once saved: saving "
                                    "again under the same name stores the "
                                    "next version of it, and based_on "
                                    "takes it."),
                },
                "based_on": {
                    "type": "string",
                ...
                "theme": {
                    ...
                    "description": ("The ground the lines are drawn on "
                                    "(default blueprint; 'technical' is "
                                    "black lines on drawing-paper white -- "
                                    "the print-style technical drawing)."),
                },
                "max_size": {
                    "type": "integer",
                ...
                "views": {
                    ...
                    "items": {
                        ...
                        "properties": {
                            ...
                            "dimensions": {
                                "type": "boolean",
                                "description": ("Draw the script's "
                                                "declared part.measurement "
                                                "dimensions in this cell "
                                                "(default: on for "
                                                "orthographic model "
                                                "cells)."),
                            },
    ...
    },
    {
        "name": "save_blueprint",
        "description": (
            "Store the blueprint draft currently on the user's screen in "
            "the project, as a drawing attached to the accepted revision. "
            "Call it when the user says the draft is right (or asks to "
            "save it) -- make_blueprint renders and iterates, this is the "
            "decision. name is the drawing's identity: save again under "
            "the same name after further drafting and the store keeps the "
            "next version; omit it to keep the name the draft was "
            "rendered under. The stored sheets are listed by "
            "inspect_model scope=blueprint, and make_blueprint "
            "based_on='<name>' loads one back into the draft. Refuses "
            "when nothing has been drafted yet. The user can also press "
            "Save in the Blueprint Editor's header; Export there writes "
            "the PNG to a path of their choosing without touching the "
            "project."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
                "name": {
                    "type": "string",
                    "description": ("The drawing's identity in the store "
                                    "(default: the name the draft was "
                                    "rendered under; empty stores it "
                                    "unnamed and unrevisable)."),
                },
            },
        },
    },
...
def _tool_make_blueprint(tool_input):
    """Render the composed sheet as the live DRAFT (ADR-178).

    Nothing is stored here any more: the sheet lands on the user's screen
    through ``cadex_drawings.set_draft`` (and in this reply as an image),
    and the project store is written only when ``save_blueprint`` — or the
    panel's Save button — says the iteration is over. That is what emptied
    the store of the near-duplicates every ADR-150-era iteration left.

    Still in ``_ENGINE_TOOLS`` for ``based_on`` (ADR-157), which reads a
    stored sheet's recipe back out of the project store and drafts it
    again with this call's arguments on top; still NOT in
    ``MUTATING_TOOLS`` — a draft is a view of the model, not an edit of
    it, and must not enter the undo stack.
    """
    ...
    cadex_drawings.set_draft(scene, sheet, label=name)

    cells = ", ".join(
        "@cell-{:d} {:s}".format(position + 1,
                                 str(view.get("label") or view.get("view")
                                     or "view"))
        for position, view in enumerate(sheet.get("views") or ()))
    where = ("in the user's Blueprint Editor"
             if cadex_drawings.editor_open()
             else "ready for the user's Blueprint Editor (they have none "
                  "open; suggest opening one from the editor-type menu)")
    text = ("Draft {:s} -- NOT stored. {:s}, {:s} layout "
            "-- {:s}. Cells: {:s}.".format(
                where, theme, str(sheet.get("layout") or ""),
                str(sheet.get("legend") or ""), cells or "none"))
    if based_on:
        text += " Drafted from the stored sheet {!r}.".format(based_on)
    if sheet.get("note"):
        text += " Note: {:s}.".format(str(sheet["note"]))
    text += (" Iterate here -- render again with what changes -- and when "
             "the user is happy, save_blueprint stores it{:s}. The user "
             "can click a cell to tag it for you, and press Save or "
             "Export in the editor's header.".format(
                 " as {!r}".format(name) if name else ""))
    return [
...
def _tool_save_blueprint(tool_input):
    """Store the live draft through the engine (ADR-178).

    The one write path is ``cadex_drawings.save_draft`` — the panel's Save
    button calls the same function, so the tool and the button cannot
    drift. In ``_ENGINE_TOOLS`` (it writes the project store through
    ``put_blueprint``), NOT in ``MUTATING_TOOLS`` (the ``import_geometry``
    precedent: a store write is not a scene edit).
    """
    ...
    payload, error = cadex_drawings.save_draft(
        scene, name=str(name).strip() if name is not None else None)
    if error:
        return _text(_truncate(error)), True

    saved = (cadex_drawings.draft() or {}).get("saved") or {}
    root = cadex_backend.project_root(scene)
    stored_path = os.path.join(root, "blueprints", str(payload.get("name")))
    text = ("Blueprint stored at {:s} ({:d} bytes), attached to revision "
            "{:s}.".format(stored_path, int(payload.get("bytes") or 0),
                           str(payload.get("revision") or "?")[:12]))
    if saved.get("name"):
        text += (" Named {!r}, version {:d} -- keep drafting and save "
                 "again for the next version, or revise later with "
                 "based_on={!r}.".format(saved["name"],
                                         int(saved.get("version") or 1),
                                         saved["name"]))
    else:
        text += (" Stored unnamed -- give it a name next time to make it "
                 "revisable.")
    text += " The draft stays on screen and keeps tracking the model."
...
_HANDLERS = {
    ...
    "save_blueprint": _tool_save_blueprint,
}
