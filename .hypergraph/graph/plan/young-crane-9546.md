...
1. **Qualify one fifth servo (mission 4; ancient-tide-5930, brave-stone-9609).** Both visibility units landed and STATE is reconciled through civic-moss-7263. Record the existing four servo identities, then inspect at most two new manufacturer candidates for one distinct positional hobby-servo SKU compatible with the existing ServoPart recipe. Pin exact variant, voltage-qualified stall torque, mounting centres/holes, output-axis and envelope datums, document identities/URLs/hashes and explicit cosmetic approximations. No guessed interface, continuous-torque inference, public SKU or runtime edit. Distinguish remaining servo/actuator counts without repeating the completed L3 audit or treating aliases as breadth. Record the acceptance contract, recipe boundary and executable independent-worker/publication checks in provenance/decision docs and one work record. Run the existing packaged lifecycle/library baseline and label its source/stage identity and limits honestly. At most two candidates; if neither qualifies, record exact missing evidence and replan instead of running later ranks. No stopped motor/solenoid search or horn-STEP scope. [rec: twilight-lake-8164] [rec: sunny-canyon-1138] [rec: civic-moss-7263] [rec: rough-gate-7949]
2. **Prove the qualified servo interfaces independently (mission 4; ancient-tide-5930).** Conditional on rank 1 qualifying one exact SKU and reuse of the current recipe. Use real OCCT with source-derived canonical and rotated/translated geometry, independent mounting/output-axis surface and material/void probes; bounding boxes and copied implementation arithmetic alone are insufficient. Record actual measurements, compatibility, approximation/refusal limits and regression inputs. No public SKU delivery in this proof unit; temporary standalone probes may establish candidate evidence, and any committed tests stay green on the current product. Run applicable gates for touched zones (full engine suite for engine Python), at most one full build. If the recipe needs a new abstraction or proof fails, record the blocker and replan; no broad servo refactor. One coherent commit and work record. [rec: rough-gate-7949]
3. **Deliver only that proven fifth servo through the existing library (mission 4; ancient-tide-5930, brave-stone-9609).** Conditional on both qualification and independent proof records. Add the one SKU to existing discovery/ServoPart behavior, with meaningful source-derived dimension, torque-unit, actual-worker canonical/placed and packaged publication tests. Run the full engine suite, at most one build-engine, then finish stage-engine before packaged lifecycle/library gates. Report source/payload identity, all failures/skips and local relocation limitations; a development stage is not a portable release. Update PROVENANCE, relevant docs/dates, ADR and ROADMAP; one commit plus one record with real state targets. This can satisfy the five-servo subset only, not ten actuators, full breadth/L3, manufacturer horns, physical fit or unattended robot behavior. No GUI, remote execution, new API/family or copied GPL code. [rec: rough-gate-7949]
