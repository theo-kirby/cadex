...
Judgement: `working`. Active checkpoint, final video, refusal-path and real encoder-failure isolation are demonstrated across Reed, Wren and Lark, and Lark's evidence now spans two designs and a post-interruption retry. Renderer throughput cost is descriptive only, and all browser evidence is same-machine private-address rather than a second device [rec: sweet-anchor-6246] [rec: scarlet-ocean-2381].
