...
1. **Run the documented walk on a two-servo leg (missions 2 and 6, the ladder rule).** One `cadex walk --engine <bundle>` against the installed bundle refreshed in morning-summit-7848 (it carries ADR-241 and ADR-242; no rebuild, no restage), fresh scratch project outside the checkout, environment overrides unset, from a prompt for a single robot leg: hip and knee on two MG90S, a foot pad, M3 hardware, position servos at MG90S torque and speed limits, a joint-angle sensor per axis, a task holding a named crouch angle pair with a small effort cost. `--iterations 1 --envs 4 --seed 0 --timeout 600`, the training venv, `JAX_PLATFORMS=cpu`, under the process-tree monitor at 2.9 GB and 850 s. Record per leg: clean, needed a guess, or needed a person, verbatim error where one occurs; timing and RSS are observations. Record explicitly whether the inventory carries catalog ids or the design fused the servos; if placed, run the bound-agreement checker over the catalog rows. Inspect the four views and the section. A source change needed to get through a leg is the finding, recorded and not fixed in this unit. No shell gate: `shell/` is untouched. Declare impacts on crisp-reef-5607 and swift-dusk-2951 for evidence gained, and on damp-moon-9297 only if the review surface is found wanting [rec: forest-hollow-9339] [rec: morning-summit-7848].
2. **The inventory names catalog parts the design fused away (missions 6 and 4).** `inspect scope="inventory"` gains one list: catalog-stamped published outputs that no `component_link` row places, each with name, family and part number; `cadex inventory` renders it under its own heading in `docs/inventory.md`; the walk's `review.json` inventory block and the `PROGRESS.md` counts row carry the count. Real-kernel test in the inventory suite's shape: one `lib.servo` placed as a component and a second fused into a plate; the inventory names the fused one and counts one catalogued. First establish, on the pan-tilt rerun project if its scratch directory survives or on that fixture, whether a fused `lib.*` result is a published output at all; if it is not, record the finding, stop, and name the part program's dependency graph as the separate unit it would take under a later bet. Engine and CLI suites; ADR-027 goldens if the inspect response is pinned; ADR-243 line; the `cadex inventory` row in docs/CLI.md and the `inspect` sentence in docs/INTEGRATION.md in the same commit. No `OP_ARG_SPECS` change, no `shell/` diff, no restage; if it ships, the installed bundle is stale by Python files and stays so in this unit [rec: forest-hollow-9339] [rec: morning-summit-7848] [rec: fair-rose-5950].

Both previously selected units landed and the clearance thread is closed: the supported refresh put both frame fixes into the installed bundle with the worker byte-identical across source, stage and bundle, the packaged gate passed 15 against it, and a four-leg pan-tilt walk exited 0 in 406.33 s at 1.750 GB with no leg needing a person or a guess; the swept fix landed measured before and after with a regression that fails on the pre-fix source, engine 2077 passed / 52 skipped and CLI 195 without skips; the bundle's clearance on the catalog-placed pan-tilt assembly passed 20 bound comparisons with a negative control that fails 10. What the rerun added is that the design turn fused both catalog servos and the review said nothing about it, which is rank 2. Signals: 35 iterations, 6.4h elapsed, 8.6h left, the five-hour subscription window at 83%; two units fit and a third does not. GUI stays unlaunched, remote stays scripted-only, Later criteria parked [rec: morning-summit-7848] [rec: southern-otter-5999] [rec: forest-hollow-9339].
