...
1. **Qualify linked-part source independence once (missions 1 and 2).** Inspect ADR-138, docs/CLI.md's link/import contract, cli/tests/test_linked_part.py, engine linked-part suites and their recorded evidence first. If existing evidence proves consumer cold restore with the source path absent, record it and stop without a duplicate experiment. Otherwise use the existing qualified stage and two fresh external disposable projects initialized by ordinary CLI: accept a minimal solid in A, link/import it into B, close all clients, rename only disposable A so the recorded source path is absent, then cold-reopen B in a new CLI process. Use a documented inspection/export operation that actually restores/builds the imported solid; printing source alone does not prove geometry. Derive commands and expected semantics from code/docs before execution. Record exit/error codes, accepted revision/digest, linked asset hash and solid facts before/after; preserve B's docs and restore A's temporary name afterward. No manual metadata edits, durable-project mutation, refresh survey, training, walk, provider, stage refresh, GUI or remote execution. This is existing snapshot-contract qualification, not catalog identity, dependency discovery or a proposed path-repair service [rec: ancient-current-9419] [rec: sleepy-meadow-2615].

2. **Correct only a demonstrated source-independence mismatch (missions 1 and 2; conditional).** If unit 1 demonstrates misleading guidance or a bounded implementation defect, correct exactly that case while retaining immutable snapshots and explicit refresh. Add a behavioral regression only for an implementation defect. CLI edits require the full CLI suite; engine/protocol/payload/shell edits require their AGENTS.md gates, with at most one full build. Lifecycle behavior changes update docs and scaffold together, and ADR/ROADMAP where required. If the contract holds, existing evidence covers it or the correction cannot be bounded safely, this unit is not dispatchable: return the finding to separate planning, with no automatic repeat or adjacent survey [rec: sleepy-meadow-2615].

Consent qualification is complete: six staged CLI exits 0/0/3/0/0/0, protected docs/assets and cold recovery held, and 14 recovery tests passed. Its conditional correction is not dispatchable. All seven stored/carried suffixes match in source; runtime Save-As evidence is unchanged. Do not repeat either audit or infer a reporting feature from blank rejection-envelope fields [rec: silver-rain-7333] [rec: empty-ledge-4581].

Signals: 195 iterations, 11.9h elapsed, 3.1h left, $85.47 API-equivalent; Claude five_hour 65%, seven_day 54%; Codex seven_day 46%. One new standing-maintenance direction selected from three, serving missions 1 and 2. The provider-free walk passed and its tracking rejection was repaired; no missing leg justifies another baseline. No Later promotion, charter-gap retirement or criterion reopening. Live repair remains controller-parked: no automatic retry, availability probe, repeated hold record, provider switch, paid fallback, GUI or remote execution [rec: lucid-snow-0350] [rec: scarlet-journey-0203] [rec: sunny-walrus-5847] [rec: sleepy-meadow-2615].
