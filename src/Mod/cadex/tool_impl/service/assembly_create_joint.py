...
def run(
    ...
    def verify(result: dict[str, Any]) -> dict[str, Any]:
        verification = domain_runtime.assembly_solver_verification(result)
        ...
        verification["checks"] = [connector_check, *verification["checks"]]
        if not connector_check["ok"]:
            verification["ok"] = False
            verification.setdefault(
                "error",
                "Joint connector readback failed: expected two resolved connector frames.",
            )
        return verification
