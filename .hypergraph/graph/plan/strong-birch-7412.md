...
1. **The walk criterion's evidence is complete; the frontier moved for the first time in twenty-two iterations (mission 2).** `ot4-mix55` — the same crank-slider prompt `ot4-mix52` had failed at `train`, a second empty project, `CADEX_MODEL=claude-opus-5`, no `--resume`, no supplied script — **completed every leg: exit 0 in 1680.78 s**, peak tree RSS 2,312,118,272 bytes, no watchdog. Design 1649.63 s (revision `70fd2a53`, digest `ee279ea9`; four bodies, one closed loop; the all-revolute plan was built and refused as redundant by MbD, then spent on freedoms real hardware has — a ball rod end at the crank pin, a keyed cylindrical bushing at the rail; six project ADRs, five domain notes, five `PROGRESS.md` rows). Train 26.71 s on CPU, reward/step −0.4055, witness error 1.14e-08 against 1e-04; declare 0.81 s; rollout −19.85 at seed 1; all four eyes. CLI gate 289 passed [rec: chilly-basin-7378]. `crisp-reef-5607` reads "the criterion's evidence is complete" and retains `working`, because the tick is the human's and not a maintainer's. **The tick is not this loop's to take, and planning takes none** [rec: chilly-basin-7378] [rec: golden-dune-8756].

2. **The conditional mjx-free check is delivered, resolved the other way round, and is dropped as a conditional (mission 2).** It was written as "build it only if the guide alone did not change what an unaided design turn authors". The actor built it first and in a better place: ADR-281 makes `assembly.task` refuse a model MJX cannot build **at declaration time**, naming both geoms, both bodies and both kinds, with `candidate_collision_pairs` writing out MuJoCo's static contact filter (`mjx.geom_pairs` reproduced exactly on the walk's own exported model, the static table matching `mjx.has_collision_fn`) and no `mjx` import in the engine; `assembly.mjcf` and `assembly.rollout` untouched; engine suite 2,099 passed / 54 skipped, CLI 289 passed [rec: sleepy-grove-5790]. On the re-run **the refusal never fired** — the design turn authored only boxes and capsules in a contact group facing an empty one, with its own written reason. That is one run of evidence that the constraint reaches an unaided turn, **not a guarantee**, and it is worth nothing more than one run [rec: chilly-basin-7378].

3. **The eyes are evidenced, and one of them is measuring the wrong thing (missions 6 and 2).** All four review calls ran inside a completed walk for the first time: render four views in 0.90 s, XZ section cutting 4 of 4 objects with no misses, inventory 4 components / 0 catalogued, clearance 6 pairs / 0 unknown / **1 intersection naming frame and slider at 648.0 mm³, reported and not gating** — `docs/CLI.md` now says exit 0 does not imply no interference rather than leaving a reader to infer it [rec: chilly-basin-7378] [rec: idle-pond-4961]. What that run also shows is the limit short rank 2 takes: the intersection was measured at the **initial solved pose**, and the 151 solved rollout frames that followed — 74.62° of crank rotation, 20.32 mm of coupler travel — were never checked, because `core.inspect` scope `clearance` reads one pose and `assembly.rollout` has no clearance surface at all. The criterion's evidence stands; the eye's depth is the open work [rec: chilly-basin-7378] [rec: sleepy-hollow-9498].

4. **When this rung's units land and short empties, a fresh walk is the unit — the charter says so, and this pass does not pre-empt it (mission 2).** The rule is "run the documented headless entry point end to end, every time the short rung is empty"; short is not empty, and a second run of the same prompt would re-measure a measured thing at 1680.78 s and a design turn's tokens. **The walk worth running next is the one short rank 2 makes informative**: a mechanism whose review can name an interference that only exists while it moves. Until then: no extra seeds, no alternate-model campaign, no clock-based retry, and a provider refusal remains not a geometry defect [rec: chilly-basin-7378] [rec: golden-dune-8756] [rec: northern-quartz-8401] [rec: square-crest-8869].

5. **Preserve the four seeded criteria's evidence and the human's decision.** All four remain `working` in the projection and **none is retired, blocked or ticked by planning.** The walk now has a fresh from-nothing run through every leg on top of the resumed and earlier from-nothing successes [rec: chilly-basin-7378]; the second-mechanism evidence stands on `ot4-carriage` [rec: rare-cliff-9595] [rec: wandering-jasper-6102]; all four headless review calls exist and have run, with ADR-275's standalone derived sections, ADR-277's missed-object/movement join and ADR-273's coverage ranking behind them [rec: idle-pond-4961]; **Three modes** has the scripted remote handoff (ADR-200/255/268) and the leg-by-leg documented GUI shape (ADR-269) that its own wording allows, with actual remote and GUI runs forbidden by this run's constraints rather than missing [rec: idle-falcon-3004] [rec: cool-eagle-6400]. ADR-276 delivered project model continuity; ADR-278 the detached pending receipt; ADR-279 the both-ends architecture bound; ADR-280 the named cause on a failed training leg; ADR-281 the MJX refusal. These leave dispatch without becoming new prerequisites for acknowledging delivery [rec: restless-wave-7376] [rec: salty-wolf-2393] [rec: sage-glacier-2165] [rec: neat-grotto-9232] [rec: silent-anchor-7513].

6. **Delivered work leaves dispatch.** Seeds 0–3 completed with fixed exported model/task and objective/action identities, training seeds varied and rollout seed 7 held; reward 170.952826823611–175.935971673601, translation 30.078469436328–31.421759939733 mm, rotation zero, every review succeeding and prior artifacts surviving. History updates replace complete files only after successful writing. Recent DECISIONS entries survive the bounded prompt on fresh and resumed turns, and the lifecycle guide no longer promises unconditional repository initialization. The section offset is derived from the accepted snapshot's own bounds and since ADR-273 ranked by objects actually cut, with ADR-275 exposing derivation when standalone `--offset-mm` is omitted (explicit zero remains explicit); the remote dispatcher carries and re-points a warm start; a second walk of a project no longer locks it out (ADR-272); a failed training leg's `--json` error carries the trainer's stderr tail rather than benign startup warnings, with live passthrough preserved (ADR-280). Protection is per-file, not concurrency, multi-document or power-loss safety; context availability is not model understanding [rec: honest-banner-0821] [rec: solar-snow-8543] [rec: western-cliff-6876] [rec: idle-falcon-3004] [rec: rising-rain-0117] [rec: neat-grotto-9232] [rec: silent-anchor-7513] [rec: golden-dune-8756].

7. **Keep comparison and retention limits.** Four-seed ranges are descriptive; the 30 mm action midpoint remains a confound and neither matching objectives nor favorable travel proves equivalent control difficulty or learned improvement. `ot4-mix55`'s reward/step −0.4055 and rollout −19.85 are a toy-scale loop smoke test at 5 iterations × 16 environments, **not** a claim about learned control, and its 1,649 s design leg is one sample of one prompt on one model. Root review exclusions and policy exclusions after default negations govern untracked files; existing tracked/staged paths retain ordinary Git semantics. Legacy identity rows remain unavailable and numeric comparison references may differ from same-kind identity references. A derived section offset changes the artifact path for future runs and proves no motion clearance. No automatic migration, untracking or standalone-rollout reporting work follows [rec: honest-banner-0821] [rec: easy-beacon-3803] [rec: modest-prairie-3356] [rec: steady-chart-0544] [rec: chilly-basin-7378].

8. **Keep the charter boundary.** Keep mode rehearsals, inherited-removal campaigns, catalog, fleet, report rendering and gait training parked; the parked rungs move only when the human edits the charter. Kernel rollout clearance is the one thing this pass promotes, in the bounded form short rank 2 states and on the justification long item 7 asked for — not as a campaign [rec: square-crest-8869]. The quill overlap that ADR-005 called deliberate was disputed by a live design turn and designed out to zero, which is a warning about inferring a cause from a project's own record as much as from arithmetic. No charter gap is deleted, retired or marked done by this pass [rec: restless-cabin-9629] [rec: western-grotto-7499] [rec: sleepy-hollow-9498] [rec: humble-star-3313].
