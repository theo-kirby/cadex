...
def servo_group(pos, rots, half_y):
    ...
    wall = place(part.box(FL_L + 2.0, 2.0 * half_y, T,
                 ...
                 rots, pos)
    pocket = part.box(SB_L + 1.6, SB_W + 1.6, SB_H - 3.0,
                      origin=[-SH_X - 0.8, -(SB_W + 1.6) / 2.0, -SB_H - 1.0])
    tools = [place(pocket, rots, pos),
             place(part.cylinder(1.15, T + 6.0,
                                 origin=[CX - 14.0, 0.0, FL_Z - T - 3.0]),
                   rots, pos),
             place(part.cylinder(1.15, T + 6.0,
                                 origin=[CX + 14.0, 0.0, FL_Z - T - 3.0]),
                   rots, pos)]
...
def build_leg(sgn, tag):
    ...
    # shin: outboard plate, knee servo at the top facing inboard
    hub_ap, th_ap = hub_group([0.0, y, ZA], po)
    sv_kn, wl_kn, tl_kn = servo_group([0.0, y, ZK], pn, 11.0)
    shin_plate = obox(-13.0, 13.0, 6.0, 6.0 + T, ZA - 11.0, ZK + 11.0, sgn)
    shin = part.compound(
        [part.cut(part.fuse([hub_ap, shin_plate, wl_kn]), tl_kn + th_ap),
         sv_kn])

    # thigh: inboard plate, hip pitch servo at the top facing outboard
    hub_kn, th_kn = hub_group([0.0, y, ZK], pn)
    sv_hp, wl_hp, tl_hp = servo_group([0.0, y, ZHP], po, 11.0)
    thigh_plate = obox(-13.0, 13.0, -9.0, -9.0 + T, ZK - 11.0, ZHP + 11.0,
                       sgn)
    thigh = part.compound(
        [part.cut(part.fuse([hub_kn, thigh_plate, wl_hp]), tl_hp + th_kn),
         sv_hp])
    ...
    hip = part.compound(
        [part.cut(part.fuse([hub_hp, hip_plate, wl_hr]), tl_hr + th_hp),
         sv_hr])
...
eye2 = part.cylinder(4.0, 14.0, origin=[8.0, -9.0, TZ + p.torso_h + 10.0],
                     direction=[1.0, 0.0, 0.0])
torso_print = part.cut(part.fuse([pelvis, shell, head, hub_hl, hub_hrr]),
...
def leg_bodies(sgn, c_hip, ch, c_thigh, ct, c_shin, cs, c_ankle, ca,
    ...
    bt = assembly.body(c_thigh, density_kg_m3=DENS, collision=[
        coll_box(30.0, 26.0, ZHP - ZK,
                 [0.0, y - sgn * 4.0, (ZK + ZHP) / 2.0], ct)])
    bs = assembly.body(c_shin, density_kg_m3=DENS, collision=[
        coll_box(30.0, 26.0, ZK - ZA,
                 [0.0, y + sgn * 6.0, (ZA + ZK) / 2.0], cs)])
...
policy = assembly.policy(
    balance,
    weights="biped-balance.cxpolicy",
    sha256="f2911ebb9f8585086f31cfa04b9dac339ccc972678b8b69b8700214e99565e14",
    label="balance_policy")

run = assembly.rollout(policy,
                       frames_per_second=50,
                       seed=int(p.rollout_seed),
                       label="balance_rollout")
