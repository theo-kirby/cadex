...
**The mat's major grid lines are world-anchored (ADR-343, commit `d93183ba` on main, owner work).** Before, zooming past the 24 m floor slid the grid under the model: a headless probe on the operator dashboard (`ot6-heron`) scaling the fit distance by k found the floor at 24 m for k ≤ 1.3, then 25.025 m at k = 2 and 50.05 m at k = 4, with the texture tiled from the plane's corner so world x = 0's phase inside a 2 m block went 0 → 0.256 → 0.512 — up to half a block, so the "1 METER" squares did not sit where a metre is. `floor.js` now builds a unit plane scaled to the footprint with the texture offset `-(size/2)/(2·pitch)` mod 1 set by `resizeStageFloor`, and `environment.setSize` repaints only when pitch or minor changes; a top-down camera aimed at world (3 m, 3 m) at 1.5, 3, 6 and 12 m shows a major-line crossing at the frame centre every time. Regression test over floor sizes 24, 25.025, 50.05, 84 and 671.3 m; video, style, design and server suites 216 passed, 1 skipped. The minor mesh still steps with framing (ADR-331), by design. The new test was not run against the old code, which lacks `resizeStageFloor`; the slide is evidenced by the probe numbers [rec: copper-union-7670].
...
- copper-union-7670 — ADR-343: the floor grid anchored to the world origin; the measured slide on zoom before, the world-(3,3) check after, the size-parametrised regression test
