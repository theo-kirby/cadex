...
1. **Involute spur gear and rack as library values (mission 4; idle-tower-1624, brave-stone-9609).** One commit: `CadexCatalog.py` gains a small ISO 53/ISO 54 gear-standard table and `gear_spec`; `cadex_library_api.py` gains `spur_gear(module, teeth, face_width, *, bore=None)` and `rack(module, teeth, face_width, height)` built through the existing part API from one involute generator (sampled flanks, tip and root arcs, one closed face, extrude). Refuse tooth counts below a documented minimum; record undercut below 17 teeth in `spec["approximate"]`; the spec carries module, tooth count, pitch/base/root/tip diameters, pressure angle, pitch-circle tooth thickness and the standard citation. Tests in `test_library.py`: stubbed construction and listing (new exports and a `gears` family) plus the actual-worker real-kernel shape used for N20 and the joint, asserting one valid solid, tip diameter m(z+2), root diameter m(z-2.5), volume between the root and tip cylinders, rack pitch pi·m and tooth height 2.25 m. Same commit: PROVENANCE §8g standard citation, L3-COVERAGE row, ROADMAP line, one ADR at the next free number, and the `describe_api` golden only if the family listing moves it. Gates: full engine pytest, at most one `pixi run build-engine`, `pixi run stage-engine` finished before the fresh packaged lifecycle gate. One work record with real impacts on idle-tower-1624 and brave-stone-9609. Not authorized: new dependencies, protocol op changes, shell changes, helical or profile-shifted teeth, torque or strength ratings, dynamics coupling claims, walk or scaffold changes. [rec: placid-delta-6677] [rec: steady-rain-3009]
2. **Rack and pinion composed, with mesh and clearance tests (mission 4; idle-tower-1624).** Dispatched only after rank 1's committed record and green gates. `lib.rack_and_pinion(module, pinion_teeth, rack_teeth, face_width, *, backlash=0)` returns a compound of the two named solids at the standard centre distance (pinion pitch radius above the rack pitch line), the spec carrying centre distance, travel per revolution and the datums. Mesh test: real-kernel common volume of pinion and rack below tolerance at five or more rotation phases with the matched rack translation. Clearance test: 0.25 m root clearance measured between tip and root, and minimum flank distance within the declared backlash bound. Same docs, gates and record shape as rank 1; packaged gate after a finished stage. A failing mesh test holds rank 3 and is recorded, not widened. [rec: placid-delta-6677]
3. **Planetary gearbox, conditional (mission 4; idle-tower-1624).** Only after rank 2 lands with its record. `lib.planetary(module, sun_teeth, planet_teeth, planets, face_width)`: internal ring from the same generator with ring teeth = sun + 2·planet, refuse (sun + ring) not divisible by the planet count with a diagnostic, planets equally spaced at the sun–planet centre distance, a plain carrier plate, and the fixed-ring ratio 1 + ring/sun in the spec. Mesh and clearance tests for one sun–planet pair and one planet–ring pair; same gates, docs and record. Replan after the three rather than expanding; the stopped servo, motor and accessory searches stay stopped. [rec: placid-delta-6677]
