...
**Engine-selection guidance no longer overstates what protects the walk.** CLI payload resolution refuses only a nonempty manifest/module schema mismatch; agreement proves neither worker completeness nor shared source provenance, and `docs/CLI.md` §6 and the resolver comment now say so (commit c84fd306, ADR-061 correction). Runtime unchanged; the full installed-engine CLI gate passed **195 with zero skips**, real engine and toy training cases included. Reconcile judgement: retain `working` on the existing toy CPU evidence; this is a description correction that adds no leg, reruns no walk and expands no GUI/remote qualification [rec: tiny-haven-0347] [rec: keen-pebble-3574].
