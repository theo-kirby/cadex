...
The copy procedure requires authoring, training and rendering to stop first; it is not atomic and external symlinks remain outside its contract. The original was renamed temporarily, not deleted. Browser evidence is same-machine private-address access, not a second-device test. Copy100's seed-0 fall at 0.62 seconds is not a ten-seed comparison or evidence of gait improvement; the quota-refused product-agent request led to an explicitly disclosed actor fallback [rec: clever-fern-7568].
