...
1. **Qualify the existing ordinary bundled engine (missions 1, 2 and 6).** Run the documented design then complete headless walk on one fresh hinged-arm project using CLI `--engine` against the manifest root inside the existing local Cadex.app, without launching the app. Record resolved binary/module identity and overrides; no dev-tree substitution or external-stage fallback counts. Inspect four views, an interior section, inventory/named clearance pair, policy witness, revision and committed project-doc bytes. Use existing CPU toy settings and monitoring (15 minutes/3 GB maximum per training run); run packaged lifecycle and full CLI gates sequentially against that same engine and report skips/selection honestly. Preserve compact evidence without policy or rollout payloads or machine paths. An unavailable bundle or refusal names the failed leg and ends this unit. This tests the historical bundle failure, not an assumed current defect [rec: zesty-aspen-6846] [rec: empty-rain-5162].
2. **Conditional stale-packaging refresh (missions 1, 2 and 6).** Dispatch only if rank 1 establishes a stale installed copy. Use the existing documented packaging path, reading build_app.sh first, with at most one full build and no manual module copying or environment-scrub bypass. If prerequisites require another full build, re-plan the split first. Re-run the failed public walk and same-engine gates; shell-zone work also runs pixi run gate. A source defect requires a new bet. A clean baseline withdraws this unit; a clean refresh exhausts the direction, with no repeat administrative or no-change unit [rec: empty-rain-5162].

VISION principles 5 and 3 are corrected; their old repair dispatch is withdrawn, with no charter-gap retirement. Signals: 27 iterations, 4.4h elapsed, 10.6h left. Only this new maintenance direction is selected; GUI stays unlaunched, remote/GPU/provisioning idle and Later criteria parked. Work must name the protected working walk/review criteria and the actual qualification gained or remaining failure [rec: polished-moss-9358] [rec: neat-summit-3586] [rec: empty-rain-5162].
