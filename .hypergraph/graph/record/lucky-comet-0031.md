---
node_id: 2a5e4b18-990b-5687-a769-4675e901663c
slug: lucky-comet-0031
title: 'Ouroboros run: ot5 [f35d19b4] — operator directive'
created_at: '2026-09-12T14:49:28+00:00'
parents:
- odd-banner-6071
summary: ''
---
...
- commit: 011fb5d0927bc753ba5bbd4797e833d136e39c67

## State Impact

- target: NEW gap-d1-live-project-dashboard-reachable — Charter gap, status open: **D1. A live project dashboard is reachable.** One documented command serves one selected project over the machine's Tailscale/private-network address. A browser can open it without a desktop session on the server. Evidence: a headless-browser smoke test against that address, with its command and re. Flip to working only when the criterion is verifiably met.
- target: NEW gap-d2-browser-shows-right-model — Charter gap, status open: **D2. The browser shows the right model and specs.** Interactive 3D orbit/zoom, component identity, declared parameters, design specs and project decisions come from the selected accepted revision. Selecting an earlier run shows its model and specs, visibly identified as historical. Evidence: browse. Flip to working only when the criterion is verifiably met.
- target: NEW gap-d3-training-visible-while-runs — Charter gap, status open: **D3. Training is visible while it runs.** The biped's real GPU training updates status, iteration, reward and loss histories, episode length and checkpoint availability without a page reload. Committed telemetry updates appear within five seconds under the measured test conditions. Missing or stale. Flip to working only when the criterion is verifiably met.
- target: NEW gap-d4-policy-videos-render-persist — Charter gap, status open: **D4. Policy videos render, persist and play headlessly.** At least one verified intermediate checkpoint is rendered and appears in the dashboard while training remains active, and the final policy also has a saved video. Both play and download in the browser; each identifies the model revision, pol. Flip to working only when the criterion is verifiably met.
- target: NEW gap-d5-review-history-survives-design — Charter gap, status open: **D5. Review history survives a design change.** Each run retains the model/script revision, specs, task/training configuration, metrics, policy identity and review/video references needed to interpret it. After a design edit and retraining, both runs remain selectable with their own curves, models . Flip to working only when the criterion is verifiably met.
- target: NEW gap-d6-save-reopen-restart-preserve — Charter gap, status open: **D6. Save, reopen and restart preserve the project.** Save/reopen and restarting the dashboard and engine preserve accepted identity, specs, run history, curves and video access. Restarting the dashboard during training neither stops nor duplicates that training. Evidence: an automated lifecycle te. Flip to working only when the criterion is verifiably met.
- target: NEW gap-d7-save-as-copy-produces — Charter gap, status open: **D7. Save-As/copy produces an independent project.** A documented headless operation copies the project and its retained review artifacts; another server can inspect the copy. Changing/retraining the copy leaves the original unchanged, and the copy remains usable with the original unavailable. Evid. Flip to working only when the criterion is verifiably met.
- target: NEW gap-d8-interrupted-failed-runs-remain — Charter gap, status open: **D8. Interrupted and failed runs remain understandable.** Test a controlled training interruption, a failed run, and missing/partial review output. The dashboard distinguishes interrupted/failed/stale states from success, preserves prior completed results and explains the next CLI action. Evidence:. Flip to working only when the criterion is verifiably met.
- target: NEW gap-d9-fresh-biped-completes-whole — Charter gap, status open: **D9. The fresh biped completes the whole recorded lifecycle.** The product agent creates and documents a new biped, trains and reviews it through this system, uses that review to make a reasoned design change, and retrains. Both runs have saved playable videos and measured displacement, survival an. Flip to working only when the criterion is verifiably met.
