...
Gui::Action* StdCmdDrawStyle::createAction()
{
    ...
    QObject::connect(pcAction, &Gui::ActionGroup::aboutToShow, [](QMenu* menu) {
        ...
        Command* gridCommand =
            Application::Instance->commandManager().getCommandByName("Cadex_ToggleGrid");
        if (!gridCommand) {
            return;
        }

        if (!menu->property("CadexGridActionAdded").toBool()) {
            menu->addSeparator();
            gridCommand->addTo(menu);
            menu->setProperty("CadexGridActionAdded", true);
        }
...
// Cadex_ToggleGrid
//===========================================================================
DEF_STD_CMD_AC(CadexCmdToggleGrid)

CadexCmdToggleGrid::CadexCmdToggleGrid()
    : Command("Cadex_ToggleGrid")
{
    sGroup = "Standard-View";
    sMenuText = QT_TR_NOOP("Toggle &Grid");
    sToolTipText = QT_TR_NOOP("Toggles the reference grid in 3D views");
    sStatusTip = sToolTipText;
    sWhatsThis = "Cadex_ToggleGrid";
}

void CadexCmdToggleGrid::activated(int iMsg)
{
    Q_UNUSED(iMsg);
    doCommand(Command::Gui, "import CadexGrid; CadexGrid.toggle_grid()");
...
bool CadexCmdToggleGrid::isActive()
{
    auto hGrp = App::GetApplication().GetParameterGroupByPath(
        "User parameter:BaseApp/Preferences/Mod/Draft"
    );
...
Action* CadexCmdToggleGrid::createAction()
{
    Action* pcAction = Command::createAction();
    pcAction->setCheckable(true);
    return pcAction;
...
void CreateViewStdCommands()
{
    ...
    rcCmdMgr.addCommand(new CadexCmdToggleGrid());
