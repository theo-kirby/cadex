...
| D4 videos | Same two training receipts; [retry](interruption86-evidence.json); [current full decode/playback/download](current94-evidence.json); [encoder failure during real training](render98-evidence.json); [interrupted download recovery](download104-evidence.json); [browser-cancelled download recovery](download106-evidence.json) | Intermediate recordings played while training remained active and finals persisted. [RENDER98.md](RENDER98.md) repeats real encoder-failure isolation on Lark: the failed re-render was shown with retry guidance beside the still-available recording, `lark2-final` kept playing, and the sole trainer kept its PID through recovery to completion. A download of the real final video cancelled from the browser part-way through left the page polling and playing, and the next download matched the retained bytes. |
| D5 history after revision | [Agent revision/comparison](revision84-evidence.json), [copy browser history](copy85-evidence.json), [current historical selection](current94-evidence.json), [completed runs with unstored policies listed and closed](policy-store111-evidence.json) | Both designs retain their own documents, configurations, curves, policy identities and videos; earlier bytes preserved. A completed run whose policy lived only under its `train/` (`lark96-restart`, `lark109-engine2`) is listed as a retention gap with the store command until the store holds it (ADR-327); both were stored through the page's own command with every record byte unchanged. |
| D6 reopen/restart | [Two in-place creation restores](evidence.json), [two copy restores](copy85-evidence.json), [service restarts with retained Lark video](../operator-review/README.md), [engine killed and restarted during real training](engine109-evidence.json) | Lark save/reopen, [dashboard restart during real GPU training](RESTART96.md) and [engine kill/restart during real GPU training](ENGINE109.md) now have receipts: same trainer PID/start identity, increasing telemetry on the open page, preserved historical playback, unchanged accepted identity and earlier run files, and completion on the persistent URL. |
...
| D8 interruptions/failures | [Real Lark interruption and successful retry](interruption86-evidence.json), [failed observation with finished training explained and its store command followed](policy-store110-evidence.json) | Failed status, explanation, new-attempt guidance and prior results preserved. [Lark missing/partial-video injection](VIDEO95.md) now verifies explicit guidance, preserved historical playback/download and recovery after restoring artifacts. `lark109-engine` (ADR-326) is shown as training that finished with a failure after it, with the store state and the exact `cadex asset` command; running it flipped the open page to `stored` in 0.2 s with the run still `failed` and every earlier file unchanged. A `completed` run in the same state is listed under problems with the command ([receipt](policy-store111-evidence.json), ADR-327); a failed one is not, because its failure is the problem. |
