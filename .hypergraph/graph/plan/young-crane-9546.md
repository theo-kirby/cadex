...
1. **Planetary gearbox as a library value (mission 4; idle-tower-1624, brave-stone-9609).** Dispatched now: ranks 1 and 2 of the gear sequence landed with records and green gates (ADR-233: 2050/52 and 107/0; ADR-234: zero common volume at 27 phase builds, 2061/52 pre-build, 119/0 packaged) and are reconciled, so the condition placid-delta-6677 set on this unit is met. `lib.planetary(module, sun_teeth, planet_teeth, planets, face_width)`: internal ring from the ADR-233 generator with ring teeth = sun + 2·planet, refuse (sun + ring) not divisible by the planet count with a diagnostic, planets equally spaced at the sun–planet centre distance, a plain carrier plate, the fixed-ring ratio 1 + ring/sun in the spec, one compound placed like every other library value; `CadexCatalog.planetary_spec` derives the numbers the way `rack_and_pinion_spec` does. Mesh and clearance tests for one sun–planet pair and one planet–ring pair in the real-kernel shape of ADR-234, with at least one negative control. Same commit: ADR-235, PROVENANCE §8g, L3-COVERAGE, ROADMAP, XSCRIPT; the `describe_api` golden only if the family listing moves. Gates: full engine pytest, at most one `pixi run build-engine`, `pixi run stage-engine` finished before the fresh packaged lifecycle/library gate. One work record with impacts on idle-tower-1624 and brave-stone-9609; a passing planetary closes the compound-mechanisms criterion and the record says so. Not authorized: new dependencies, protocol op changes, shell changes, helical or profile-shifted teeth, torque or strength ratings, dynamics coupling claims, walk or scaffold changes. [rec: flat-river-8853] [rec: placid-delta-6677] [rec: mellow-garden-0940] [rec: wild-beacon-4213]
2. **Assembly inventory with catalog ids, one CLI call (mission 6; damp-moon-9297).** Dispatched after rank 1's committed record. Verified premises: `LibraryPart` carries `family` and `part_number` on every library value, and `assembly.component` links a `source` mapping to another domain's output, so the part domain publishes catalog identity on library-value outputs and a new `inspect` scope walks an assembly output's components back to their sources, returning placed parts with catalog ids, placement and source output name. `inspect` takes `scope` as a string, so the new scope is an argument value inside the existing op and needs no `OP_ARG_SPECS` change. `cadex` exposes it as one CLI call that writes the inventory into the project directory (a documented path and filename, JSON plus a short Markdown table); `docs/CLI.md` gains the row in the same commit. Tests: stubbed protocol coverage for the scope, a real-kernel test that assembles two library values and reads the inventory back with both ids, and the packaged lifecycle gate after a finished stage. One work record with an impact on damp-moon-9297. Not authorized: an `OP_ARG_SPECS` change, any shell change, a walk or scaffold change; wiring the walk's review step is a later unit that carries `project_docs.py` and its test together. [rec: flat-river-8853] [rec: placid-delta-6677] [rec: wild-marsh-9611]
3. **Clearance and intersection check naming pairs, conditional (mission 6; damp-moon-9297).** Only after rank 2 lands with its record, and only if the loop signals leave room for a unit that adds one read op. The op takes an assembly output and returns the pairs of placed solids whose common volume exceeds a tolerance or whose minimum distance falls below a declared clearance, named by component label and catalog id; INTEGRATION.md row, `OP_ARG_SPECS`, shell client update and response golden in the same commit (methodology rule 6); `cadex` writes the report into the project directory. The only check today is the declared swept clearance validated at rebuild (ADR-130). Real-kernel test with one interfering and one clear pair; packaged gate after a finished stage. If the budget does not allow it, it returns to medium with this specification. [rec: flat-river-8853] [rec: placid-delta-6677]
