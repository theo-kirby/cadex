...
def prepare_isolated_measurement(
    ...
    staging = Path(tempfile.mkdtemp(prefix="cadex-geometry-"))
    try:
...
def cleanup_isolated_measurement(prepared: dict[str, Any]) -> None:
    ...
    if staging.name.startswith("cadex-geometry-"):
        shutil.rmtree(staging, ignore_errors=True)
...
def _openscad_artifact(
    ...
    # The isolated OpenSCAD engine was removed, so no persisted OpenSCAD
    # measurement artifact ever exists; measurement always exports the live
    # BREP shape directly.
    return None
...
def _resolve_distance_reference(service: Any, reference: Any) -> dict[str, Any]:
    ...
    fidelity = str(
        getattr(obj, "CadexGeometryFidelity", "exact_brep") or "exact_brep"
    )
    return {
        "ok": True,
        "kind": "shape",
...
def _shape_center(shape: Any) -> Any:
    """Whole-shape center; Part.Compound exposes no CenterOfMass attribute."""
    center = getattr(shape, "CenterOfMass", None)
    if center is not None:
        return center
    weighted = [
        (solid.CenterOfMass, float(solid.Volume))
        for solid in list(getattr(shape, "Solids", None) or [])
        if float(solid.Volume) > 0.0
    ]
    total = sum(volume for _, volume in weighted)
    if total > 0.0:
        return SimpleNamespace(
            x=sum(c.x * volume for c, volume in weighted) / total,
            y=sum(c.y * volume for c, volume in weighted) / total,
            z=sum(c.z * volume for c, volume in weighted) / total,
        )
