...
1. **Rehearse the walk at its historical-inventory boundary (missions 1/2/6).** Run one public model-free existing-script toy walk in an isolated project with explicit CPU, one iteration/four environments, fresh output and policy name. Verify policy, rollout, all four reviews, project history and resource use. Then accept one minimal named-component change via the normal script command and run standalone inventory, without additional training. Compare the old review's counts/link, the new docs/inventory.md and Git's historical report. Replace redundant CLI inventory prose with the measured latest-report versus per-run distinction; align the scaffold by concise reference. This is one rehearsal plus a documentation correction, not an audit-only dispatch or a claim of lost Git history. Run the CLI gate, update verified dates, ADR removal notes and ROADMAP. Each training run ≤15 minutes/3 GB; no model, GUI, remote or generated-output commit [rec: floral-arrow-7365] [rec: scarlet-ocean-2920].
2. **Retain the existing named inventory in each walk review (missions 1/2/6).** After unit 1 confirms the source-derived limitation, replace the lossy count-only projection with existing inspection fields for revision, component/source names and catalog identities; preserve existing summary fields and label the report path as latest-project documentation. Reuse existing review serialization and extend an existing lifecycle regression with different inventories across reviews, asserting old saved rows/revision survive while current docs advance. No engine operation, archive subsystem, catalog inference or new test matrix. Consolidate repeated projection/prose so this finite direction removes more than it adds; update guide/scaffold, ADR and ROADMAP, and run the full CLI gate using the requested CPU fixture without an outer override. If unit 1 disproves the finding, replan instead of implementing. Stop after resolution; no automatic adjacent campaign [rec: keen-field-4379] [rec: scarlet-ocean-2920].
