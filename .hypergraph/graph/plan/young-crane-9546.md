...
1. **Remove the unsupported inventory tally in a fix-forward commit (missions 6 and 2).** Review the eight-file change captured in 997293b8, then delete its generator-call registry, report field, generated-minus-placed inventory claims and downstream CLI/walk counts, together with tests whose sole purpose is that discarded contract. Keep ADR-236 catalog identity on actually placed components and all pre-existing inventory behavior. Do not revert or rewrite the earlier commit; make a new subtractive correction. The diagnostic cannot infer physical part quantities or name fused parts from calls. Verify the retained real-kernel inventory behavior for placed catalog components, including one generated value linked more than once; the inventory must count placed instances without claiming a purchase deficit. Add an ADR documenting why the unqualified tally was removed, correct its dangling ADR-243 references, update CLI/INTEGRATION and lifecycle/scaffold docs wherever current behavior is described, and ROADMAP if a landed item exists. Run full engine and CLI suites, one supported engine build/install/stage for the removal, then the packaged lifecycle gate against that fresh payload; state any gate failure and bundle staleness honestly. No shell diff or GUI launch. Record impact on damp-moon-9297, preserving its already-working baseline while correcting source that its latest evidence did not qualify [rec: odd-ridge-9607] [rec: fair-rose-5950] [rec: staid-willow-6557].
2. **Teach the walk to place catalog hardware as components (missions 2 and 6).** Promote the existing guidance unit: the walk's design instructions and project scaffold explain that purchased hardware is placed as assembly components, separate from printed solids; transformed catalog bodies may serve as clearance cutters without implying another purchased part. Keep the convention concise and aligned with the existing engine API, with no new tool, registry, validator or dependency. Pin the observable prompt/scaffold contract in the existing tests, update the walk documentation and its date in the same commit, and record the direction in DECISIONS and a landed ROADMAP item where required. Run the full CLI suite against a built engine; run any other zone gate only if that zone is touched. This is guidance, not proof an agent followed it and not a claim that discarded generator calls are missing hardware [rec: forest-hollow-9339] [rec: morning-summit-7848] [rec: odd-ridge-9607] [rec: staid-willow-6557].
3. **Retry the two-servo leg once quota permits (missions 2 and 6, the ladder rule).** The prior attempt reached no accepted design. Wait for the provider's reported reset or affirmative availability evidence before dispatch; no repeated provider probes, paid fallback or provider switch. If still blocked when eligible, defer this unit and report the blocker rather than waiting in an actor. After ranks 1 and 2 land, run the same documented walk and mechanism prompt from silent-mist-5233 in a fresh project outside the checkout, with the corrected CLI and an explicitly identified engine carrying the correction (prefer rank 1's fresh payload; do not claim the older installed bundle contains it). Use the training venv, JAX_PLATFORMS=cpu, --iterations 1 --envs 4 --seed 0 --timeout 600, and the existing process-tree guard at 2.9 GB and 850 s. Report every leg as clean, needing a guess/person, or not reached, with exact errors. Inspect all four views and section, compare script catalog usage with placed inventory manually, and use the bound-agreement checker if catalog rows exist. No source correction in the rehearsal; failures name the next unit. No absence-of-tally value is evidence that no catalog part was fused. Record crisp-reef-5607 and swift-dusk-2951 evidence, and damp-moon-9297 only for review findings [rec: silent-mist-5233] [rec: morning-summit-7848] [rec: staid-willow-6557].

Signals: 37 iterations, 6.8h elapsed, 8.2h left; Claude five_hour 75% and seven_day 47%. Two provider-independent units and one conditional rehearsal fit this planning horizon; utilization does not overturn the observed session refusal. The prior output-only diagnostic is superseded by its failed prerequisite, not shipped. Existing lifecycle/review criteria stay working, no charter checkbox changes, Later criteria stay parked, GUI remains unlaunched and remote remains scripted-only [rec: odd-ridge-9607] [rec: silent-mist-5233] [rec: staid-willow-6557].
