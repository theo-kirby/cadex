"""One bounded real GPU training run on Heron, the two-DoF MG90S arm (D8,
...
This is the Robin driver (``../robin/train.py``) with two Heron facts: the
accepted view carries 15 components (three printed parts and twelve catalog
parts, ADR-339) and the authorship default names Heron. The model, task and
...
authored_by = sys.argv[4] if len(sys.argv) > 4 else 'Heron, authored by the product agent from the D8 creation prompt and corrected three times by the same agent against quoted measurements (ADR-339); the reach task is declared in the same script'
learning_rate = float(sys.argv[5]) if len(sys.argv) > 5 else 3.0e-4   # the trainer's default (Robin diverged at it; Finch did not)
...
ev.mkdir(exist_ok=True); train.mkdir(parents=True, exist_ok=False)   # Heron has no evidence/ before this unit
...
assert components == 15, components   # three printed parts and twelve catalog parts (ADR-339)
