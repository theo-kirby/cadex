...
"""Authoritative Cadex tool contracts and runtime registry."""
...
class ToolRegistry:
    def __init__(self) -> None:
        self._tools: dict[str, CadexTool] = {}

    def register(self, tool: CadexTool) -> None:
        if tool.name in self._tools:
            raise ValueError(f"Cadex tool already registered: {tool.name}")
    ...
    def register_spec(
        ...
        tool = CadexTool(spec=ToolSpec.from_mapping(raw_spec), handler=handler)
        self.register(tool)
        return tool
