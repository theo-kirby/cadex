...
| `cadex_print.py` | **cadex ADR-156.** Which outputs are parts to print: a cache of the engine's roster, and `toggle()`, which reads it, flips one name and pushes the **whole** list back through `set_printable`. Nothing here draws and nothing here writes a file — the panel is one operator row per entry in `ui.py` and the STLs are the engine's to write. The cache costs no round trip: the roster rides in the `inspect scope="script"` block `cadex_backend._adopt_script_state` already takes on open and after every accepted rebuild, which is also what keeps it from ever being older than the model. A mark is **not** a `set_params` table and deliberately bypasses the `begin_set_tables` pump: that pump coalesces a drag's stream into one rebuild, and a print mark changes no geometry, so there is no rebuild to coalesce. |
