...
1. **Correct obsolete solver-migration recovery guidance (mission 1, standing maintenance).** Replace only VISION.md's obsolete Open questions paragraph with the resolved ADR-187 behavior: changed accepted output is refused; the user explicitly re-accepts the engine-stored script through the shipped recovery action. Verify against cadex_backend.py, the chat panel and the existing regression; update the document date, add the ADR-187 correction note for removing the stale claim, and follow ROADMAP conventions. Commit one narrow documentation correction and one record with real state targets. Run documentation/diff and graph checks; report that no runtime gate was rerun for prose-only work. No runtime edit, migration framework or broad audit is selected [rec: weathered-haven-8656] [rec: green-wolf-7549].
2. **Conditional correction of a concrete review finding (mission 1).** Dispatch only for a critic must-fix on rank 1 or source evidence contradicting its proposed wording. Resolve that exact finding in one small unit with the touched zone's required gate; a runtime defect requires a recorded scope decision before implementation. If the documentation correction is accepted without a finding, this unit is not actionable: re-plan, do not manufacture a repair or rerun the clean walk [rec: green-wolf-7549].

The cold revisit landed with policy/review identity preserved and 195 CLI passes, zero skips; its former persistence repair has no trigger. No charter gap is retired. Signals: 21 iterations, 4.0h elapsed, 11.0h left; only the bounded correction and conditional review response are selected. Keep GUI unlaunched, remote/GPU/provisioning idle, and Later criteria parked; at most one full build per unit if required by its zone [rec: fair-cedar-7455] [rec: green-wolf-7549].
