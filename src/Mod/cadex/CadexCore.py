...
"""Core Cadex context and read-only FreeCAD tools."""
...
MAX_GEOMETRY_REFERENCES = 12
GEOMETRY_REFERENCE_NOTE = (
    "User-pinned geometry for THIS message. `subelement` names are only valid at "
    "`captured_at_revision`; if it differs from `cad_revision`, re-resolve via "
    "`partdesign.find_subelements` using the fingerprint (object_name, geometry_type, "
    "near_point from center_of_mass, plus direction/normal/radius/length)."
)
...
class CadexService:
    ...
    def __init__(self, dotenv_path: Path | None = None) -> None:
        ...
        self._geometry_references: list[dict[str, Any]] = []
        self._geometry_handle_high_water: dict[str, int] = {}
        ...
        self._project_store = CadexProjectStore(self._local_session_id)
    ...
    def xscript_enabled(self) -> bool:
        return bool(load_settings().xscript_enabled)
    ...
    def complete_modeling_engine_read(prepared: dict[str, Any]) -> str:
        ...
        if cached:
            return cached
        return CadexProjectStore.read_modeling_engine_manifest(
    ...
    def accept_modeling_engine_read(
        ...
        if clean != "xscript":
            raise RuntimeError(f"Cadex project returned invalid modeling engine {clean!r}.")
    ...
    def modeling_engine_state(self, workbench: str | None = None) -> dict[str, Any]:
        from CadexScriptedDomains import XSCRIPT_VERSION

        active_workbench = workbench or self.active_workbench_name()
        xscript_enabled = self.xscript_enabled()
        if xscript_enabled:
            xscript_state = {
                "ready": True,
                "version": XSCRIPT_VERSION,
                "error": "",
            }
        else:
            xscript_state = {
                "ready": False,
                "version": XSCRIPT_VERSION,
                "error": "XScript is disabled in Cadex Preferences.",
            }
        return {
            "selected": self.modeling_engine(),
            "global": True,
            "workbench": active_workbench,
            "xscript_preference_enabled": xscript_enabled,
            "available_engines": [
                *(["xscript"] if xscript_enabled else []),
            ],
            "xscript": xscript_state,
        }

    def set_modeling_engine(self, engine: str) -> dict[str, Any]:
        clean = str(engine or "").strip().lower()
        if clean != "xscript":
            raise ValueError("The only modeling engine is xscript.")
        ...
        if not self.xscript_enabled():
            raise RuntimeError("Enable the xscript engine in Cadex Preferences before selecting it.")
    ...
    def coerce_modeling_engine_for_workbench(self, workbench: str | None) -> dict[str, Any]:
        """No-op: the scripted engines author on every workbench.

        Kept as a stable seam for the GUI workbench-change hooks; there is no
        Part Design-only engine left to coerce away from.
        """

        return {"ok": True, "changed": False, "engine": self.modeling_engine()}

    def active_document_engine_classification(self) -> dict[str, Any]:
        """Classify the active document as plain | xscript | xscript | mixed."""

        from CadexScriptedDomains import document_engine, document_program_refs

        doc = self._active_document()
        project_root = str(self.project_scope_snapshot().get("root") or "").strip()
        if doc is None or not project_root:
            return {
                "engine": "plain",
                "program_count": 0,
        ...
        return {
            "engine": document_engine(doc, project_root),
            "program_count": len(refs),
            "project_root": project_root,
        }

    def convert_active_document_to_xscript(self) -> dict[str, Any]:
        """Re-tag the active document's programs to xscript and flip the engine.

        Metadata-only: each program manifest's schema tag is rewritten (the api
        graph and published BREP are engine-agnostic, so no recompute is needed)
        and the project engine becomes xscript.
        """

        from CadexScriptedDomains import (
            document_program_refs,
            retag_programs_to_xscript,
        )
        ...
        if doc is None or not project_root:
            raise RuntimeError(
                "Save the document before converting it to the xscript engine."
            )
        ...
        self._ensure_modeling_engine_change_allowed()
        refs = document_program_refs(doc)
        retag = retag_programs_to_xscript(project_root, refs)
        result = self._project_store.set_modeling_engine("xscript")
        self._cache_modeling_engine("xscript")
        self._provider_working_object_names = []
        return {"ok": True, "engine": "xscript", **retag, **result}
    ...
    def document_persistence_state(self) -> dict[str, Any]:
        ...
        if doc is None:
            return {
                ...
                "document": None,
                "file_path": None,
                "message": "Create and save a Cadex document to enable Cadex.",
            }
        ...
        if not file_path:
            return {
                ...
                "file_path": None,
                "message": "Save this Cadex document to enable Cadex.",
            }
        return {
            "enabled": True,
            "reason": "ready",
    ...
    def _object_summary(obj: Any) -> dict[str, Any]:
        ...
        role = str(getattr(obj, "CadexScriptedRole", "") or "")
        if role:
            summary["scripted_role"] = role
            summary["scripted_engine"] = str(
                getattr(obj, "CadexScriptedEngine", "") or ""
            )
            summary["scripted_model_id"] = str(
                getattr(obj, "CadexScriptedModelId", "") or ""
            )
            output_key = str(
                getattr(obj, "CadexScriptedOutputKey", "") or ""
            )
            if output_key:
    ...
    def _is_private_scripted_object(obj: Any) -> bool:
        return (
            str(getattr(obj, "CadexScriptedRole", "") or "")
    ...
    def _reference_artifact_dir(self) -> Path:
        ...
        Project roots always live under the central Cadex data dir, so
        references are never written next to the CAD file. Without a project
        context the default path still lands inside ``cadex_data_dir()``.
        """
        try:
        ...
        return cadex_data_dir() / "references"
    ...
    def _load_reference_images_for_active_project(self) -> None:
        try:
            ...
            path = cadex_data_dir() / "references.json"
        key = str(path)
    ...
    def _write_reference_images(self) -> None:
        try:
            ...
            raise RuntimeError(
                f"Cadex references could not be written: {exc}"
            ) from exc
    ...
    # ------------------------------------------------------------------
    # Pinned geometry references
    #
    # Deliberate, message-scoped picks of live geometry (mirrors the
    # reference-image subsystem, but delivered as structured JSON rather than
    # image blocks). Held in memory for the session; each carries a geometric
    # fingerprint so the agent can re-resolve the target via
    # ``partdesign.find_subelements`` after a recompute drifts topology names.
    # ------------------------------------------------------------------

    def capture_selection_reference(self) -> dict[str, Any]:
        """Pin the live 3D selection as durable geometry references.

        Reads the selection itself (symmetric with ``selection_summary``) and
        appends one reference per picked subelement, each with a stable handle
        and a geometric fingerprint. Never raises for bad input; returns a
        structured ``{"ok": False, "error": ...}`` instead.
        """
        try:
            import FreeCADGui as Gui
        except Exception:
            return {"ok": False, "error": "Selection is only available in the GUI."}
        try:
        ...
        if not selected:
            return {
                "ok": False,
                "error": "Select an edge, face, or vertex in the 3D view first.",
            }
        revision = self.structural_document_revision()
        counters = self._geometry_handle_counters()
        captured: list[dict[str, Any]] = []
        # ``counters`` is the persistent per-kind high-water mark; mutating it in
        # place (via _next_geometry_handle) keeps handles monotonic so a removed
        # @edge-1 is never handed out again while later pins reference it.
        truncated = False
        for item in selected:
            try:
                obj = item.Object
            ...
            sub_objects = list(getattr(item, "SubObjects", []) or [])
            picks: list[tuple[str | None, Any]]
            if not sub_names:
                picks = [(None, None)]
            else:
                picks = [
                    (name, sub_objects[index] if index < len(sub_objects) else None)
                    for index, name in enumerate(sub_names)
                ]
            for sub_name, sub_shape in picks:
                if len(self._geometry_references) + len(captured) >= MAX_GEOMETRY_REFERENCES:
                    truncated = True
                    break
                world_space = bool(sub_name) and "." in str(sub_name)
                if sub_name is None:
                    reference = self._build_object_reference(obj, revision, counters)
                else:
                    # Non-assembly picks read obj.Shape[idx-1] (object-local) so
                    # the fingerprint matches find_subelements exactly; only
                    # dotted assembly paths fall back to the world-space
                    # SubObjects shape (there is no single local basis to use).
                    reference = self._build_subelement_reference(
                        obj,
                        sub_name,
                        revision,
                        counters,
                        sub_shape=sub_shape if world_space else None,
            ...
            if truncated:
                break
        self._geometry_references.extend(captured)
        result: dict[str, Any] = {
            "ok": True,
            "references": [dict(entry) for entry in captured],
            "count": len(self._geometry_references),
        }
        if truncated:
            result["truncated"] = True
            result["note"] = (
                f"At most {MAX_GEOMETRY_REFERENCES} geometry references may be pinned; "
                "some picks were dropped."
            )
        return result

    def _geometry_handle_counters(self) -> dict[str, int]:
        """Persistent per-kind high-water mark; handles never renumber on removal.

        Reconciled with any currently pinned handles so a service reconstructed
        without the counter (e.g. via ``__new__`` in tests) still advances past
        live references rather than colliding with them.
        """
        counters = getattr(self, "_geometry_handle_high_water", None)
        if counters is None:
            counters = {}
            self._geometry_handle_high_water = counters
        for entry in self._geometry_references:
            match = re.match(r"^@([a-z_]+)-(\d+)$", str(entry.get("handle", "")))
            if match is None:
                continue
    ...
    @staticmethod
    def _next_geometry_handle(counters: dict[str, int], kind: str) -> str:
        counters[kind] = counters.get(kind, 0) + 1
        return f"@{kind}-{counters[kind]}"

    def _build_object_reference(
        self, obj: Any, revision: str, counters: dict[str, int]
    ) -> dict[str, Any]:
        from tool_impl.service.partdesign_find_subelements import _bounding_box_dict

        reference: dict[str, Any] = {
            "id": uuid.uuid4().hex[:12],
            "kind": "object",
            "handle": self._next_geometry_handle(counters, "object"),
            "object_name": str(getattr(obj, "Name", "") or ""),
            "object_label": str(getattr(obj, "Label", "") or getattr(obj, "Name", "")),
        ...
        if shape is None:
            ...
            reference["fingerprint_note"] = "no shape geometry"
            return reference
        try:
            fingerprint["bounding_box"] = _bounding_box_dict(shape.BoundBox)
        except Exception:
            pass
        try:
            fingerprint["volume"] = round(float(shape.Volume), 6)
        except Exception:
            pass
        try:
            fingerprint["shape_type"] = str(shape.ShapeType)
        except Exception:
            pass
        reference["fingerprint"] = fingerprint
        return reference

    def _build_subelement_reference(
        self,
        obj: Any,
        sub_name: str,
        revision: str,
        ...
        *,
        ...
        topo = str(sub_name).split(".")[-1]
        is_sketch = str(getattr(obj, "TypeId", "")) == "Sketcher::SketchObject"
        if is_sketch:
            kind = "sketch_element"
        else:
            prefix_match = re.match(r"^(Edge|Face|Vertex)\d+$", topo)
            kind = {
                "Edge": "edge",
                "Face": "face",
                "Vertex": "vertex",
            }.get(prefix_match.group(1) if prefix_match else "", "subelement")
        reference: dict[str, Any] = {
            "id": uuid.uuid4().hex[:12],
            "kind": kind,
            "handle": self._next_geometry_handle(counters, kind),
            "object_name": str(getattr(obj, "Name", "") or ""),
            "object_label": str(getattr(obj, "Label", "") or getattr(obj, "Name", "")),
            "object_type": str(getattr(obj, "TypeId", "") or ""),
        ...
        if is_sketch:
            ...
            reference["fingerprint_note"] = (
                "Sketch geometry: re-resolve with sketch tools, not "
                "partdesign.find_subelements."
            )
            return reference
        fingerprint, note = self._subelement_fingerprint(
            obj, sub_name, kind, sub_shape=sub_shape, world_space=world_space
        )
        reference["fingerprint"] = fingerprint
        if note:
            reference["fingerprint_note"] = note
        if world_space:
            reference["coordinate_space"] = "world"
        return reference

    def _subelement_fingerprint(
        self,
        obj: Any,
        sub_name: str,
        kind: str,
        *,
        ...
        """Geometric fingerprint whose fields match ``find_subelements`` output.

        Non-assembly picks read the object-local ``obj.Shape`` element so the
        numbers are byte-identical to what ``partdesign.find_subelements``
        returns for the same subelement; assembly picks fall back to the
        world-space ``item.SubObjects`` shape. Every field is guarded
        independently so a single failed evaluation never drops the reference.
        """
        from tool_impl.service.partdesign_find_subelements import (
            _bounding_box_dict,
            _canonical_geometry_type,
            _outward_normal,
            _vector_dict,
        )
        ...
        fingerprint: dict[str, Any] = {}
        element = sub_shape
        owner_shape = getattr(obj, "Shape", None)
        if element is None:
            topo = str(sub_name).split(".")[-1]
            match = re.match(r"^(Edge|Face|Vertex)(\d+)$", topo)
            if match is None or owner_shape is None:
                return fingerprint, "no shape geometry"
            index = int(match.group(2))
            try:
                if kind == "edge":
                    element = list(getattr(owner_shape, "Edges", []) or [])[index - 1]
                elif kind == "face":
                    element = list(getattr(owner_shape, "Faces", []) or [])[index - 1]
                elif kind == "vertex":
                    element = list(getattr(owner_shape, "Vertexes", []) or [])[index - 1]
            except Exception:
                return fingerprint, "subelement index out of range"
        if element is None:
            return fingerprint, "no shape geometry"

        if kind == "vertex":
            try:
                point = getattr(element, "Point", None)
                if point is None:
                    point = element.CenterOfMass
                fingerprint["point"] = _vector_dict(point)
            except Exception:
                pass
        ...
        if kind == "edge":
            ...
            try:
                fingerprint["geometry_type"] = _canonical_geometry_type(
                    type(curve).__name__ if curve is not None else ""
                )
            except Exception:
                pass
            try:
                fingerprint["length"] = round(float(element.Length), 6)
            except Exception:
                pass
            try:
                fingerprint["center_of_mass"] = _vector_dict(element.CenterOfMass)
            except Exception:
                pass
            if fingerprint.get("geometry_type") == "line":
                try:
                    direction = element.tangentAt(element.FirstParameter)
                    if float(direction.Length) > 1e-9:
                        direction.normalize()
                        fingerprint["direction"] = _vector_dict(direction)
                except Exception:
                    pass
            try:
                radius = getattr(curve, "Radius", None)
                if radius is not None:
                    fingerprint["radius"] = round(float(radius), 6)
            except Exception:
                pass
            try:
                fingerprint["bounding_box"] = _bounding_box_dict(element.BoundBox)
            except Exception:
                pass
            return fingerprint, None
        ...
        try:
            ...
            pass
        try:
            fingerprint["area"] = round(float(element.Area), 6)
        except Exception:
            pass
        try:
            fingerprint["center_of_mass"] = _vector_dict(element.CenterOfMass)
            ...
            pass
        if fingerprint.get("geometry_type") == "plane" and not world_space:
            try:
                outward, _diagnostic = _outward_normal(owner_shape, element)
                if outward is not None:
                    fingerprint["outward_normal"] = _vector_dict(outward)
            except Exception:
                pass
        try:
    ...
    def remove_geometry_reference(self, reference_id: str) -> dict[str, Any]:
        ident = str(reference_id or "").strip()
        if not ident:
            return {"ok": False, "error": "Geometry reference id cannot be empty."}
        for index, entry in enumerate(self._geometry_references):
            if entry.get("id") == ident or entry.get("handle") == ident:
                removed = self._geometry_references.pop(index)
                return {
                    "ok": True,
                    "removed": dict(removed),
                    "count": len(self._geometry_references),
                }
        return {"ok": False, "error": f"No pinned geometry matches '{ident}'."}

    def clear_geometry_references(self) -> dict[str, Any]:
        cleared = len(self._geometry_references)
        self._geometry_references = []
        # A full clear is a fresh slate: handles restart at 1. Individual
        # removals keep the high-water mark so they never renumber.
        self._geometry_handle_high_water = {}
        return {"ok": True, "cleared": cleared}

    def geometry_references_summary(self) -> dict[str, Any]:
        if not self._geometry_references:
            return {}
        return {
            "count": len(self._geometry_references),
            "note": GEOMETRY_REFERENCE_NOTE,
            "references": [dict(entry) for entry in self._geometry_references],
        }
    ...
    def _geometry_summary(
        ...
        item = {
            ...
            "construction": CadexService._geometry_construction_state(
                geometry,
                index,
                sketch,
            ),
        ...
        shape_diagnostics = CadexService._geometry_shape_diagnostics(geometry)
    ...
    def _geometry_shape_diagnostics(geometry: Any) -> dict[str, Any]:
        ...
        result: dict[str, Any] = {
            "actual_bounds": CadexService._curve_bound_box_summary(shape.BoundBox),
        }
    ...
    def _sketch_junction_diagnostics(
        ...
        for geometry in geometry_summaries:
            ...
            for role in ("start", "end"):
                ...
                key = CadexService._rounded_endpoint_key(endpoint["point"])
        ...
        for endpoints in endpoint_groups.values():
            ...
            for first, second in itertools.combinations(endpoints, 2):
                ...
                first_tangent = CadexService._outward_endpoint_tangent(first)
                second_tangent = CadexService._outward_endpoint_tangent(second)
    ...
    def _sketch_native_geometry_diagnostics(
        ...
        if summaries is None:
            summaries = [
                CadexService._geometry_summary(item, index, sketch)
                for index, item in enumerate(geometry)
            ]
    ...
    def _point_artifact_project_root(
        ...
        if not root:
            raise RuntimeError(
                "Save or initialize the active Cadex project before managing "
                "point artifacts."
            )
        return root
    ...
    def approve_point_artifact(
        ...
        *,
        ...
        """Approve a human-selected point file for stable-id XScript use."""

        from CadexPointArtifacts import approve_point_artifact
    ...
    def remove_point_artifact(
        ...
        *,
        ...
        from CadexPointArtifacts import remove_point_artifact
    ...
    def _conversation_scope(self) -> dict[str, Any]:
        ...
        per-document project folder under the central Cadex data dir —
    ...
    def conversation_snapshot_for_save(self, doc: Any) -> dict[str, Any]:
        ...
        store: CadexConversationStore | None = None
        if (
            self._conversation_cache_document_uid == document_uid
    ...
    def create_conversation(self) -> dict[str, Any]:
        if not self.document_persistence_state().get("enabled"):
            raise RuntimeError(
                "Save this Cadex document before starting a conversation."
            )
    ...
    def activate_conversation(self, conversation_id: str) -> dict[str, Any]:
        if not self.document_persistence_state().get("enabled"):
            raise RuntimeError(
                "Save this Cadex document before opening a conversation."
            )
    ...
    def relocate_conversation_store_for_document_file(
        ...
        if not source:
            raise RuntimeError(
                "No Cadex conversation store was captured before save."
            )
        result = CadexConversationStore.relocate_directory(
            source,
    ...
    def prepare_conversation_turn(
        ...
        if not persistence.get("enabled"):
            raise RuntimeError(
                "Save this Cadex document before recording a conversation."
            )
    ...
    def persist_prepared_conversation_turn(prepared: dict[str, Any]) -> dict[str, Any]:
        ...
        with _CONVERSATION_WRITE_LOCK:
            store = CadexConversationStore(project_root)
            ...
            if CadexService._conversation_has_equivalent_turn(conversation, entry):
    ...
    def prepare_conversation_read(
        self, conversation_id: str | None = None
    ) -> dict[str, Any]:
        """Capture project identity so the transcript can be read off-thread."""
    ...
    @staticmethod
    def read_prepared_conversation(prepared: dict[str, Any]) -> list[dict[str, Any]]:
        """Read one conversation's turns off the document thread (file I/O only)."""
        ...
        return [dict(item) for item in history.get("conversation") or []]

    def recompute_diagnostics(self) -> dict[str, Any]:
        from CadexTransactions import recompute_diagnostic_summary
    ...
    def provider_tool_surface(self, workbench: str | None = None) -> dict[str, Any]:
        from CadexSession import provider_tool_schemas
    ...
    def is_provider_tool_available(
        ...
        from CadexSession import is_provider_safe_tool
    ...
    def _waypoint_summary(waypoint: Any, index: int) -> dict[str, Any]:
        ...
        if placement is not None:
            item["placement"] = CadexService._placement_summary(placement)
        return item
    ...
    def _part_objects(self) -> list[Any]:
        ...
        return [
            ...
            and (
                ...
                or str(getattr(obj, "CadexScriptedRole", "") or "")
                == "publication"
            )
        ]
...
def get_service() -> CadexService:
    global _service
    if _service is None:
        _service = CadexService()
    return _service
