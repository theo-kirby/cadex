...
1. **Correct stale orientation snapshots (mission 1, standing maintenance).** In .hypergraph/AGENTS.md, replace the obsolete “Today” frontier sentence (file lifecycle broken, RL blocked on a GPU checkout) and “Ten nodes today” count with durable references to generated STATE.md. Verify against the current projection and cited work records; remove duplicated transient claims rather than introducing new counts. Keep this one narrow prose correction, with its required ADR note, real state-impact record and applicable ROADMAP convention; do not invent a completed feature checkbox. Run diff/documentation and graph checks, explicitly reporting no runtime gate rerun for prose-only work. No graph protocol rewrite or broad documentation audit is selected [rec: first-wing-3387] [rec: twilight-key-7506] [rec: careful-sand-9714].
2. **Conditional response to a concrete review finding (mission 1).** Dispatch only for a critic must-fix or source evidence contradicting rank 1. Resolve that exact finding with the touched zone's gate; a runtime change requires a new recorded scope decision. If accepted cleanly, this unit is not actionable: re-plan without manufacturing repairs, repeating the maintainer handoff or replaying the clean walk [rec: first-wing-3387] [rec: careful-sand-9714].

The previous recovery correction is accepted, its untriggered repair is withdrawn, and state reconciliation now includes the administrative dead end. No charter gap is retired. Signals: 23 iterations, 4.1h elapsed, 10.9h left. Select only this bounded subtractive direction; keep GUI unlaunched, remote/GPU/provisioning idle and Later criteria parked; at most one full build per unit if its zone requires it [rec: twilight-key-7506] [rec: careful-sand-9714] [rec: first-wing-3387].
