...
1. **Preserve completed lifecycle and review criteria (missions 1, 2 and 6).** Four fresh walks — the hinged arm, the carriage, the pan-tilt head on two catalog MG90S, and the pan-tilt rerun through the refreshed installed bundle — and cold persistence remain working; the installed bundle carries both clearance frame fixes with the assembly worker byte-identical to source and passes the packaged lifecycle gate 15; clearance, inventory, section and render agree on a five-component catalog-placed assembly, with a negative control that has power. The engine-selection guidance states only the verified schema check. No packaging repair, training replay or same-mechanism repeat is selected [rec: first-branch-9614] [rec: quiet-vine-3426] [rec: fair-cedar-7455] [rec: strong-raven-3067] [rec: empty-banner-7438] [rec: light-timber-5868] [rec: southern-otter-5999] [rec: morning-summit-7848].
2. **Catalog-part integrity in review (missions 6 and 4).** Short holds the inventory signal naming catalog outputs the design fused rather than placed. When it lands, the follow-on is one guidance unit: the convention that catalog parts are placed as components and never fused into printed solids, stated where the walk's design turn reads it and pinned the way the ADR-200 scaffold wording is, then one ladder-rule walk to see whether the count drops. If the signal cannot see fused parts because they are not published outputs, the follow-on is instead the part program's dependency graph, its own unit under its own bet. Swept clearance over the rollout trace into the walk's review stays unselected: several units of new runtime surface, and the review criterion as worded is met at the initial pose [rec: forest-hollow-9339] [rec: morning-summit-7848] [rec: wild-marsh-9611].
3. **The clearance direction is closed (missions 1, 2 and 6).** ADR-241 and ADR-242 ship in the installed bundle; the review surface no longer publishes a number that is false for catalog parts at the initial pose or along a traced sweep. The three real contacts on the first pan-tilt project remain that project's design business and are not selected: the iterate criterion is ticked and a design turn there closes no criterion [rec: southern-otter-5999] [rec: morning-summit-7848] [rec: light-timber-5868].

No new charter gaps, feature checkboxes or Later promotions are selected. The payload-identity guidance direction is exhausted; no broader documentation audit follows. The refreshed application remains a local unrelocated deployment; GUI-attached lifecycle and remote execution remain unexercised. Policy witness success is toy execution evidence, review clearance is initial-pose, the swept check runs only under a script's own guard, and sections are tessellated [rec: strong-raven-3067] [rec: modest-summit-8554] [rec: keen-pebble-3574] [rec: southern-otter-5999].
