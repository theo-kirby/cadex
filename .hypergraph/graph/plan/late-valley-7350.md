...
This machine is a new baseline and the earlier walk measurements were taken on a different one. The first walk here — `cadex walk --prompt` into a durable project outside this repository, `--model claude-opus-5`, `JAX_PLATFORMS=cpu`, one iteration / four envs / seed 0 / timeout 600 — ran 1:06:25 at 318,556 KiB peak RSS and exited 3 at the train leg, with the design leg alone taking 3,983.3 s. Treat the nt3 wall-clock and RSS figures above as another machine's observations until a clean walk here supplies its own [rec: open-hollow-2140].

The thing to prove, standing: that `assembly.mjcf` returns for a rig the design agent actually produces. A grounded one-body model accepts in 0 s and the reference `linear-carriage` in 0.55 s, so the export is not slow in general — but a ten-component catalog rig with per-component collisions, an actuator, observations, sensors, disturbances and a reward burns 300 s of CPU and dies by SIGXCPU. Until that holds, the north star's unattended prompt cannot reach its training leg at all [rec: open-hollow-2140] [rec: ancient-key-7299].

## Negative knowledge

- [scope: cross-machine measurement | confidence: high | evidence: open-hollow-2140] The walk timings, peak-RSS figures and bundle observations recorded below and above were taken on the previous machine. They are not this machine's baseline and must not be cited as one; the only measurement taken here is a walk that exited 3. Equally, this machine's single failure does not retract them [rec: open-hollow-2140] [rec: ancient-key-7299].
