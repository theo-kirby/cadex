...
1. **Deliver named-plane section views, then wire them into the walk (missions 6 and 2; damp-moon-9297).** Promote only after the current short units land or block. Reuse the delivered CPU rendering path. One CLI call takes a named axis plane and offset and writes a project-relative section artifact of the accepted revision. Establish the smallest correct geometry contract: exact engine-side sections if available through the existing contract, or explicitly labeled tessellation sections with tested clipping and closed contours/caps; never present a projected silhouette as a section or an approximate mesh cut as exact BREP. Preserve placements, distinguish empty cuts from errors, and test interior/cavity geometry and plane offsets. Delivery and walk wiring are separate units, each with relevant gates; wiring updates review, project commits, scaffold and mode docs with real-recipe and stand-in parity. The probe establishes reusable projection, not section correctness [rec: zesty-aspen-6846] [rec: mellow-beacon-8815].
2. **Evidence the complete headless-review criterion (missions 6 and 2; damp-moon-9297).** After render and section calls and wiring land, run the same documented walk on both established mechanisms. Verify named-angle images, named-plane section, catalog-id inventory and named-pair clearance are used by review and recorded in both projects. Report comparable numbers, inspection of visual contents, costs, approximation and remaining omissions. Only work impact declarations and maintainer reconciliation can close the gap. Initial-pose clearance is not swept-motion coverage; neither the two-component probe nor current walks qualify large assemblies [rec: clear-ash-2884] [rec: zesty-aspen-6846] [rec: mellow-beacon-8815].

CPU render delivery and wiring move to short because clearance and the route probe landed; the empty-short walk rerun precedes them. Three-mode parity is evidenced under documented-only GUI and remote constraints; update its artifact contract as review grows. The ordinary local bundle needs refresh before any bundled verification claim, but that is not a prerequisite to independent CLI rendering and does not authorize a GUI launch [rec: gilded-basin-9946] [rec: clear-ash-2884] [rec: zesty-aspen-6846] [rec: mellow-beacon-8815].

Parked catalog, fleet, outside-source, report and reduction criteria remain outside dispatch. No new direction or charter-gap retirement is proposed [rec: modest-summit-8554] [rec: mellow-beacon-8815].

## Negative knowledge

- [scope: CPU render probe limits | confidence: high | evidence: zesty-aspen-6846] Valid Blender blueprint and render_views calls refuse background mode; exit 0 observed returned errors, not rendered PNGs. The ordinary bundle has a stale library_catalog_identity import; the staged-engine override hydrates correctly but does not qualify the ordinary bundle. CPU SVG probe proves solved placement on 24 triangles only. Centroid painter ordering, exposed diagonals, ephemeral rebuild buffer paths, curves and large assemblies require product qualification; no GUI, new dependencies or replacement shell follows [rec: zesty-aspen-6846].
