...
1. **Disable only the deleted GUI translator writer paths (mission 3; round-glacier-2865, green-sea-3991).** One commit: in src/Tools/updatecrowdin.py remove apply/install's updateTranslatorCpp tail and replace updateTranslator dispatch with an explicit retired-command diagnostic without a progress request; retain the helper and PySide import until rank 2. Preserve credential/project startup, every other command, locations, mappings, App/Base copy/archive/QRC behavior and all retained Qt/resources. Add isolated regression coverage in src/Mod/cadex/cadex_tests/test_translation_updater.py; stub imports, credentials, service and filesystem effects before dispatch, with no live network or repository resource writes. Supporting scope is the audit, ADR, ROADMAP, FREECAD ledger, existing notice/manifest if accuracy requires, and one work record. Run isolated retired/retained dispatch, App/Base copy, Swedish mapping and synthetic archive/QRC checks, full engine pytest, committed-HEAD licensing/manifest checks, at most one release build, baseline-compared inherited CTest, unchanged App/Base TS hashes and generated QM/QRC checks, and installed FreeCAD.Qt load/translate/remove. If payload/staging changes, finish staging before the fresh packaged lifecycle gate; label existing-stage evidence and omissions honestly. External GUI-registration users lose that side effect; external use is unknown. No other inherited source, build rule, shell change or updater cleanup is authorized. [rec: eager-garden-8009] [rec: cold-clover-8123] [rec: damp-sand-1115]
2. **Delete only the verified-disabled GUI writer and exclusive import (mission 3; round-glacier-2865, green-sea-3991).** A separate commit, dispatched conditionally after rank 1's committed record and passing gates: remove updateTranslatorCpp and its exclusive PySide6/PySide2 import block from the same source file; retain the retired-command diagnostic, startup and all non-GUI behavior. Direct external helper imports cease to work; no temporary production scaffolding or whole-tool deletion is authorized. Use the same supporting-file boundary and per-stage verification as rank 1, including isolated regressions, full engine suite, committed-HEAD licensing/manifest, at most one release build, baseline CTest, retained translation hashes/resources/runtime probe, and a fresh packaged gate if staging changes. Record ADR/ROADMAP/ledger evidence and real impacts; report surviving M file/inserted/deleted totals separately, with no whole-file saving. Incomplete disable evidence holds this unit; finish verification or record the blocker before any deletion. Replan after the pair instead of expanding scope or restarting stopped catalog searches. [rec: eager-garden-8009] [rec: cold-clover-8123] [rec: damp-sand-1115]
