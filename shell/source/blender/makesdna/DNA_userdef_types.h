...
struct UserDef {
   ...
   *
   * Mesh ships "Mesh" as the default (cadex ADR-024): the chat-driven
   * modelling template is the product, and a new user should meet it
   * without having to find it in a menu. Only a *fresh* profile takes this
   * default -- an existing userpref.blend keeps whatever it stored -- and
   * `--app-template default` still escapes to stock Blender.
   *
   * This is a modified upstream file; see docs/mesh/UPSTREAM_DIFFS.md.
   */
  char app_template[64] = "Mesh";
