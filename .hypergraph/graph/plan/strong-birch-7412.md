...
1. **Remaining lifecycle context maintenance (missions 1/2).** The finite seed measurement is spent. Correct the source-demonstrated loss of recent decisions at the prompt limit, then the existing live history iterate only when supplied capacity permits. The resumed-context regression proves delivery/persistence, not completeness of overflowing logs or provider understanding; the new bounded regression addresses the former only. No fresh mechanism, extra seeds, rerun of completed regressions as a separate unit, or general context system follows [rec: restless-cabin-9629] [rec: humble-sky-8445] [rec: honest-banner-0821].

2. **Delivered work leaves dispatch.** Seeds 0–3 completed with fixed exported model/task and objective/action identities, training seeds varied and rollout seed 7 held. Reward range 170.952826823611–175.935971673601; translation 30.078469436328–31.421759939733 mm; rotation zero. Every review succeeded and prior artifacts/history survived. History updates now replace complete files after successful writing; six failure/retry tests and full CLI gate 251 passed, no skips. Protection is per-file, not concurrency, multi-document or power-loss safety. No additional failure-hardening campaign follows [rec: honest-banner-0821] [rec: solar-snow-8543].

3. **Keep comparison and retention limits.** Four-seed ranges are descriptive; the 30 mm action midpoint remains a confound and neither matching objectives nor favorable travel proves equivalent control difficulty or learned improvement. Root review exclusions and policy exclusions after default negations govern untracked files; existing tracked/staged paths retain ordinary Git semantics. Legacy identity rows remain unavailable and numeric comparison references may differ from same-kind identity references. No automatic migration, untracking or standalone-rollout reporting work follows [rec: honest-banner-0821] [rec: easy-beacon-3803] [rec: modest-prairie-3356].
