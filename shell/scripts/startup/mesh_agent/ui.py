...
class MESH_AGENT_OT_chat_copy(Operator):
    """Copy one transcript message to the system clipboard.

    Blender's label widget cannot be text-selected -- that is the widget,
    not a setting -- so the transcript's text is reachable the way the rest
    of the UI reaches text: an operator that writes the clipboard. One
    button per message beats one per line, because what a person pastes is
    the message.
    """

    bl_idname = "mesh_agent.chat_copy"
    bl_label = "Copy Message"
    bl_description = "Copy this message's text to the clipboard"
    bl_options = {'INTERNAL'}

    index: bpy.props.IntProperty(default=-1, options={'HIDDEN'})

    def execute(self, context):
        messages = agent_module.get_agent().history.messages
        if not 0 <= self.index < len(messages):
            return {'CANCELLED'}
        context.window_manager.clipboard = messages[self.index].text
        self.report({'INFO'}, "Message copied")
        return {'FINISHED'}


class MESH_AGENT_OT_chat_toggle_tools(Operator):
    """Expand or collapse the transcript's tool-call runs.

    One WindowManager flag rather than per-run state: what the user decides
    is "do I want to watch the tool calls", and that answer does not change
    per run. The flag is session state, deliberately not saved in the file.
    """

    bl_idname = "mesh_agent.chat_toggle_tools"
    bl_label = "Tool Calls"
    bl_description = "Show or hide the individual tool calls"
    bl_options = {'INTERNAL'}

    def execute(self, context):
        wm = context.window_manager
        wm.mesh_chat_tools_expanded = not wm.mesh_chat_tools_expanded
...
#: How a tool call announces itself in the transcript (agent._on_stream).
_TOOL_PREFIX = "· "


def _transcript_groups(messages):
    """The display list: messages, with consecutive tool calls in one group.

    Twenty tool calls in a row are one thing that happened -- the assistant
    working -- and drawing them as twenty rows buries the conversation.
    Grouping happens *before* the draw loop trims the transcript, so a long
    tool run no longer eats the visible window either.

    Yields ``("tools", [(index, text), ...])`` for a run of tool-call status
    rows and ``("message", index, message)`` for everything else. Empty
    assistant messages (the streaming placeholder before any text arrives)
    are dropped: an empty box says nothing, and it would split a tool run
    in two.
    """
    groups = []
    for index, message in enumerate(messages):
        if message.role == "status" and message.text.startswith(_TOOL_PREFIX):
            name = message.text[len(_TOOL_PREFIX):]
            if groups and groups[-1][0] == "tools":
                groups[-1][1].append((index, name))
            else:
                groups.append(("tools", [(index, name)]))
            continue
        if message.role == "assistant" and not message.text:
            continue
        groups.append(("message", index, message))
    return groups
...
def _draw_interface(layout, context):
    ...
    # The blueprint settings box is gone (ADR-179): the toggle above is the
    # whole viewport surface — a look, with the agent's blueprint_view tool
    # setting theme and grid — and drawings live in the Blueprint Editor.
...
class CADEX_CHAT_PT_transcript(Panel):
    ...
    def draw(self, context):
        ...
        # AI section of the Preferences window, which is where the fix is
        # applied (the header's gear opens it).
        ...
        # minus panel padding and the copy button). blf-measured wrapping can
        # come later.
        wrap = max(24, int(context.region.width / 7.2) - 10)

        expanded = bool(getattr(context.window_manager,
                                "mesh_chat_tools_expanded", False))

        for group in _transcript_groups(agent.history.messages)[-40:]:
            if group[0] == "tools":
                calls = group[1]
                if len(calls) > 1:
                    # The run in one clickable row: how many, and what ran
                    # last -- which, mid-turn, is what is running now.
                    header = layout.row()
                    header.emboss = 'NONE'
                    header.operator(
                        MESH_AGENT_OT_chat_toggle_tools.bl_idname,
                        text="{:d} tool calls — {:s}".format(
                            len(calls), calls[-1][1]),
                        icon=('DISCLOSURE_TRI_DOWN' if expanded
                              else 'DISCLOSURE_TRI_RIGHT'))
                if len(calls) == 1 or expanded:
                    column = layout.column(align=True)
                    column.enabled = False
                    for _index, name in calls:
                        column.label(text=_TOOL_PREFIX + name, icon='TOOL_SETTINGS')
                continue

            _kind, index, message = group
            ...
            # The copy button, top-right of the box. Emboss off so twenty
            # messages do not read as twenty buttons.
            copy = row.column()
            copy.emboss = 'NONE'
            copy.operator(MESH_AGENT_OT_chat_copy.bl_idname,
                          text="", icon='COPYDOWN').index = index
...
classes = (
    ...
    MESH_AGENT_OT_chat_copy,
    MESH_AGENT_OT_chat_toggle_tools,
...
def register():
    ...
    # Session state, not file state: whether the transcript shows every tool
    # call or one row per run. On the WindowManager so it survives loading a
    # file but never saves into one.
    bpy.types.WindowManager.mesh_chat_tools_expanded = bpy.props.BoolProperty(
        name="Show Tool Calls",
        description="Show every tool call instead of one row per run",
        default=False,
    )
    for cls in classes:
...
def unregister():
    ...
    del bpy.types.WindowManager.mesh_chat_tools_expanded
