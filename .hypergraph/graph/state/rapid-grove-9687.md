...
**F7 has been dispatched twice on `claude-opus-5`, and its create prompt is live right now on `ot7-plover-d` under a raised turn bound. No slot has been spent and all four are still held [rec: strong-hollow-7483] [rec: cool-grotto-2512].**

*The first Opus dispatch produced geometry and ran out of clock.* A window probe read `allowed` at 14 % of the five-hour window and 2 % of the seven-day, so the frozen `plover.create.prompt.txt` (digest checked against the freeze by the runner) went into a fresh `ot7-plover-c` on `claude-opus-5`. The call **reached the model** and ran the full **1800.0 s** bound — 124 frames, 49 tool calls (4 `describe_api`, 30 `inspect`, 10 `write_script`, 4 `edit_script`, 1 `rebuild`), 1.29 MB of transcript, **zero actor design edits** — and was killed at the bound while reading its own measured fit back through `inspect scope=clearance`. Under ADR-356 that is an **interruption**: no slot spent, retry in a fresh project. Two deviations from the runner default are recorded as deliberate: `--turns 1` (a kill part-way through a four-turn schedule discards every completed turn with it, because the retry must go to a fresh project) and launching under `setsid` so the turn outlives the dispatching actor. Receipt `docs/probes/ot7/attempts/plover-c-interrupted.json`, commit `afd3b773` [rec: strong-hollow-7483].
...
Reconcile judgement: **`open`, and now for an ordinary reason.** The org-level provider block is gone and is not reproduced; the turn-bound obstruction is measured and raised; what stands between F7 and a result is one live turn nobody has collected yet. Done may not be claimed while F7 is unattempted, by the resume directive's own closing rule [rec: gentle-sand-0141], and an interrupted turn is not an attempt [rec: strong-hollow-7483].
...
- strong-hollow-7483 — F7's first Opus dispatch: the frozen create prompt reached the model on `ot7-plover-c`, ran the full 1,800 s bound across 49 tool calls and was killed mid-repair; an ADR-356 interruption at 0 slots, with 13/435 static and 52 swept failures measured on an unaccepted 30-component biped, naming the turn bound as the binding constraint
- cool-grotto-2512 — ADR-388 raised the bound to 3,600 s and finalised `ot7-plover-b` as a `runner_died` interruption at 0 slots; F7's create prompt is live on `ot7-plover-d` on Opus, uncollected, with the Opus window cost measured at ~17 points per half hour
