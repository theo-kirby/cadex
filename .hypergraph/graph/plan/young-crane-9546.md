...
1. **Rehearse the documented provider-free walk once (missions 1, 2 and 6).** Both previous units landed: restore bookkeeping is required and ADR-247 removes only redundant session identity writes, with 206 CLI passes and zero skips. Start a new isolated project from the documented plate-and-arm example in `examples/lifecycle/`, through the ordinary script acceptance command, and run the documented `cadex walk` without `--prompt` once. Use the existing qualified local stage and CPU training venv, one iteration / four envs / seed 0 / trainer timeout 600 s, with an external 850 s and 2.9 GiB process-tree guard. Derive exact flags from the example and docs/CLI.md; no provider call, fake design turn, stage refresh or durable nt3-leg mutation. Inspect persisted PROGRESS rows and commit subjects, portable output labels (use an absolute in-project --out), policy witness and rollout numbers, inventory and named offending clearance pairs, four named render views and the XZ section. Report real leg failures, elapsed/peak measurements and ignored versus tracked artifact paths; never commit checkpoints or rollouts into this repository. This is downstream maintenance of the walk/headless-review criteria, not new agent-design or resumed-session evidence. Record the first concrete failing leg or the clean result, then stop; do not repeat this rehearsal automatically [rec: vast-ledge-4610] [rec: ready-otter-2700] [rec: cool-garden-5811].

2. **Correct only a defect demonstrated by that rehearsal (missions 1, 2 and 6; conditional).** If unit 1 identifies a reproducible defect in the existing lifecycle/review contract with a narrow safe correction, fix that one defect and verify the failing case plus legitimate success behavior. Preserve project user work and truthful acceptance/session state. CLI edits require the full CLI suite; engine Python requires the full engine suite; protocol/payload or shell edits require their prescribed gates, with at most one full build. Update walk documentation and scaffold together for behavior changes, ADR and ROADMAP where required. If the rehearsal is clean, or no safe bounded correction is established, this unit is not dispatchable: return the finding for replanning without inventing a defect, rerunning the provider-free baseline, probing availability or retrying the live repair. No new review capability, catalog identity system or worker campaign follows [rec: ready-otter-2700] [rec: cool-garden-5811].

Signals: 189 iterations, 11.3h elapsed, 3.7h left, $85.47 API-equivalent; Claude five_hour 65%, seven_day 54%; Codex seven_day 37%. Both refusal-metadata units are complete. The actual live repair remains parked under controller-owned scheduling after the usage-credit refusal. No automatic retry, timed eligibility probe, repeated hold record, provider switch, paid fallback, GUI or remote execution. Zero new product directions and no Later promotion or charter-gap retirement; this is one finite standing-maintenance walk selected from three considered options [rec: sunny-walrus-5847] [rec: ready-otter-2700] [rec: cool-garden-5811].
