...
**N20 interfaces independently verified:** source and staged actual workers measure shaft diameter 3 mm, flat-to-opposite 2.5 mm, flat axial extent Z=1..10 mm, and mounting bores of radius 0.8 mm at X=±4.5, Y=0, Z=-1..0 mm. Canonical and rotated/translated instances pass 160 material/void probes, valid-single-solid and equal-volume checks. Full engine: 2023 passed / 52 skipped; fresh packaged lifecycle/library: 91 passed, no skips. The assumed 1 mm blind-bore depth and sharp flat transition remain approximations. The local 2.4 GB stage has 144 relocation violations and is not shippable [rec: windy-lily-2895].

**RI50 qualification parked (ADR-223):** the manufacturer source audit and fixture follow-up add evidence in `docs/L3-COVERAGE.md`, with source URLs and hashes, but no catalog entry. Inspected ratings, fixture BOM/STEP headers and support text establish neither RI50-specific cooling, winding temperature and test duration/duty nor linkage between the rating sheet and no-Hall drawing revision. Geometry proof and delivery remain gated [rec: slender-harbor-9625] [rec: shy-hill-8139]. The fixture's populated sheet lists a pivot, rear cover, two MR148 bearings and six M2.5x4 screws; the other two sheets are empty [rec: slender-harbor-9625] [rec: shy-hill-8139].

**AT2814 alternative qualification blocked (ADR-223):** the T-MOTOR AT2814 Long Shaft KV900 bench report and V2.0 drawing were audited in `docs/L3-COVERAGE.md`. The selected row reports 0.291 N m at 7601 rpm, 11.05 V and 27.02 A, with 68 C surface temperature after three minutes; ambient, current definition, cooling and test-to-drawing revision linkage remain unresolved. No winding qualified and no geometry proof or catalog delivery followed. The bounded motor-search sequence stops [rec: mild-harvest-8460].

**Fifth-servo source audit stopped (ADR-229):** `docs/FIFTH-SERVO-AUDIT.md` pins manufacturer documents, hashes and interface blockers for HS-311 and HS-422. Neither qualifies for unchanged ServoPart: open mounting mouths cannot be reproduced by the stated circular drills; output-stack qualification is incomplete and HS-422 dimension labels conflict. Four servo rows remain. Existing local-stage lifecycle/library tests pass **91, no skips** [rec: hidden-ridge-7342].

**Manufacturer STEP accessory audit stopped (ADR-231):** no catalog identities or vendor assets were added. Neither horn nor pigtail qualified; source, fit and rights blockers are tracked in the manufacturer-source gap. The inspected mesh and linked-part imports do not establish the assumed script-owned raw STEP delivery path [rec: steady-reef-0162].

**Gears family shipped (ADR-233, ADR-234):** `lib.spur_gear` and `lib.rack` over `CadexCatalog.gear_spec` (ISO 53 profile constants, ISO 54 series 1 modules) share one involute generator, verified on the source suite, a rebuilt engine and a fresh staged payload (2050 passed / 52 skipped; packaged lifecycle/library 107, no skips) [rec: wild-beacon-4213]. `lib.rack_and_pinion` composes them over `CadexCatalog.rack_and_pinion_spec` as a two-solid compound placed like every other library value, with `bore`, `rack_height` and `rotation_degrees` keywords; the composition lives in the `gears` family notes, so the `describe_api` golden did not move. Fresh packaged lifecycle/library gate: 119 passed, no skips [rec: mellow-garden-0940].

**Planetary gearbox: not published; the first internal-ring composition failed its mesh (ADR-235).** The existing spur generator, used to cut an internal ring's tooth spaces with a virtual external gear, gives correct 0.25 mm root-circle clearances but an overlapping planet–ring mesh (0.000354806 mm³ at phase 0 against a 1e-6 bound; sun–planet is clean). A standalone probe under `docs/experiments/` reproduces the number without the uncommitted patch; PROVENANCE §8g, ROADMAP and L3-COVERAGE carry the notes. The uncommitted planetary proposal (three files) and its pre-existing `test_planetary_spec_numbers` failure remain unqualified and must be resolved explicitly before any resumption; nothing in the payload changed. Compound-mechanism progress is tracked on idle-tower-1624 (parked) [rec: proud-cliff-9629].

**Library values carry their catalog identity downstream (ADR-236, commit 5045847a)** [rec: fair-rose-5950]. `lib.bolt("M3", 12)` returns a `LibraryPart` that knows its family and part number, but `.body` is an ordinary part solid, so a script returning it published an anonymous solid and the identity died at the result contract. `cadex_library_api` now records `{canonical definition: {family, part_number}}` for every body a generator hands back during a run (cleared at each `create_library_api` staging), and `cadex_project_worker._stamp_catalog_identity` writes it onto the matching output as a top-level `catalog` key, so a `lib.*` part is identifiable in the accepted report. The stamp is written *beside* the definition with the canonical definition as the join key, never inside `properties`: `compute_project_digest` hashes `definition`, and a key there would have moved every existing `lib.*` project's digest and forced the ADR-064 class of re-accept. A test asserts the canonical definition is byte-identical across the stamp and that the digest is still. This is the library half of the assembly inventory tracked on `damp-moon-9297`; engine suite 2070 passed / 52 skipped, packaged lifecycle gate 15 passed [rec: fair-rose-5950].

**A catalog part's running fit is now declarable from the agent's own instructions (ADR-373) [rec: fierce-falcon-2378].** A bearing seat between a printed part and a `lib.bearing` body is a gap the design means, not an oversight: at 0.05 mm it sits under the engine's 0.1 mm undeclared-pair default and read `below clearance`, and the prompt's only advice for a failing pair was to fix the geometry — which for this pair means widening a seat that was right. The declaration is `clearances=[(a, b, 0.05)]`, and it now appears in `CLI_OVERLAY` rather than only in `docs/XSCRIPT.md`, together with the reason `contacts=` cannot say it (a contact intent holds the pair to 0.001 mm, so it renames the failure rather than clearing it). Nothing in the catalog itself changed, and no retained design was re-declared: Finch's four bearing seats still read `below clearance` until a design turn declares them [rec: fierce-falcon-2378].
...
- [scope: the ADR-235 internal-ring cut from the spur generator, one configuration at phase 0 | confidence: high | evidence: proud-cliff-9629] Reproducible planet–ring overlap of 3.5e-4 mm³ with correct root clearances. Chordal cutout error is a candidate cause, not a diagnosis; no other phase, tooth count or module was measured, so the approach is not disproved in general.
...
- proud-cliff-9629 — ADR-235: the internal-ring composition fails its planet–ring mesh, reproduced by a standalone probe; no planetary published, proposal preserved unqualified
