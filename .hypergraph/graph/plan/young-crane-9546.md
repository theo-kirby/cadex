...
1. **Teach the walk to place catalog hardware as components (missions 2 and 6).** The removal is verified; continue the existing guidance unit: the walk's design instructions and project scaffold explain that purchased hardware is placed as assembly components, separate from printed solids; transformed catalog bodies may serve as clearance cutters without implying another purchased part. Keep the convention concise and aligned with the existing engine API, with no new tool, registry, validator or dependency. Pin the observable prompt/scaffold contract in the existing tests, update the walk documentation and its date in the same commit, and record the direction in DECISIONS and a landed ROADMAP item where required. Run the full CLI suite against a built engine; run any other zone gate only if that zone is touched. This is guidance, not proof an agent followed it and not a claim that discarded generator calls are missing hardware [rec: forest-hollow-9339] [rec: morning-summit-7848] [rec: odd-ridge-9607] [rec: staid-willow-6557]. [rec: windy-dune-3488] [rec: modest-valley-3313]
2. **Retry the two-servo leg once quota permits (missions 2 and 6, the ladder rule).** The prior attempt reached no accepted design. Dispatch only after the provider's reported 08:30 Europe/Madrid reset or affirmative availability evidence; no repeated provider probes, paid fallback or provider switch. If still blocked when eligible, defer this unit and report the blocker rather than waiting in an actor. After rank 1 lands, run the same documented walk and mechanism prompt from silent-mist-5233 in a fresh project outside the checkout, with the corrected CLI and an explicitly identified engine carrying the correction (prefer windy-dune-3488's fresh payload; do not claim the older installed bundle contains it). Use the training venv, JAX_PLATFORMS=cpu, --iterations 1 --envs 4 --seed 0 --timeout 600, and the existing process-tree guard at 2.9 GB and 850 s. Report every leg as clean, needing a guess/person, or not reached, with exact errors. Inspect all four views and section, compare script catalog usage with placed inventory manually, and use the bound-agreement checker if catalog rows exist. No source correction in the rehearsal; failures name the next unit. No absence-of-tally value is evidence that no catalog part was fused. Record crisp-reef-5607 and swift-dusk-2951 evidence, and damp-moon-9297 only for review findings [rec: silent-mist-5233] [rec: morning-summit-7848] [rec: staid-willow-6557]. [rec: windy-dune-3488] [rec: modest-valley-3313]
3. **Diagnose stale worker-cache identity in isolation (missions 1 and 2).** One bounded evidence unit, executable before rank 2 while provider quota blocks it. Trace cache naming, hardlink creation/reuse and build/install writes involved in windy-dune-3488. In a disposable external scratch root using copies of the affected inputs, attempt to reproduce bytes changing or stale bytes being reused under an existing content-addressed name. Record hashes, inode/link relationships, exact operations and whether the supported build/install path can cause it; distinguish observation from a hypothesized cause. Use a minimal worker/rebuild probe if needed, without provider dispatch or training. Do not mutate the real shared cache, accepted projects, source, payload or installed bundle. Stop after one reproduction or a documented non-reproduction; identify the smallest corrective boundary for a later bet, with no cache redesign or fix in this unit. Record impact on early-arbor-7123 and forest-wind-0342 only as supported; quarantine is not a product fix. No full build is required for this scratch diagnosis [rec: windy-dune-3488] [rec: modest-valley-3313].

Signals: 39 iterations, 7.1h elapsed, 7.9h left; Claude five_hour 75%, seven_day 47%, Codex seven_day 6%. Removal is complete: engine 2078 passed/52 skipped, CLI 195 passed/no skips, fresh packaged lifecycle/inventory 24 passed. Finish staging before payload-reading suites. The installed bundle remains stale. No repeated provider probes, actor waiting, charter changes or Later promotion; replan from the selected units' evidence [rec: windy-dune-3488] [rec: silent-mist-5233] [rec: modest-valley-3313].
