...
p = params(
    ...
    ankle_h=num(40.0, unit="mm", min=26.0, max=52.0, step=0.5,
    ...
    thigh_len=num(58.0, unit="mm", min=40.0, max=95.0, step=1.0,
                  label="Thigh length"),
    hip_gap=num(30.0, unit="mm", min=16.0, max=42.0, step=0.5,
    ...
    plate_r=num(11.0, unit="mm", min=0.0, max=16.0, step=0.5,
                label="Plate corner radius"),
    joint_clear=num(20.0, unit="mm", min=14.0, max=30.0, step=0.5,
                    label="Clearance kept round a joint"),
...
FL_Z = -6.5
CX = -SH_X + SB_L / 2.0
# A servo bolts its flange DOWN onto a wall, so the wall it mounts to
# lives between MB and MB + wall thickness away from the shaft plane.
MB = -FL_Z
HW = 11.5   # half width of a limb plate, in X
EO = 11.0   # how far a plate reaches past the axis it turns about
...
def rounded(shape):
    # Only safe on bodies with no pre-rounded plates in them: OCC will
    # not blend a solid that already carries fillet faces.
    return part.fillet(shape, RR, edges="all", on_failure="skip")
...
def servo_group(pos, rots, half_y):
    ...
    wall = place(part.box(FL_L + 2.0, 2.0 * half_y, LT,
                          origin=[CX - FL_L / 2.0 - 1.0, -half_y,
                                  FL_Z - LT]),
                 rots, pos)
...
def build_leg(sgn, tag):
    ...
    sole = rbox(-p.foot_len * 0.35, p.foot_len * 0.65,
                -p.foot_w / 2.0, p.foot_w / 2.0, 0.0, p.foot_t, sgn)
    riser = rbox(FL_Z - LT, FL_Z, -11.7, 22.7, 0.0, RH - 6.0, sgn)
    pad = rbox(-24.0, -9.0, -8.0, 19.0, p.foot_t, RH - 7.5, sgn)
    foot = part.compound(
        [part.cut(part.fuse([sole, riser, pad, wl_ar]), tl_ar), sv_ar])
    ...
    # wide below the pitch servo, then a narrow strip beside its
    # mounting wall, so the bracket never crosses the servo flange
    gus_a = rbox(6.0, 6.0 + T, -MB - LT, 3.0, RH, ZA - 7.0, sgn)
    gus_b = rbox(6.0, 6.0 + T, -MB - LT, -MB, RH, ZA + 6.0, sgn)
    ankle = part.compound(
        [part.cut(part.fuse([hub_ar, gus_a, gus_b, wl_ap]),
                  tl_ap + th_ar), sv_ap])
    ...
    sh_out = rbox(-HW, HW, MB, MB + LT, ZA - EO, ZK + EO, sgn)
    sh_in = rbox(-HW, HW, -LW, -LW + LT, ZA + CLR, ZK + EO, sgn)
    sh_parts = [hub_ap, sh_out, sh_in, wl_kn]
    if ZK - CLR > ZA + CLR + 5.0:
        sh_parts.append(rbox(-HW, -HW + LT, -LW, MB + LT,
                             ZA + CLR, ZK - CLR, sgn))
    shin = part.compound(
        [part.cut(part.fuse(sh_parts), tl_kn + th_ap), sv_kn])
    ...
    tg_in = rbox(-HW, HW, -MB - LT, -MB, ZK - EO, ZHP + EO, sgn)
    tg_out = rbox(-HW, HW, LW - LT, LW, ZK + CLR, ZHP + EO, sgn)
    tg_parts = [hub_kn, tg_in, tg_out, wl_hp]
    if ZHP - CLR - 2.0 > ZK + CLR + 5.0:
        tg_parts.append(rbox(-HW, -HW + LT, -MB - LT, LW,
                             ZK + CLR, ZHP - CLR - 2.0, sgn))
    thigh = part.compound(
        [part.cut(part.fuse(tg_parts), tl_hp + th_kn), sv_hp])

    # hip bracket: pitch horn below, hip roll servo above
    hub_hp, th_hp = hub_group([0.0, y, ZHP], po)
    sv_hr, wl_hr, tl_hr = servo_group([0.0, y, ZHR], rf, 9.0)
    # stops short of x = 4 so it never fouls the torso's pelvis plate
    # or the roll horn turning inside it
    hip_plate = rbox(-HW, 3.5, 6.0, 6.0 + LT, ZHP - EO, ZHR + 9.0, sgn)
    hip = part.compound(
        [part.cut(part.fuse([hub_hp, hip_plate, wl_hr]),
                  tl_hr + th_hp), sv_hr])
...
TZ = ZHR + 20.0
# pelvis: a tapered yoke with an arch between the legs, carrying a
# raised socket round each hip roll axis
PY = HY + 14.0
PTW = p.torso_w / 2.0 - 6.0
pel_face = part.face(part.wire([[6.0, -PY, ZHR - 8.0],
                                [6.0, 0.0, ZHR + 6.0],
                                [6.0, PY, ZHR - 8.0],
                                [6.0, PTW, TZ + 2.0],
                                [6.0, -PTW, TZ + 2.0]], closed=True))
pelvis = part.fuse([
    part.extrude(pel_face, [T, 0.0, 0.0], output_type="solid"),
    part.cone(15.0, 12.0, 5.0, origin=[6.0, HY, ZHR],
...
def probe(a, b, nm):
    return part.fuse([a, b], output_type="compound", label=nm)


# temporary interference probes: volume(A) + volume(B) - volume(A + B)
u_fa = probe(WORLD["foot"], WORLD["ankle"], "u_fa")
u_as = probe(WORLD["ankle"], WORLD["shin"], "u_as")
u_st = probe(WORLD["shin"], WORLD["thigh"], "u_st")
u_th = probe(WORLD["thigh"], WORLD["hip"], "u_th")
u_ht = probe(WORLD["hip"], torso_print, "u_ht")
w_foot = part.transform(WORLD["foot"], label="w_foot")
w_ankle = part.transform(WORLD["ankle"], label="w_ankle")
w_shin = part.transform(WORLD["shin"], label="w_shin")
w_thigh = part.transform(WORLD["thigh"], label="w_thigh")
w_hip = part.transform(WORLD["hip"], label="w_hip")
w_torso = part.transform(torso_print, label="w_torso")
...
def leg_bodies(sgn, c_hip, ch, c_thigh, ct, c_shin, cs, c_ankle, ca,
    ...
    bt = assembly.body(c_thigh, density_kg_m3=DENS, collision=[
        coll_box(2.0 * HW + 4.0, LW + MB, ZHP - ZK,
                 [0.0, y + sgn * (LW - MB) / 2.0, (ZK + ZHP) / 2.0], ct)])
    bs = assembly.body(c_shin, density_kg_m3=DENS, collision=[
        coll_box(2.0 * HW + 4.0, LW + MB, ZK - ZA,
                 [0.0, y - sgn * (LW - MB) / 2.0, (ZA + ZK) / 2.0], cs)])
...
result = {
    ...
    "u_fa": u_fa, "u_as": u_as, "u_st": u_st, "u_th": u_th, "u_ht": u_ht,
    "w_foot": w_foot, "w_ankle": w_ankle, "w_shin": w_shin,
    "w_thigh": w_thigh, "w_hip": w_hip, "w_torso": w_torso,
