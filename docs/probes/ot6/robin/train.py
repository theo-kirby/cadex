"""One bounded real GPU training run on Robin, the two-wheeled balancer (D7,
ADR-328): a checkpoint video published while training is active, the
PERSISTENT operator dashboard observed (never a temporary server), renderer
overhead measured from the trainer's own committed iteration intervals, and a
retained final video -- both videos in the dark reference look (ADR-331,
ADR-332) and naming what they show (ADR-333).

This is the Finch driver (``../finch/train.py``) with two Robin facts: the
accepted view carries 24 components (five printed parts and nineteen catalog
parts, ADR-337) and the authorship default names Robin. The model, task and
policy outputs are still discovered from the CLI's own envelopes by kind, and
the observer and the video checker beside the Wren driver are reused by path.
Concurrent rendering is bounded to the one checkpoint video, rendered while
the trainer runs, and its impact is the before/during/after update intervals.

usage: pixi run python train.py PROJECT RUN URL [AUTHORED_BY] [LEARNING_RATE]   (PYTHONPATH=cli:cli/tests)
LEARNING_RATE (default: the trainer's own 3e-4) is recorded in the run's requested block and passed as --learning-rate.
...
authored_by = sys.argv[4] if len(sys.argv) > 4 else 'Robin, authored by the product agent from the D7 creation prompt; reset lift repaired (ADR-337) and the wheel D-bore revised as an analytic prism by the actor, both entered with cadex script --set; the balance task is declared in the same script'
learning_rate = float(sys.argv[5]) if len(sys.argv) > 5 else 3.0e-4   # the trainer's default; robin1 diverged at it
repo = Path.cwd(); ev = p / 'evidence'; run = p / 'runs' / name; train = run / 'train'
generic = Path(__file__).resolve().parent.parent.parent / 'wren-fresh'   # observe.py and check_video.py are project-agnostic
ev.mkdir(exist_ok=True); train.mkdir(parents=True, exist_ok=False)   # Robin has no evidence/ before this unit
...
model_xml = output_file(e, 'assembly_mjcf_xml', 'xml'); task_json = output_file(e, 'assembly_training_task_json', 'json')
...
geometry = {n.name: sha(n) for n in (model_xml, task_json)}
cli(name + '-training-render', 'render')
with project_lock(p):
    retain_training_view(p, run)
assert read(run / 'training-view.json')['model']['available']
components = len(read(run / 'training-view.json')['model']['components'])
assert components == 24, components   # five printed parts and nineteen catalog parts (ADR-337)
base = dict(project_root=p, mode='offboard-checkpoint-experiment', accepted_revision=e['accepted_revision'], digest=e['digest'], params=e['params'], param_specs=m['param_specs'], specs_source='successful CLI export', identity_source='successful params export envelope', requested={'iterations': 240, 'envs': 1024, 'seed': 0, 'checkpoint_every': 20, 'timeout_seconds': 3600, 'learning_rate': learning_rate, 'authored_by': authored_by}, task_bundle=task_json, task_sha256=sha(task_json), model_xml=model_xml)
write_run_record(run, status='running', snapshot_docs=True, **base)
unit = 'cadex-' + name
cmd = ['systemd-run', '--user', '--scope', '--unit=' + unit, '-p', 'MemoryMax=20G', 'timeout', '--signal=TERM', '--kill-after=20s', '3600', str(Path.home() / 'cadex-train-venv/bin/python'), 'training/cadex_train.py', str(task_json), '--out', str(train / (name + '.cxpolicy')), '--iterations', '240', '--envs', '1024', '--seed', '0', '--checkpoint-every', '20', '--learning-rate', repr(learning_rate)]
...
observer = subprocess.Popen(['pixi', 'run', 'python', str(generic / 'observe.py')], env=obsenv, stdout=obslog, stderr=subprocess.STDOUT)
...
def playback(policy, pname, active):
    ...
    script, substitutions = re.subn(r'weights="[^"]+",\s*sha256="[a-f0-9]+"', 'weights=' + json.dumps(policy.name) + ', sha256=' + json.dumps(digest), source)
    ...
    receipt_path = output_file(e, 'assembly_policy_receipt_json', 'json'); trace_path = r / 'rollout/assembly-simulation-trace.json'
    receipt = read(receipt_path); assert receipt['witness_error'] < receipt['witness_tolerance']
    t = read(trace_path); assert t['policy']['policy_sha256'] == digest
    for n in (model_xml.name, task_json.name): assert (r / 'rollout' / n).read_bytes() == (train / n).read_bytes()
...
try:
    ...
    trainer_exit = proc.wait(timeout=3700)
    ...
    stop.set()
    if proc.poll() is None: print('Trainer continues under independent 3600 second timeout', flush=True)
    log.close()
