...
1. **The lifecycle loop's last unmeasured leg (mission 2).** The leg itself is now measured: `humble-star-3313` ran a real provider turn on `--resume` that read `ot4-quill`'s own `PROGRESS.md`, disputed the project's ADR-005, wrote project ADR-007, changed the geometry, and drove the review's clearance eye from one offending pair (960 mm³) to zero. What is left is narrower and is one iteration: **a single uninterrupted `walk --prompt … --resume` invocation, start to review**, because that run was assembled in two pieces after a kill. It is **not provider-blocked** — `dry-rain-0489`'s refusal was scoped to the walk's default model, and three named models answer on this login (short unit 1's evidence). Nothing may displace it: no fresh mechanism, no extra seed, no rerun of a completed regression as its own unit, no general context subsystem, and no token-free floor taken in place of an attempt [rec: sleepy-mesa-0821] [rec: humble-star-3313] [rec: dry-rain-0489] [rec: candid-fern-3092] [rec: terse-crane-6585].

2. **What a human still needs in order to tick each seeded criterion.** The four criteria sit at `working`, and the reconciler is explicit that the tick "stays a charter decision rather than a maintainer one." Stated once, so no pass has to re-derive it. **The walk exists and is tested headlessly** — one uninterrupted invocation whose design leg is a live turn against project history; short unit 1 is that run, and everything else it needed is spent. **The agent can see its work without a screen** — the four eyes have been exercised together on live geometry across three rigs, and the one named remaining miss is precise: the derived section still returns `cmp_swing_arm` `empty` on `ot4-swing2`, because the ranking counts bounds crossed and the accept test asks only whether *some* object cut. That is short unit 3. **Three modes, one shape** — needs nothing under this run's constraints: the remote limb is scripted through ADR-200/255/268 and the GUI limb documented leg by leg and pinned through ADR-201/269, and its only honest gap, a real dispatch to a real box, is forbidden here. **The walk holds on a second mechanism** — the second mechanism's walk was broken by `train --put` overwriting the asset its accepted script declares by sha256, and ADR-272 fixed it; `ot4-swing2` and `ot4-carriage` both walk end to end now, so a fourth rig is worth less than short unit 3, which improves the eye on the rigs that exist [rec: sleepy-mesa-0821] [rec: dry-rain-0489] [rec: copper-canyon-0231] [rec: cool-eagle-6400] [rec: western-grotto-7499] [rec: falling-willow-7995].

3. **Carry `--detach` through the CLI's walk (mission 2).** Demoted from short, not dropped. `idle-falcon-3004` closed the remote limb's last declared shape difference — a warm start travels and re-points — and named this as the one that is left: "`--detach` still does not travel through the CLI's walk." Small, self-contained, `cli/` plus `training/`, no engine, protocol, payload or `shell/` diff, and testable against the stand-in `ssh`/`rsync` box fixture that record already built. It sits here because it polishes a limb whose remaining honest gap — a real dispatch to a real box — this run's constraints forbid either way, so it must never displace a walk leg or an eye that still misses a part [rec: sleepy-mesa-0821] [rec: idle-falcon-3004] [rec: terse-crane-6585].

4. **Delivered work leaves dispatch.** Seeds 0–3 completed with fixed exported model/task and objective/action identities, training seeds varied and rollout seed 7 held; reward 170.952826823611–175.935971673601, translation 30.078469436328–31.421759939733 mm, rotation zero, every review succeeding and prior artifacts surviving. History updates replace complete files only after successful writing. Recent DECISIONS entries survive the bounded prompt on fresh and resumed turns, and the lifecycle guide no longer promises unconditional repository initialization. The section offset is derived from the accepted snapshot's own bounds with `cadex section --plane/--offset-mm` unchanged, and the remote dispatcher now carries and re-points a warm start. Protection is per-file, not concurrency, multi-document or power-loss safety; context availability is not model understanding; derived coverage was computed from stored summaries, not a live cut. No additional failure-hardening or context campaign follows [rec: honest-banner-0821] [rec: solar-snow-8543] [rec: western-cliff-6876] [rec: idle-falcon-3004].
