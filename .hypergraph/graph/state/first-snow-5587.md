...
**The closing report at `docs/probes/ot7/REPORT.md` carries F4's completed repair turn and, after a correction, consistent slot totals; critic acceptance of done remains unmet.** Three sections were written forward on 2026-09-15: iteration 46 (ADR-356: the F4 row reads zero attempts, one interrupted call apart from three void ones, retry named) [rec: sunny-chart-3499]; iteration 48 (the frozen repair prompt completed on `ot7-heron-repair-d`, the three ot6 defects read against the collector's after-read, the F4 table and evidence rows updated, the seeded repair removed from the open list, receipts `repair-void-c.json` and `repair-completed-d.json` at 10.3 KB and 10.1 KB) [rec: stormy-snow-5452]; and iteration 49 (ADR-357: the headline, slot table, F4 evidence row and remaining-work order now say the repair prompt is spent and three continuations are unspent, where iteration 48 had written "none" in the column every other design shows as "3 of 3"; the d receipt carries a `ruling` field with its historical `status: exhausted` and `slots_spent` kept) [rec: terse-mesa-7963]. The runner README, the prompts README limits table and the retained index were corrected in the same commit [rec: terse-mesa-7963]. Reconcile judgement: the document half of F10 is current and self-consistent; done acceptance is not claimable while F4 has three continuations and F5–F7 have every slot unspent. Retain `open`.

**How the report got here.** It was rewritten forward under ADR-355 with the amendment section, a per-design table of void calls, attempts, unspent slots and retry project, and the 2026-09-14 "exhausted" and "terminal incomplete" wording kept and marked superseded, not deleted; the six pre-restart calls are listed apart from design attempts as void by code (`docs/probes/ot7/attempts/void-calls.json`) [rec: narrow-wave-7452]. The F4 timeout on `ot7-heron-repair-b` was written in with its receipt (`repair-timeout-b.json`, 11,180 bytes), then ruled interrupted under ADR-356 with a `ruling` field added and the historical values kept [rec: rare-birch-0755] [rec: sunny-chart-3499]. Iteration 48's "F4 exhausted" claim was rejected by the critic against the charter's exhaustion policy and corrected forward rather than edited away [rec: terse-mesa-7963].
