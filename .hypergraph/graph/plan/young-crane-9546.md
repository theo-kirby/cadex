...
**One thing leads this run: the lifecycle walk, run end to end on this machine.** The plan inherited from nt3 held two conditional slots that were explicitly not dispatchable and a paragraph telling the controller to stop dispatching; that disposition was true for the machine and credit state it was written on and is now spent. The trigger it demanded has arrived: the first walk on this machine exited 3 at the train leg after 1:06:25, on the named criterion **The walk exists and is tested headlessly** (`crisp-reef-5607`) [rec: open-hollow-2140] [rec: ancient-key-7299].

1. **Bisect the dynamics declarations that burn the worker CPU cap (missions 1 and 2; dispatchable now).** The walked rig's accepted `script.py` rebuilds geometry-only in 1 s and then burns exactly 300 s at `params --set policy_on=1`, exit 3, `The isolated domain worker exited without a result`. Add its dynamics declarations back one class at a time — bodies with per-component collisions, the servo actuator with its limits, observations, sensors, the two ranged `assembly.disturbance` values, the reward list — timing each at `policy_on=1` on the dev-tree engine. The unit names the class that crosses the cap and either fixes it or records why it cannot be fixed cheaply; it does not redesign the dynamics surface. Gate: `pixi run python -m pytest src/Mod/cadex/cadex_tests` for an engine change, `cli/tests` for a CLI one; the packaged gate only if the payload is touched. Timing from outside the worker, because script-level `print(..., flush=True)` is lost on the kill [rec: open-hollow-2140] [rec: ancient-key-7299].

2. **Make the CPU-cap kill legible (missions 1 and 2; dispatchable, independent of unit 1).** `_resource_limits` applies `RLIMIT_CPU` from `cpu_limit_seconds` in CPU-seconds while the parent's timeout is the same number in wall-clock seconds, so a threaded pass on a 32-core box reaches the former first and dies by SIGXCPU (returncode -24) as an opaque "exited without a result" rather than a budget refusal. Map that signal to an explicit refusal naming the cap, and state the asymmetry where `DEFAULT_SCRIPTED_TIMEOUT_SECONDS` is documented. Message and document before any change to the cap itself, per the question policy's reversible-option rule. Small engine diff plus a regression that fails on the old source; engine suite [rec: open-hollow-2140] [rec: ancient-key-7299].

3. **Re-run `cadex walk --prompt` end to end (mission 2; the charter's standing short unit).** A clean run is the evidence that closes `crisp-reef-5607`; a failed run names the next unit and is not a wasted iteration. Run it into a durable project outside this repository so no generated artifact enters the run branch, with `--model` supplied by `$CADEX_MODEL` (ADR-249) and `JAX_PLATFORMS=cpu`. Do this whenever the rung above it is empty [rec: open-hollow-2140] [rec: ancient-key-7299].

No new direction is proposed. The allowance is one and none is needed: five criterion gaps are open and the next unit is named by evidence rather than invented. No charter-derived gap is retired, blocked or superseded by this pass — none is blocked. Gait scale (`late-pond-2851`) and catalog breadth (`brave-stone-9609`) stay parked under Later criteria, where only a human charter edit promotes them [rec: ancient-key-7299].

Signals at this pass: 1 iteration, 1.5h elapsed, 46.5h left; claude seven_day 58%, five_hour 35%; codex seven_day 60%. The walk itself costs about an hour of wall-clock, so three units plus a re-run fit without crowding out the maintainer passes [rec: ancient-key-7299].

## Negative knowledge

- [scope: the failed walk's own diagnosis | confidence: high | evidence: open-hollow-2140] The design agent's `docs/rejected.md` asserts that a single grounded box with one box collision exhausts the worker cap. That is measured false: such a model accepts in 0 s, and the reference `linear-carriage` accepts in 0.55 s with a box, 1 s with a mesh and 0 s with a hull, while `plane` is refused correctly by MuJoCo. Do not treat a design turn's written diagnosis as evidence; the fault is in that rig's declarations, not in `assembly.mjcf` as such. Two facts for the bisect: the cap is `RLIMIT_CPU` in CPU-seconds against a wall-clock parent timeout, and script-level prints are lost on SIGXCPU, so instrumenting from inside the script does not locate the stall [rec: open-hollow-2140].

- [scope: nt3's provider-credit parking, retired | confidence: high | evidence: open-hollow-2140] The entries below that park work on usage-credit refusals are superseded. ADR-249 at `bc203d28` makes `--model` read `$CADEX_MODEL` before `DEFAULT_MODEL`; probing on this login, `claude-sonnet-5`, `claude-opus-5` and `claude-haiku-4-5-20251001` all answered while the constant `claude-fable-5` did not. A machine whose default model has no credit is no longer a reason to park a walk. This does not license a provider probe as a unit, and the durable nt3-leg baseline noted below still stands [rec: open-hollow-2140] [rec: sunny-walrus-5847].

- [scope: the inherited conditional slots | confidence: high | evidence: ancient-key-7299] nt3's short horizon — two conditional, non-dispatchable repair slots and a recommendation that the controller stop actor dispatch — is spent, not deleted. It described a frontier with no trigger; a trigger now exists. Do not restate that disposition, and do not read this replacement as authority for an audit campaign, a repeated hold record or scope expansion beyond the three ranked units [rec: curious-badger-0797] [rec: ancient-key-7299].
...
- [scope: eligibility scheduling, historical | confidence: high | evidence: proud-beacon-8002] The 06:30 UTC boundary was met at 06:30:04 and the single attempt ran and completed; the preflight rule and the 113-iteration scheduling stall are spent. No clock observation, eligibility record or provider probe is a unit now; a refusal on the iterate is recorded once [rec: happy-tide-1066] [rec: proud-beacon-8002].

- [scope: repeated premature dispatch | confidence: high | evidence: lawful-orchard-3508] Iterations 47 and 48 performed no rehearsal or provider request. Re-recording the scheduling hold cannot satisfy the critic's required lifecycle evidence. Controller enforcement is required but unproved; plan prose alone has not prevented dispatch. No repeated administrative unit is selected [rec: dawn-light-8546] [rec: lawful-orchard-3508] [rec: strong-trail-5488].
...
- [scope: the walk as evidence for catalog-part clearance | confidence: high | evidence: morning-summit-7848] The design turn is nondeterministic about placing versus fusing catalog parts: the pan-tilt rerun fused both `lib.servo` bodies into hand solids, so its structural clearance check passed vacuously and proved nothing about the fix. Evidence on catalog rows came from the earlier pan-tilt project through the same bundle instead: 20 comparisons, 0 failures, negative control 10 failures. Do not re-roll a walk to seek catalog components; check an assembly that has them. ADR-242's swept path is exercised only by its regression, because the walk's design turns declare no simulation clearance pairs [rec: morning-summit-7848] [rec: southern-otter-5999].
...
- strong-trail-5488 — Fold repeated premature dispatch evidence and current budget; distinguish one pending rehearsal from zero eligible actor units.

- happy-tide-1066 — Permit a local eligibility preflight within the pending rehearsal, preserving its single-attempt boundary and all scope limits.
...
- ancient-key-7299 — reset the short rung to the walk: bisect the policy_on=1 CPU-cap stall, make the SIGXCPU kill legible, then re-run the walk end to end
