...
1. **Qualify first-visit Git ownership once (missions 1 and 2).** The provider-free rehearsal and critic remediation have landed; no failing lifecycle leg was demonstrated. Use the ordinary documented `cadex script --set` acceptance command and the existing qualified stage in three external disposable roots: fresh with no Git, already its own Git repository, and nested inside a parent work tree. Inspect docs/CLI.md, project-doc scaffold and existing project_docs tests first; derive exact flags from the example. Establish which case initializes Git, writes or preserves ignore rules, commits accepted work, or leaves the parent index alone. Snapshot user-owned ignore files, unrelated files and index entries before and after; distinguish expected accepted-project changes from user-work loss. Query representative nonexistent checkpoint/trace paths with git check-ignore instead of creating or committing generated artifacts. Report actual notes, commit subjects and preserved bytes, and the exact documented mismatch if any. No training, walk, provider, stage refresh or durable-project mutation. This is a finite ownership-contract check, not another lifecycle rehearsal or a claim that the setup error was a product bug; stop after the three cases [rec: lucid-snow-0350] [rec: scarlet-journey-0203] [rec: polished-tide-5222].

2. **Correct only the qualified Git-contract mismatch (missions 1 and 2; conditional).** If unit 1 demonstrates misleading docs/scaffold, correct that exact distinction together; if it demonstrates a narrow product defect, fix only that case while preserving legitimate ownership and user-file behavior. A project-root repository and a nested project need not behave alike. Do not install or overwrite ignore rules in an existing repository merely to enforce the scratch experiment's setup preference. CLI edits require full CLI tests; other touched zones require their AGENTS.md gates, with at most one full build. Behavior changes update walk docs and scaffold together; ADR/ROADMAP where required. If all three cases match the contract, or no bounded safe correction exists, this unit is not dispatchable: return the evidence for replanning, with no automatic repeat or broader Git campaign [rec: scarlet-journey-0203] [rec: polished-tide-5222].

Signals: 191 iterations, 11.5h elapsed, 3.5h left, $85.47 API-equivalent; Claude five_hour 65%, seven_day 54%; Codex seven_day 39%. One new standing-maintenance direction serves missions 1 and 2, selected from three considered options. No Later promotion, charter-gap retirement, or criterion reopening. The live repair remains controller-parked after the usage-credit refusal; no automatic retry, availability probe, repeated hold record, provider switch, paid fallback, GUI or remote execution [rec: sunny-walrus-5847] [rec: polished-tide-5222].
