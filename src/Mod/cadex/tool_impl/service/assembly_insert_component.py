...
TOOL_SPEC = {
    ...
    "description": (
        "Insert one existing named object (Part feature, PartDesign Body, "
        "primitive, or App::Link such as one created by "
        "file.link_external_part) into an exact assembly as a new linked "
        "component "
...
def _validate_component_source(service: Any, source: Any) -> dict[str, Any]:
    ...
    if type_id == "App::Link" and scripted_publication.is_publication(source):
        ...
        if health.get("valid_non_null") and int(
            ...
            return {
                "ok": True,
                "component_type": "XScript publication",
                "shape": health,
            }
        return _invalid(
    ...
    if source.isDerivedFrom("App::Link"):
        linked = getattr(source, "LinkedObject", None)
        if linked is None:
            return _invalid(
                "The App::Link has no linked object to use as a component.",
                requested_object={"name": source.Name, "label": source.Label, "type": type_id},
            )
        nested = _validate_component_source(service, linked)
        if nested.get("ok"):
            return {
                "ok": True,
                "component_type": type_id,
                "linked_object": getattr(linked, "Name", None),
                "linked_component_type": nested.get("component_type"),
            }
        return _invalid(
            "The App::Link's target is not a valid standalone component.",
            requested_object={"name": source.Name, "label": source.Label, "type": type_id},
            linked_object_validation=nested,
        )
    ...
    return _invalid(
        ...
        allowed_types=[
            ...
            "registered solid XScript publication",
