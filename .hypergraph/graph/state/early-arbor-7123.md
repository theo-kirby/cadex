...
- **`pixi run test-engine` runs the full engine pytest directory without a build.** AGENTS.md retains that guidance and removes the stale test count; documentation export/check and whitespace verification passed. Reconcile judgement: status stays `working`; this correction adds no runtime qualification [rec: curious-badger-6887].
...
- **The hypergraph tooling comes in two halves that upgrade separately.** The skills under `.claude/skills/hypergraph-*` are **committed** and arrive with a clone; the `hypergraph` CLI is a Python package and is not, so each machine runs `uv tool install hypergraph-protocol` once. `uv tool upgrade hypergraph-protocol` moves the CLI, `hypergraph upgrade` moves this repo's copies, and neither can see the other's half — `check` warns on the skew [rec: twilight-sail-5604]. Onboarding now labels `hypergraph --version` as the installed CLI version and points to `hypergraph_version` in `.hypergraph/config.yml` for the project-copy version, removing the stale duplicated 0.0.7 assertion. Documentation export/check and whitespace verification passed; status remains `working`, with no new runtime qualification [rec: weathered-trail-0874].
