...
1. **Repair worker-bundle snapshot integrity (missions 1 and 2).** One bounded engine correction at shared_worker_bundle creation/reuse: detach module snapshots from mutable source inodes, derive the name from the bytes actually published, and reject or replace already-present mismatched bundles, including legacy hardlinks. Preserve atomic complete publication, identical-content reuse and missing-member recovery; ensure a publication-race fallback validates identity before returning. Keep project asset staging's separate contract and avoid a cache redesign, new registry or dependency. Pin the two-root A-to-B stale-reuse reproduction with a regression that fails on the old code, and cover pre-existing corruption and a controlled source change between hashing and staging. Record untested concurrency limits rather than promising general race safety. Run the full engine suite, at most one full build via pixi run build-engine, then finish stage-engine before the fresh packaged lifecycle gate and built-engine CLI suite. Include a bounded real worker/rebuild probe with module hashes and a warm-cache timing observation to expose reuse cost; no provider or training dispatch is needed for this repair. Update the behavior doc/date, ADR and landed ROADMAP item; record forest-wind-0342 and lifecycle impacts only to the extent verified. Never blanket-delete the shared cache or alter accepted projects. If qualification cannot finish, report the exact remaining gate and replan before the rehearsal [rec: nimble-basin-8423] [rec: windy-dune-3488] [rec: amber-fjord-1560].
2. **Retry the two-servo leg once quota permits (missions 2 and 6, the ladder rule).** The prior attempt reached no accepted design. Dispatch only after the provider's reported 08:30 Europe/Madrid reset or affirmative availability evidence; no repeated provider probes, paid fallback or provider switch. If still blocked when eligible, defer this unit and report the blocker rather than waiting in an actor. After rank 1 is qualified, run the same documented walk and mechanism prompt from silent-mist-5233 in a fresh project outside the checkout, with the corrected CLI and an explicitly identified engine carrying the correction (use the fresh payload qualified by rank 1; windy-dune-3488's earlier stage proves only the tally correction, and the installed bundle remains stale). Use the training venv, JAX_PLATFORMS=cpu, --iterations 1 --envs 4 --seed 0 --timeout 600, and the existing process-tree guard at 2.9 GB and 850 s. Report every leg as clean, needing a guess/person, or not reached, with exact errors. Inspect all four views and section, compare script catalog usage with placed inventory manually, and use the bound-agreement checker if catalog rows exist. No source correction in the rehearsal; failures name the next unit. No absence-of-tally value is evidence that no catalog part was fused. Record crisp-reef-5607 and swift-dusk-2951 evidence, and damp-moon-9297 only for review findings [rec: silent-mist-5233] [rec: morning-summit-7848] [rec: staid-willow-6557]. [rec: windy-dune-3488] [rec: modest-valley-3313] [rec: misty-tide-6394] [rec: amber-fjord-1560].

Signals: 41 iterations, 7.3h elapsed, 7.7h left; Claude five_hour 75%, seven_day 47%, Codex seven_day 7%. Guidance is complete (CLI 195 passed/no skips); scratch diagnosis is complete, product repair is not. Two units only. No repeated provider probes, actor waiting, provider switch, GUI launch, remote execution, charter edits or Later promotion. After either unit yields a blocker, replan from evidence; do not fill the wait with unrelated work [rec: misty-tide-6394] [rec: nimble-basin-8423] [rec: silent-mist-5233] [rec: amber-fjord-1560].
