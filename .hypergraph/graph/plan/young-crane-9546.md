...
1. **Correct the GPU-only training rationale (mission 2, standing maintenance).** In docs/VISION.md guiding principle 5, explain offboard training through its dependency and packaging boundary, with a short reference to training/SETUP.md for CPU toy and GPU paths. Preserve agent-driven training, policy ingestion and human judgement; remove the obsolete dispatch-history digression. Limit the change to this paragraph, its verification date and a dated ADR-084 maintenance note. Verify against the supported CPU path and recorded walk; run source/document, diff and graph checks and explicitly report no runtime gate or build rerun for prose alone. Record real impacts on early-arbor-7123 and crisp-reef-5607; invent no ROADMAP feature checkbox. No runtime, scaffold or wider documentation audit is selected [rec: lucky-canyon-6724] [rec: civic-snow-4700].
2. **Conditional response to a concrete review finding (mission 2).** Dispatch only for a critic must-fix or source evidence contradicting rank 1. Resolve that finding under the touched zone's gate; any runtime scope needs a new bet. Clean acceptance exhausts this direction: re-plan without manufacturing repairs, repeating administrative handoffs or replaying a clean walk [rec: careful-sand-9714] [rec: civic-snow-4700].

The orientation correction landed and its untriggered repair is withdrawn. No charter gap is retired. Signals: 25 iterations, 4.3h elapsed, 10.7h left. Carry only the already selected subtractive direction; keep GUI unlaunched, remote/GPU/provisioning idle and Later criteria parked. At most one full build per unit if its zone requires it [rec: lean-union-6426] [rec: lucky-canyon-6724] [rec: civic-snow-4700].
