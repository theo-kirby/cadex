...
1. **Fresh mixed-joint lifecycle coverage is now blocked at `train`, not at design and not at the provider (mission 2).** `ot4-mix52` passed design from nothing: exit 0 in 1,135.85 s, a four-body closed-loop planar slider-crank, mobility 1, worst closure residual 0.0015 mm over a 2 s driven run, servo unsaturated, project DECISION lines and three domain notes landed. `train` failed in 2.27 s on `mjx.put_model` raising `NotImplementedError` for `(mjGEOM_CYLINDER, mjGEOM_BOX)`; declare, rollout and all four eyes were not reached and the review block was empty. The geometry half of the criterion is evidenced; the training half is not, and the named cause is one design decision made without the constraint in view. Short leads with the guide trim that carries it, then one walk re-run. ADR-274's silence follow-up remains something to observe in an eligible walk, not a scheduled rerun. No extra seeds and no repeated refusal-only dispatch [rec: golden-dune-8756] [rec: sharp-vine-8567] [rec: restless-wave-7376] [rec: idle-pond-4961] [rec: sage-glacier-2165].

2. **A mechanical, `mjx`-free unsupported-geom-pair check at MJCF export — this pass's one new direction (mission 2).** Conditional: build it only if short's walk re-run shows the guide alone did not change what an unaided design turn authors. Shape: a static table of collision pairs MJX does not implement, applied to the exported model's contact-eligible geoms, surfaced as a warning the design turn sees inside its own leg rather than nineteen minutes later. Guardrails: never an `mjx` or `jax` import under `src/Mod/cadex` or in a staged payload (ADR-084); a warning, not a refusal to export; no claim that the MJCF is invalid MuJoCo; no engine dependency on the trainer; and if an op's result shape changes, `OP_ARG_SPECS`, `docs/INTEGRATION.md` and the shell client move in the same PR. If the prose worked, this is dropped rather than built [rec: golden-dune-8756] [rec: sharp-vine-8567].

3. **Preserve the four seeded criteria's evidence and the human's decision.** All four remain working in the projection. The walk has an uninterrupted resumed success, earlier from-nothing successes, and now a fresh from-nothing design leg; the requested mixed-joint run remains unevidenced past `train`. All four headless review calls exist and have run; ADR-275 delivered standalone derived sections (unsupported 2/10 at explicit zero versus ok 6/10 derived on ot4-swing2). ADR-277 delivered the missed-object/movement join (five stored swing misses: three moving, two stationary); ADR-276 delivered project model continuity; ADR-278 the detached pending receipt; ADR-279 the both-ends architecture bound; ADR-280 the named cause on a failed training leg, with 289 CLI passes. These leave dispatch, without becoming new prerequisites for acknowledging delivery [rec: idle-pond-4961] [rec: restless-wave-7376] [rec: salty-wolf-2393] [rec: golden-dune-8756]. Three modes already have the scripted remote handoff and documented GUI shape allowed here; actual remote and GUI runs remain forbidden. The second-mechanism evidence remains valid. No checkbox, state status or charter gap is retired by planning [rec: neat-grotto-9232] [rec: sage-glacier-2165] [rec: northern-quartz-8401] [rec: cool-eagle-6400] [rec: falling-willow-7995] [rec: silent-anchor-7513].

4. **Finish the existing detached walk handoff (mission 2).** Its prerequisite has landed: ADR-278 gives `cadex train --remote --detach` a project-local run locator and an explicit pending result that verifies and stores no policy. The dependent unit carries that locator through the walk and documents and scripts completion through the dispatcher's existing watch/pull operations, then the ordinary policy verification, declaration, rollout and review. Pending is distinct from success; completion must not repeat design or training, consume stale policy bytes, or lose warm-start provenance. Reuse existing run artifacts and policy checks, with local transport stand-ins pinning the pending, failed and completed paths; no live remote or GUI execution and no background scheduler. It sits behind short's two units, not behind a missing prerequisite. This remains the existing `--detach` direction, not a new requirement for the already-working Three modes criterion [rec: idle-falcon-3004] [rec: clear-shade-1084] [rec: salty-wolf-2393].

5. **Delivered work leaves dispatch.** Seeds 0–3 completed with fixed exported model/task and objective/action identities, training seeds varied and rollout seed 7 held; reward 170.952826823611–175.935971673601, translation 30.078469436328–31.421759939733 mm, rotation zero, every review succeeding and prior artifacts surviving. History updates replace complete files only after successful writing. Recent DECISIONS entries survive the bounded prompt on fresh and resumed turns, and the lifecycle guide no longer promises unconditional repository initialization. The section offset is derived from the accepted snapshot's own bounds and since ADR-273 ranked by objects actually cut, and ADR-275 now exposes derivation when standalone `--offset-mm` is omitted (explicit zero remains explicit); the remote dispatcher carries and re-points a warm start; and a second walk of a project no longer locks it out (ADR-272). A failed training leg's `--json` error now carries the trainer's stderr tail rather than benign startup warnings, with live stderr passthrough preserved (ADR-280). Protection is per-file, not concurrency, multi-document or power-loss safety; context availability is not model understanding [rec: honest-banner-0821] [rec: solar-snow-8543] [rec: western-cliff-6876] [rec: idle-falcon-3004] [rec: rising-rain-0117] [rec: neat-grotto-9232] [rec: silent-anchor-7513] [rec: golden-dune-8756].

6. **Keep comparison and retention limits.** Four-seed ranges are descriptive; the 30 mm action midpoint remains a confound and neither matching objectives nor favorable travel proves equivalent control difficulty or learned improvement. Root review exclusions and policy exclusions after default negations govern untracked files; existing tracked/staged paths retain ordinary Git semantics. Legacy identity rows remain unavailable and numeric comparison references may differ from same-kind identity references. A derived section offset changes the artifact path for future runs and proves no motion clearance. No automatic migration, untracking or standalone-rollout reporting work follows [rec: honest-banner-0821] [rec: easy-beacon-3803] [rec: modest-prairie-3356] [rec: steady-chart-0544].

7. **Keep the charter boundary.** Four seeded criteria are working in the projection. Remote handoff remains scripted/documented and GUI attachment documented under execution constraints. Keep mode rehearsals, inherited-removal campaigns, catalog, fleet, report rendering and gait training parked. Kernel rollout clearance remains unselected on long; and the quill overlap that ADR-005 called deliberate was disputed by a live design turn and designed out to zero, which is a warning about inferring a cause from a project's own record as much as from arithmetic. No charter gap is deleted, retired or marked done by this pass [rec: restless-cabin-9629] [rec: western-grotto-7499] [rec: sleepy-hollow-9498] [rec: humble-star-3313].
