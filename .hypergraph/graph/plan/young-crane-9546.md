...
1. **Audit only the inherited translation updater offline (mission 3; round-glacier-2865, green-sea-3991).** One documentation/evidence unit for src/Tools/updatecrowdin.py: inventory every command branch, retained App translation consumers, tracked and external maintenance entry points, build/install references and writer behavior. Distinguish proven absence from unknown external use. Static reads or isolated stubs only: no network, credentials, resource writes, product edits, whole-tool deletion premise, replacement tool, App resource/Qt removal or shell work. Qualify at most one subtractive boundary with compatibility cost and separate disable/delete stages where required, or explicitly qualify none. Specify affected-zone gates, retained-translation checks, manifest/notices and at most one full build per future unit. Land dated audit/ADR/ROADMAP evidence and one work record with real impacts; run applicable documentation/licensing and graph checks, reporting omitted gates honestly. [rec: noble-clover-4083] [rec: hollow-reef-8734]
2. **Resolve the audit into a later bounded bet (mission 3; round-glacier-2865).** After rank 1's committed evidence, one evidence-only planning unit: if exactly one safe boundary qualifies, record its files, retained behavior, compatibility cost and required gates in a later bet before dispatching implementation; preserve separate disable/delete commits under the removal protocol. If none qualifies, record the stop and replan from existing gaps without expanding the audit or retrying accessory/catalog searches. This rank authorizes no code or deletion and claims no qualification in advance. Reconcile due impacts through their owning roles; never edit state as an actor. [rec: noble-clover-4083] [rec: hollow-reef-8734]
