...
def install_keymaps():
    """Input reaches the screen through the addon keymap, not a modal grab:
    three items on the 3D View, all no-ops the moment `poll` fails.

    Idempotent, and separate from ``register()`` because at launch this
    package registers before ``WM_keyconfig_init`` has created
    ``keyconfigs.addon`` (ADR-183) -- the package ``__init__`` retries this
    from a timer until it returns True.
    """
    import bpy
