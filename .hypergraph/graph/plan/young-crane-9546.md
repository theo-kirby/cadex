...
1. **Rehearse failure-to-success recovery through the public walk (missions 1/2/6).** In an isolated project recreate the two successful toy runs and injected trainer exit 7, then retry from the retained sweep (policy_on=0, lift_weight=0.0003) using the real local CPU trainer, a fresh output directory and new policy name. Warm-start from the prior successful policy/task with the task change declared. Complete verification, rollout and all four review outputs; inspect prior artifact hashes, PROGRESS preservation and the new comparison's actual reference. Record commands, numbers and any intervention needed. Each training run stays within 15 minutes and 3 GB; no model, GUI or remote dispatch. This satisfies the empty-rung rehearsal at the newly evidenced failure boundary [rec: dawn-sky-3891] [rec: northern-sage-7087].
2. **Replace terminal export coverage with the measured recovery contract (missions 1/2/6).** Once unit 1 succeeds, replace the existing real-engine regression's export-only ending with the successful retry. Pin retained sweep, prior artifacts/history, verified new policy and honest comparison reference. Replace overlapping failure prose in the CLI guide and project scaffold with concise tested retry instructions; consolidate redundant assertions so the direction removes more than it adds. No parallel harness or rollback layer. Run `pixi run python -m pytest cli/tests`, update ROADMAP and record removals in an ADR. If unit 1 fails, its exact failing leg replaces this unit with a bounded repair and regression, using every applicable zone gate [rec: dawn-sky-3891] [rec: northern-sage-7087].
3. **Finite sequence and scope.** Prior guide corrections and failure preservation are complete (223 CLI passes, no skips); the empty-dispatch record advanced nothing. Replan after these two units without automatic failure matrices or repeated rehearsals. Signals: 11 iterations, 4.1h elapsed, 43.9h left; Claude seven_day 62%, five_hour 100%, Codex seven_day 68%. No clock gate, state-gap retirement or Later promotion [rec: careful-union-7585] [rec: dawn-sky-3891] [rec: northern-sage-7087].
