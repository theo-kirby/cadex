...
1. **Complete the remaining headless-review evidence (missions 6 and 2; damp-moon-9297).** All four CLI review calls and walk integration are delivered. Sections preserve cavities and solved placements through qualified tessellation contours; the walk shares one accepted snapshot and reports empty/unsupported/error outcomes explicitly. Integration covers arm/carriage and local/remote-flag CPU stand-in parity, with 195 CLI tests passing and zero skips. What remains is the separate fresh complete-review rehearsal, now split across the two short units, with inspected artifacts, committed project docs and comparable numbers. Implementation tests do not replace this evidence [rec: placid-lily-5624] [rec: copper-timber-8947] [rec: sage-crow-3224].
2. **Preserve the lifecycle while resolving rehearsal findings (missions 2 and 6).** A failed leg or critic must-fix leads the next unit; do not reopen delivered tools without evidence. Complete both documented mechanisms through the same entry point before proposing review closure in a work impact. Only maintainer reconciliation changes the gap status. Keep GUI mode documented only and remote training scripted with local CPU stand-in parity; artifacts and scaffold already include sections. Initial-pose clearance does not establish swept-motion coverage, and qualified tessellation images/cuts do not establish analytic drawing accuracy or large-assembly throughput [rec: copper-timber-8947] [rec: sage-crow-3224].

No additional gap is promoted while the short rehearsals remain actionable. An ordinary bundle refresh is needed before a bundled verification claim, not before independent CLI work. Later catalog, fleet, outside-source, report and reduction criteria remain outside dispatch. If the active frontier closes or blocks, record that condition and assess standing maintenance within the charter; do not invent a parked target merely to consume the remaining budget. No new direction or charter-gap retirement is proposed [rec: zesty-aspen-6846] [rec: modest-summit-8554] [rec: sage-crow-3224].

## Negative knowledge

- [scope: delivered CPU render coverage and remaining limits | confidence: high | evidence: silver-key-8483] Product depth testing resolves the probe's centroid-order occlusion failure; snapshots and bounded malformed-input handling are tested. Arm and rotated/translated curved assembly images were inspected (24 and 512 triangles); the CLI gate passed 180 with zero skips. SVG wraps a 512px lossless CPU image, not analytic vectors. Shell-only visibility, transparency, analytic edges, subpixel fidelity, motion coverage and large-assembly throughput are not established. Separate command timings are not walk overhead. That render record predates sections: section delivery and walk integration have since landed, with explicit tessellation and initial-pose limits; the separate complete-review rehearsal remains outstanding [rec: placid-lily-5624] [rec: copper-timber-8947]. Blender background rendering refusals and the stale ordinary bundle from the probe remain historical constraints; no GUI launch or bundle qualification follows [rec: silver-key-8483] [rec: zesty-aspen-6846].
