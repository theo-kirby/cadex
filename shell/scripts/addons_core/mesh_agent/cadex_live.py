...
class MESH_AGENT_OT_live_push(Operator):
    """Click a body and drag: the drag is the shove.
    ...
    at :data:`PIXELS_PER_NEWTON`. Feedback is header text; the add-on has no
    draw handlers today and this does not need to be the change that adds
    one.

    A drag can never be repeated exactly, which is why the compass buttons
    exist beside it: an ADR needs a number.
    """

    bl_idname = "mesh_agent.live_push"
    bl_label = "Push"
    bl_description = "Click a part of the mechanism and drag to shove it"
    ...
    def modal(self, context, event):
        ...
        if event.type == 'MOUSEMOVE' and self._body:
            ...
            if context.area is not None:
                context.area.header_text_set(
                    "Push {:s}: {:.2f} N at {:.0f}° from world +X".format(
                        self._body, newtons, degrees))
            return {'RUNNING_MODAL'}
        ...
        if event.type == 'LEFTMOUSE' and event.value == 'RELEASE':
            ...
            if live is None:
                return {'CANCELLED'}
            live.arm_push(newtons, math.radians(degrees), self._body)
            _tag_redraw()
            return {'FINISHED'}
...
class CADEX_LIVE_PT_push(Panel):
    ...
    def draw(self, context):
        ...
        layout.operator(MESH_AGENT_OT_live_push.bl_idname,
                        text="Click and Drag in the Viewport",
                        icon='EYEDROPPER')

        column = layout.column(align=True)
        column.label(text="…or an exactly known direction:")
        for start in (0, 4):
