...
- **pi's mechanical differences**, each verified against pi 0.84.4 and
  ...
  pattern ("" = pi's configured default — pi is multi-provider and owns its
  own catalog); and the subprocess PATH is prefixed with pi's directory so
