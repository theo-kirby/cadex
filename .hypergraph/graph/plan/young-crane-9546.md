...
1. **Wire inventory into the walk's review step (mission 6 and 2; damp-moon-9297).** Use the existing inventory reader, including its complete nested-preview and mapping pagination fix [rec: fair-rose-5950] [rec: lawful-ivy-4474]. After rollout, land `docs/inventory.md` and an `inventory` block in `review.json` containing component count, catalogued count and the document's project-relative path, without tokens or an extra rebuild. Commit the report with the walk artifacts. At the review-reader boundary, no published assembly produces an explicit unavailable/empty report, not an inspection crash; preserve existing training/walk prerequisites rather than claiming a part-only project completes a dynamics walk. Keep the shared mode-artifact table, `project_docs.py` scaffold and `docs/CLI.md` aligned in the same commit, with tests [rec: wild-marsh-9611] [rec: gilded-basin-9946]. Proof: both real recipe walks assert the doc and block; extend the local/remote-flag stand-in parity assertions to cover the new report; include a no-assembly reader case. Record one real hinged-arm recipe walk and its artifact paths, using existing test-run evidence when it proves the same public entry point. Gate: `pixi run python -m pytest cli/tests` with the built engine and local CPU training venv; report all skips honestly. No prompt walk, engine/protocol change, shell change or new subcommand. [rec: crimson-canyon-5993]
2. **Clearance and intersection check naming pairs (mission 6; damp-moon-9297).** Keep the inventory-shaped route: assembly-worker measurements published beside the definition, `inspect scope="clearance"` joined to component labels/catalog ids, and `cadex clearance` writing `docs/clearance.md` with read-time clearance and common-volume thresholds [rec: restless-star-0524]. Correct the earlier overlap-only proposal: minimum-distance coverage must include separated pairs, because disjoint bounding boxes can still violate clearance; box overlap may prune common-volume work only. Do not report an unmeasured pair as clear. Preserve content-digest stillness with a regression and reuse the complete inspection reader [rec: lawful-ivy-4474]. Measure both lifecycle recipes before/after: distinguish rebuild latency from the standing whole-walk baseline (under 16 s and 1.1 GB at toy settings [rec: misty-rain-9048]); if either rebuild grows by more than 2 s or 20 percent, record the result and stop this route, making the previously proposed on-demand alternative the next bounded unit [rec: flat-river-8853]. Tests include one intersecting pair, one separated pair below clearance, one clear pair, threshold changes without rebuild and digest stillness; failures are reported, not hidden. Run the full engine suite, one `pixi run build-engine`, CLI suite against that engine, finish staging, then the packaged lifecycle gate [rec: fair-rose-5950] [rec: stormy-quill-5350]. Update inspect documentation in `docs/INTEGRATION.md`, CLI docs, ADR and ROADMAP as required; record the remaining headless-review gap. No shell change or `OP_ARG_SPECS` change; wiring is a later unit, and a new protocol op needs the measured reason and a subsequent plan honoring the cross-client contract. [rec: crimson-canyon-5993]

The three-mode audit has landed: its work record supplies the exact-wording verdict and the maintainer already projects the criterion as working; do not redispatch it [rec: gilded-basin-9946]. These two outstanding short units run before medium promotion. Once they land or block, clearance wiring is next; render and section stay ordered behind it, each followed by wiring [rec: crimson-canyon-5993]. If short becomes empty, first run the documented local toy lifecycle entry point and let that result name any next repair, as the operator's ladder requires [rec: modest-summit-8554].

Parked Later criteria are not dispatchable; inherited reduction is standing work, never an iteration target [rec: modest-summit-8554].
