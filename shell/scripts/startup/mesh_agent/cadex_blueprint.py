...
theme colour, every object silhouetted in the line colour, the true BREP edge
wires (the ``<output> Edges`` children ``cadex_hydrate`` already draws) drawn
over them, on a blueprint-blue, cutting-mat-green or grey ground — or, the
one light theme, black lines on ``technical``'s drawing-paper white.
...
would appear uninvited. ``wireframe_color_type='OBJECT'`` makes those wires
white for free because no module writes ``obj.color`` (objects default to
white), and the dark-lined ``technical`` theme switches the same field to
``'THEME'`` — the shipped UI theme's wire colour, black — because white wires
on paper white would erase the edges. The collision cage's wires ride the
same rule and go white (or black) under a blueprint, which is cosmetic and
accepted.
...
#: ``line`` the outline and wire colour. ``technical`` is the one
#: light-ground theme — black lines on drawing-paper white, the way a
#: dimensioned technical drawing prints — and the line colour's darkness is
#: what flips the wire channel in :func:`shading_values`.
THEMES = {
    "blueprint": {
...
def shading_values(theme=DEFAULT_THEME, grid=True):
    ...
    grid = bool(grid)
    # The wires have exactly two colours on offer: ``OBJECT`` reads
    # ``obj.color``, which nothing sets, so every wire renders in the object
    # default -- white; ``THEME`` reads the UI theme's wire colour, which the
    # shipped defaults pin to black (userdef_default_theme.c). A theme with
    # dark lines (``technical``) takes the second channel, because white
    # wires on drawing-paper white would erase the model's own edges.
    dark_lines = sum(colors["line"]) < 1.5
    return {
        "shading.type": 'SOLID',
        "shading.light": 'FLAT',
        ...
        "shading.wireframe_color_type": 'THEME' if dark_lines else 'OBJECT',
