...
"""Real Wren copy lifecycle, with the original path unavailable.

PYTHONPATH=cli:cli/tests pixi run python copy_lifecycle.py ORIGINAL COPY OPERATOR_URL
First copy the stopped-writer project and switch the persistent server to COPY.
Requires an unedited copy; keeps the copy and restores ORIGINAL in finally.
"""
import hashlib
import json
...
def check_page(browser, url, root, out, label):
    ...
    for run in runs:
        ...
        foot_len = record['params']['values'].get('foot_len', defaults['foot_len'])
        column = 2 if 'foot_len' in record['params']['values'] else 3
        assert float(page.text(f"#params tr[data-param='foot_len'] td:nth-child({column})")) == foot_len
        ...
        item = {'revision': record['model']['accepted_revision'], 'digest': record['model']['digest'],
                'foot_len_mm': foot_len, 'curves': points}
        # Retain model/mesh identities for comparison across independent servers.
        model = json.loads(urllib.request.urlopen(url + 'api/model/run/' + run).read())
        assert model['revision'] == item['revision'] and model['available']
        item['mesh_sha256'] = {c['output']: sha(urllib.request.urlopen(url.rstrip('/') + c['mesh']).read())
...
def main():
    ...
    unavailable = original.with_name(original.name + '-unavailable54')
    ...
    assert values(old)['foot_len'] == 105 and values(old)['policy_on'] == 1
    out = copy / 'evidence/copy54'
    ...
    try:
        ...
        for name, args in [('params', ['params', '--set', 'policy_on=0', '--set', 'foot_len=110', '--out', str(out / 'export')]),
                           ('render', ['render'])]:
        ...
        assert {k: [values(old)[k], v] for k, v in values(changed).items() if values(old)[k] != v} == {
            'foot_len': [105, 110], 'policy_on': [1, 0]}
        assert changed['accepted_revision'] != old['accepted_revision']
        assert changed['accepted_digest'] != old['accepted_digest']
        # Fresh engine restore with the source still unavailable.
        subprocess.run([sys.executable, str(repo / 'docs/probes/wren-fresh/restore.py'), str(copy), 'copy54-restore'], check=True, timeout=240, stdout=subprocess.DEVNULL)
        server = ReviewCommand(copy, 0)
        with HeadlessBrowser(find_browser()) as browser:
            receipt['second_server'] = check_page(browser, server.url, copy, out, 'second')
            receipt['persistent_server'] = check_page(browser, url, copy, out, 'persistent')
        ...
        receipt.update(copy_revision=changed['accepted_revision'], copy_digest=changed['accepted_digest'],
                       changes={'foot_len': [105, 110], 'policy_on': [1, 0]},
                       source_unavailable_during_edit_restore_browser=True, original_unchanged=True,
