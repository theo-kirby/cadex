...
1. **Fresh walk and cold project revisit (standing maintenance; missions 1, 2 and 6).** First run the documented public hinged-arm script/walk entry point on a fresh external project: training venv, local CPU, one iteration, four environments, seed zero, at most 15 minutes and 3 GB. Record exits, policy witness/reward and process-tree peak memory; inspect the four views and meaningful XZ Y=3.125 mm section. Then close the producing processes and revisit that same project using ordinary public CLI commands in fresh processes. Check restore acceptance and policy identity, project docs, and standalone inventory/clearance/render/section against the committed baseline revision/digest and actual artifact contents. Preserve the known arm contact and explicit unknown counts. Detect unexpected rebuilds, stale-success output or lost artifacts; distinguish documented behavior from defects. Do not delete caches/assets or invent recovery steps. Commit compact evidence and project PROGRESS.md, with no checkpoint/rollout bytes or machine paths; run the full built-engine CLI suite and report skips. This protects delivered behavior, not a newly declared charter gap [rec: first-branch-9614] [rec: quiet-vine-3426] [rec: modest-grotto-1192].
2. **Conditional smallest persistence correction (missions 1, 2 and 6).** Dispatch only if rank 1 or a critic identifies a concrete defect: reproduce it, make one minimal correction, prefer removing the offending redundant/stale path over adding infrastructure, and verify the failing behavior plus the zone's required gate. Update behavior docs/date, ROADMAP and ADR where required, and record real state targets. No generic durability framework, speculative tests or second mechanism replay. If rank 1 passes, this conditional unit is not actionable: record the clean outcome and re-plan; do not manufacture a repair or repeatedly rerun the walk [rec: modest-grotto-1192].

Both former short rehearsals have landed; no charter gap is retired by this plan. Signals: 19 iterations, 3.8h elapsed, 11.2h left; only this bounded evidence unit and conditional repair are selected. No GUI, SSH, GPU dispatch or provisioning. At most one full build per unit if its zone requires it. CLI evidence does not qualify an ordinary bundle. Later criteria remain parked [rec: first-branch-9614] [rec: quiet-vine-3426] [rec: modest-summit-8554] [rec: modest-grotto-1192].
