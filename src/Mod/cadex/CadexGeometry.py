...
"""Process-isolated native geometry execution for Cadex."""
...
def worker_executable() -> Path:
    ...
    names = (
        ("CadexGeometryWorker.exe", "cadexgeometryworker.exe")
        if sys.platform == "win32"
        else ("CadexGeometryWorker", "cadexgeometryworker")
    )
    for name in names:
        ...
        if candidate.is_file():
            return candidate
    raise RuntimeError(
        f"The Cadex geometry worker is missing from {bin_root}. Rebuild or reinstall Cadex."
    )
...
def validate_shape(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="cadex-validation-") as temporary:
