...
1. **Iterate the leg from its own review findings (missions 2 and 6; the one new direction).** Use the documented `cadex walk` on the existing leg project (basename `cadex-nt3-i183-leg` under the system temporary directory; accepted revision 671a1c28, digest 0ee2ba83, PROGRESS row "clearance offending 11; unknown 0; pairs checked 45"), with the same qualified payload at build/engine/cadex-engine-0.0.0-macos-arm64 (CadexScriptedRuntime.py SHA256 602164e85c399ad203517eb269ec81bca549dc07b72d303659c1cffffe1dc6df), the training venv, JAX_PLATFORMS=cpu, --iterations 1 --envs 4 --seed 0 --timeout 600, the existing 2.9 GiB / 850 s process-tree monitor, and a new --out under the project's runs/. One design prompt continues the project's conversation with --resume if the stored session is live, otherwise a fresh conversation on the same project; both are documented, and scaffolding never overwrites existing project docs. The prompt names the findings verbatim: remove the shin/foot-bolt intersections (5.8373525743 mm³ each) and the bolt/nyloc-nut intersections (9.2781252741 mm³ each) by real clearance or explicit mating; attach each driven link to its servo output with a horn or shaft boss that closes the 5 mm gap; keep the two MG90S, two M3x10 bolts and two nyloc nuts as separately placed catalog instances (`assembly.component`), never fused into printed solids; write `docs/sensors.md` and `docs/actuators.md` recording the joint-angle sensors, the torque-limited position-servo model and the damping assumption; record the change in the project's DECISIONS.md. No operator edit to the script. Then train, declare, verify/rollout and review through the same walk. Evidence: the PROGRESS row with total_reward and the change against the previous row (ADR-194); clearance offending against 11 and positive-volume against 4, unknown still 0; four views and the XZ section inspected by eye; manual script-versus-inventory tally (expect six catalog instances or an explained change); the bound-agreement checker over all pairs; project commits clean; no machine path, checkpoint or rollout in this repository. Report each leg clean, guess/person, or not reached, with exact errors. A quota refusal is recorded once and scheduling returns to the controller; no probe, wait, paid fallback or provider switch. If the agent fuses a catalog body, drops a placed part, or the design turn exceeds the guard, that is the finding: record it, and do no source repair in this unit [rec: proud-beacon-8002] [rec: weathered-hill-5955] [rec: morning-summit-7848] [rec: lawful-dune-3795].

2. **Conditional: make the domain-doc convention reachable from the walk (mission 2, the ladder's short rung).** Only if unit 1's direct request still yields no `docs/sensors.md` or `docs/actuators.md`. Read the design instruction in `cli/cadex_cli/agent.py` and the scaffold sentence in `cli/cadex_cli/project_docs.py`; make the smallest correction that makes the notes an expected output of a design turn, preferring wording in the existing instruction over any new tool or op; update the project-docs table in `docs/CLI.md` and the scaffold in the same commit; run the CLI suite and report skips honestly if the engine half does not run; ADR line; record node naming the compliance observation it answers. If unit 1 produces the notes, this unit is void and short empties; the ladder rule then applies: run the documented walk again on a prompt not yet walked, and record which leg needs a person [rec: proud-beacon-8002] [rec: wild-marsh-9611] [rec: weathered-hill-5955].

Signals: 183 iterations, 10.1h elapsed, 4.9h left; API-equivalent cost $71.93; Claude five_hour 27%, seven_day 51%, seven_day overage-included 93%; Codex seven_day 30%. They size two units, one of them conditional; percentages establish no availability. One new direction, no medium promotion, no Later promotion, no charter-gap retirement. Do not repeat the rehearsal, the worker qualification, the dependency discovery or any catalog diagnostic. No GUI launch, remote execution, bundle refresh or paid fallback [rec: weathered-hill-5955] [rec: early-gate-3510] [rec: clear-crest-9910].
