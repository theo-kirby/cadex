...
The open gaps in the order they should fall, re-ordered now that the two the walk depended on have closed. The order is still the charter's: the walk led this run and has landed, so what follows it is the evidence it unblocked [rec: glad-mesa-6299].

1. **The walk holds on a second mechanism (`swift-dusk-2951`; mission 2).** Unblocked, and now first. The criterion asks for the same entry point, with no code change specific to the mechanism, taking a second mechanism through the whole loop with both projects' `PROGRESS.md` carrying comparable numbers. Its gate — "it cannot be re-evidenced until one mechanism completes on this machine" — is open: the swing-arm rig completed, exit 0 in 17:43 [rec: humble-forest-6896] [rec: open-hollow-2140] [rec: wandering-jasper-6102]. One unit: one walk on a mechanism differing in joint and actuator type, into a durable project outside this repository. Five mechanisms of nt3 evidence stand behind it and are not withdrawn; this is evidencing on a new machine, not restarting. Do not open it by re-rolling design prompts [rec: placid-sky-7374] [rec: proud-beacon-8002].

2. **Three modes, one shape (`witty-spark-2613`; mission 2).** One unit, and a currency audit rather than a rebuild. Headless is now exercised here; remote is scripted and offline-tested here, its parity test inside the 217 `cli/tests` that passed; GUI-attached is documented and unexercised, as the run's constraints require. What is unverified is that the ADR-200 and ADR-201 documents still describe this walk's shape after ADR-249's `$CADEX_MODEL`, ADR-250's worker BLAS pin and SIGXCPU refusal, and the four-eye review leg. Correct what drifted and state the limits; do not re-script the dispatcher [rec: humble-forest-6896] [rec: gilded-basin-9946] [rec: red-comet-9710] [rec: wandering-jasper-6102].

3. **Walk reliability, standing (missions 1 and 2).** Not a criterion gap — the class of defect the walk finds by being run. The live one: the walk cannot tell that the installed engine predates the tree, so it reports exit 0 on a stale runtime, which is how the first green walk lost a rebuild's worth of time [rec: wandering-jasper-6102]. One unit, reporting before refusing. This item exists so that what a walk exposes has somewhere to land without becoming an audit campaign.

4. **The RL training loop and mg-legs (`late-pond-2851`; mission 5), parked.** The loop is proven; what is blocked is gait scale, and a real training run is not a unit of work and never fits in an iteration. The GPU is available to this run and is not this run's target. Toy witness success establishes no learned gait, and the green walk's 5 x 16 at reward/step -0.1254 is a smoke test of the loop's shape [rec: proud-beacon-8002] [rec: vast-ledge-4610] [rec: wandering-jasper-6102].

5. **Parts library (`brave-stone-9609`; mission 4), parked under Later criteria.** Catalog breadth, manufacturer STEP provenance, L3 motors and compound mechanisms are all in `## Later criteria`; the planner may not target them. Catalog placement held again on the green walk — ten components, seven catalogued, the review's inventory reading them — while pan-tilt previously fused its hardware, so occurrence ambiguity and cold identity remain open and unselected [rec: early-gate-3510] [rec: morning-summit-7848] [rec: wandering-jasper-6102].

6. **Inherited-tree reduction (`round-glacier-2865`; mission 3), standing only.** nt2 removed 2.79M lines and the charter has retired reduction as a target for this run. Take a removal only when an authorized change makes it obvious and cheap; never as an iteration's target and never as a fresh audit campaign [rec: modest-summit-8554] [rec: autumn-arrow-3125].

Preserved from the previous horizon and not reopened by this pass: the completed worker-identity repair (ADR-244), the closed clearance direction, the completed refusal-metadata work (ADR-247), the first-visit Git, consent, suffix-parity and linked-part qualifications, ADR-245/246, and now the dynamics stall itself (ADR-250, `assembly.mjcf` 300 s / exit 3 to 2.0 s on the walk's own script). Their measurements live in `long` and in the records cited there; none is a unit now. The durable nt3-leg project stays preserved outside this repository at HEAD `b244f26`; account for that baseline on any authorized visit and do not reset or commit it to manufacture success [rec: lawful-dune-3795] [rec: ready-otter-2700] [rec: fond-star-1809] [rec: lawful-wolf-9205].
