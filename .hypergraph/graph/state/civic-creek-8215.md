...
**Heron, a two-DoF MG90S arm designed by the product agent, has been through the full recorded lifecycle: designed from one prompt, corrected three times against measurements, accepted with fits, reopened, trained once (bounded), evaluated over ten seeds, with checkpoint and final videos in the D3 look on the persistent dashboard.** D8 is held `open` only because the human owns the checkbox, the same reading as D7 on `ready-sand-2621`; the records below name the evidence for every clause of the criterion, and one design decision is left open for the next turn on Heron [rec: narrow-quill-3259] [rec: mild-hill-0753].
