...
1. **Explain metadata changes after a refused walk (missions 1 and 2).** One offline reproduction, no live provider request. Read the recorded nt3-leg diff and trace the project-open/restore and agent-session writes; leave that durable project untouched. Use an isolated disposable project and a controlled provider refusal through the ordinary walk orchestration, with the qualified local engine where restoration needs it. Compare a clean accepted baseline, restore-only behavior and refused design: accepted source/revision/digest/parameters, attempt identifiers, session identity, timestamps, project status, PROGRESS and commit history. Identify exactly which writes are required to preserve truthful acceptance/session state and which are avoidable. Record the write sites, observed field changes and a smallest safe subtractive fix boundary, or explain why no defect is demonstrated. No automatic failure commit, ignored tracked metadata, blanket snapshot rollback, provider probe, cache redesign or durable-project cleanup. This advances the walk's project-as-codebase/file-lifecycle invariant; it proves no successful repair turn. Use the existing qualified stage without a refresh; run the CLI suite if adding a regression and the engine suite if touching engine Python, and report all skips and failures. Stop after this finding [rec: sunny-walrus-5847] [rec: gentle-wind-1003].

2. **Remove only reproduced unnecessary writes (missions 1 and 2, conditional on unit 1).** If diagnosis establishes an avoidable write and a safe narrow correction, implement that correction as one unit. Prove controlled refusal preserves the accepted project and truthful session/attempt metadata; include a success-path check so legitimate accepted updates still persist. Preserve useful failure diagnostics and any pre-existing user changes. Do not assert byte-identical metadata if restoration legitimately changes it. Update docs/CLI.md and the project scaffold together for any walk behavior change, add the ADR line and ROADMAP checkbox where required. Run `pixi run python -m pytest cli/tests`; engine Python additionally requires the full engine suite, and protocol/payload changes require their prescribed packaged gates. At most one full build. If no safe subtractive correction is established, this conditional unit is not dispatchable: return the finding for replanning, without manufacturing a fix or retrying the live walk. No durable-project reset or synthetic successful-run commit [rec: sunny-walrus-5847] [rec: gentle-wind-1003].

Signals: 187 iterations, 11.0h elapsed, 4.0h left, $85.47 API-equivalent; Claude five_hour 65%, seven_day 54%; Codex seven_day 34%. ADR-246 landed with four new regressions and real toy iterate evidence; the durable live check was unreached. The live repair is parked after one actual usage-credit refusal, with scheduling returned to the controller: no automatic retry, timed eligibility probe, repeated hold record, provider switch or paid fallback. One new bounded direction, no Later promotion or charter-gap retirement; no GUI, remote execution, bundle refresh, catalog diagnostic or worker campaign [rec: twilight-snow-9071] [rec: sunny-walrus-5847] [rec: gentle-wind-1003].
