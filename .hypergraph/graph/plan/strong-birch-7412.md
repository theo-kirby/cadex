...
1. **Deliver named-angle rendering, then wire it into the walk (missions 6 and 2; damp-moon-9297).** The bounded route probe is now short; implementation waits for its evidence and the outstanding short units to land or block. One CLI call writes front/top/right/iso PNGs of the accepted revision under a documented project path, preserving placements and the LGPL/GPL boundary. Use the route selected by the probe; do not presuppose Blender is available on an engine-only machine. Test actual image contents and named-angle distinctions, not file existence alone. A separate wiring unit adds paths to review, project commits, scaffold and mode-artifact documentation, with real-recipe and stand-in parity evidence. Keep zone gates and any payload gate appropriate to the selected route; no replacement engine or shell [rec: flat-river-8853] [rec: wild-marsh-9611] [rec: humble-stream-3878].
2. **Deliver named-plane section views, then wire them into the walk (missions 6 and 2; damp-moon-9297).** Reuse the proven rendering route; obtain section geometry engine-side, with a plane named by axis and offset or an accepted output's face. One CLI call writes a project-relative artifact. Implement and wire in separate units, preserving accepted-revision fidelity, explicit empty/error results, scaffold/docs and mode parity. This follows render delivery so the image path is shared rather than duplicated [rec: flat-river-8853] [rec: humble-stream-3878].
3. **Evidence the complete headless-review criterion (missions 6 and 2; damp-moon-9297).** After all four calls and their wiring land, exercise them through the same documented walk on both established mechanisms. Verify named-angle render, named-plane section, catalog-id inventory and named-pair clearance artifacts are used by review and recorded in each project. Report comparable numbers, limits and remaining omissions; only work impact declarations and maintainer reconciliation can close the gap. Initial-pose clearance is not swept-motion proof, and large-assembly cost remains unqualified [rec: placid-sky-7374] [rec: falling-birch-1503] [rec: humble-stream-3878].

Clearance wiring moved to short because both old short units landed; the charter-required fresh walk leads it. No conditional mode repair remains: the exact-wording audit supplied artifact parity while GUI execution and remote transport stayed unexercised [rec: happy-dune-3481] [rec: falling-birch-1503] [rec: gilded-basin-9946] [rec: humble-stream-3878].
