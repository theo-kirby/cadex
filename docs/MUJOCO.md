...
| 11 | The same walk with the GUI attached | the same `cadex` commands from a terminal beside the open `.blend` — **not** the in-app agent, which has only the Mesh tools (`--tools ""`, no shell, no file tool) | **Documented 2026-09-06** (ADR-201, `docs/CLI.md` §2) from the client code, no GUI launched: the CLI's `flock` is per command and released before the `PROGRESS.md` row and the commit; the shell takes no lock, so ownership is sequential by convention; an overlapping shell write is refused as `STALE_PROGRAM_REVISION` off the on-disk `script.json`, then retried once by the shell **with its own arguments** against the new revision — so it can silently overwrite the walk's accepted script or parameter values; Rebuild Model or reopen (`load_post` → `queue_open`) before the next GUI edit, never through the re-accept box. Same legs, same docs, same project-relative artifacts. Two runtime legs left open, not taken: the shell holding the same lock, and the retry refusing a foreign revision instead of rewriting it | a person or a pipeline at the terminal; the in-app agent for design turns |
