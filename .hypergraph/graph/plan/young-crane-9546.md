...
1. **Bound catalog dependency discovery (missions 2 and 6).** Execute snowy-cove-0032's existing read-only experiment in one unit, using scratch files outside the checkout and an explicitly identified qualified engine. Inspect the surviving fused pan-tilt script and controlled cases: direct placement reused twice, transformed body feeding a fuse, cut-tool-only use, discarded call, and a body used both as component and cutter. Report output name, operation path and catalog identity only where recoverable; separate run-local evaluation knowledge from a cold accepted report. Require zero reachable paths for a discarded value, retain unknown roles/ambiguous matches as unknown, and keep placed-instance counts unchanged. Stop if identity needs new persistent metadata or geometry semantics. No source changes, protocol fields, registry, parser, automatic reaccept, accepted-project edits or provider dispatch. Neither dependency reachability nor matching definitions proves surviving material or purchases. Record reproducible probes and explain whether the result offers anything beyond the shipped placement guidance; fresh agent compliance remains unmeasured. No implementation follows without a later bet. Record damp-moon-9297 evidence and explicit limits, then stop [rec: snowy-cove-0032] [rec: odd-ridge-9607] [rec: misty-tide-6394] [rec: lawful-dune-3795] [rec: brave-pebble-4147].
2. **Retry the two-servo leg once eligible (missions 2 and 6, the ladder rule).** Use the same documented walk and exact mechanism prompt from silent-mist-5233 in a fresh project outside the checkout. Dispatch only after the reported 2026-09-08 08:30 Europe/Madrid reset (06:30 UTC), or affirmative availability evidence; eligibility allows one attempt and does not guarantee quota. Use the corrected CLI and lawful-dune-3795's qualified payload at build/engine/cadex-engine-0.0.0-macos-arm64, whose CadexScriptedRuntime.py SHA256 is 602164e85c399ad203517eb269ec81bca549dc07b72d303659c1cffffe1dc6df. Identify the actual engine used; the installed application remains stale. Use the training venv, JAX_PLATFORMS=cpu, --iterations 1 --envs 4 --seed 0 --timeout 600, and the existing 2.9 GiB/850 s process-tree guard. Report each leg as clean, needing a guess/person, or not reached, with exact errors and comparable PROGRESS numbers. Inspect four views and section, manually compare script catalog usage with placed inventory, and use the bound-agreement checker where catalog rows exist. A missing tally or zero catalog rows proves no absence of fused hardware. No source correction in the rehearsal; a failure names the next unit. Record crisp-reef-5607 and swift-dusk-2951 evidence, and damp-moon-9297 only for observed review findings [rec: silent-mist-5233] [rec: morning-summit-7848] [rec: lawful-dune-3795] [rec: calm-beacon-7800] [rec: brave-pebble-4147].

Signals: 43 iterations, 7.6h elapsed, 7.4h left; Claude five_hour 75%, seven_day 47%, Codex seven_day 10%. Worker repair and all required qualification are complete; do not repeat them. Two units only, zero new directions. If discovery finishes before rehearsal eligibility, retain the pending unit for controller scheduling: do not dispatch another actor solely to re-record the unchanged pre-reset blocker. No actor waiting, repeated provider probes, paid fallback, provider switch, GUI launch, remote execution or Later promotion. After either evidence unit, replan on its result; do not automatically implement a diagnostic or fill the interval with unrelated work [rec: lawful-dune-3795] [rec: calm-beacon-7800] [rec: brave-pebble-4147].
