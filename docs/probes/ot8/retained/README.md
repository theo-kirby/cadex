...
| [`g5-retention.json`](g5-retention.json) | G5 | the regression floor at the final revision: both full suites, the packaged lifecycle gate and the packaged licensing audit against the staged payload, the two ADR-398 repeated-restore retention regressions named and passing, and a restore-then-reopen of an independent copy of all five designs this run used — the three ot7 baselines and ot8's own two products — with their accepted pins, accepted artifacts and every clearance pair equal to what was published |
