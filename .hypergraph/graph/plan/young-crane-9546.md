...
1. **Qualify output-loss consent and cold recovery once (missions 1 and 2).** First-visit Git ownership and its precise guidance correction are complete; do not repeat or expand that campaign. Inspect docs/CLI.md's `script --set --replace` contract, CLI command tests and relevant engine refusal/restore tests first. If existing evidence already proves the whole transition below, record that evidence and stop without a duplicate experiment. Otherwise use the existing qualified stage and one fresh external disposable project initialized by the ordinary CLI: accept a minimal two-output script, submit a valid candidate dropping one output without `--replace`, cold-visit the project through a new CLI process, recover through the documented script acceptance path if necessary, then explicitly accept that candidate with `--replace`. Derive exact flags and expected semantics from code/docs before execution. Record exit/error codes, working versus accepted revision/digest, accepted output names and cold recoverability at each boundary; preserve user docs and stored assets. Rejected candidates may legitimately become the working script, and restore bookkeeping may change: neither alone is data loss. No manual metadata edits, provider, training, walk, stage refresh, durable-project mutation, real checkpoint/trace generation or concurrency campaign. This is one finite consent-contract qualification, not a presumed product defect [rec: fresh-flint-1505] [rec: little-fern-0464] [rec: rich-creek-7708].

2. **Correct only a demonstrated output-loss/recovery mismatch (missions 1 and 2; conditional).** If unit 1 demonstrates misleading guidance or a bounded implementation defect, correct exactly that case and preserve the explicit replacement consent boundary. Add a behavioral regression only for a real defect; do not mirror prose or add a recovery framework. CLI edits require the full CLI suite; any engine, protocol, payload or shell edit requires its AGENTS.md gates, with at most one full build. Behavior changes update lifecycle docs and scaffold together, and ADR/ROADMAP where required. If the contract holds, existing evidence already covers it, or the correction cannot be bounded safely, this unit is not dispatchable: return the finding for replanning with no automatic repeat, broader rejection survey or Git campaign [rec: rich-creek-7708].

Signals: 193 iterations, 11.7h elapsed, 3.3h left, $85.47 API-equivalent; Claude five_hour 65%, seven_day 54%; Codex seven_day 43%. One new standing-maintenance direction selected from three, serving missions 1 and 2. The provider-free walk already passed and its tracking rejection was repaired; no missing leg justifies another baseline. No Later promotion, charter-gap retirement or criterion reopening. Live repair remains controller-parked: no automatic retry, availability probe, repeated hold record, provider switch, paid fallback, GUI or remote execution [rec: lucid-snow-0350] [rec: scarlet-journey-0203] [rec: sunny-walrus-5847] [rec: rich-creek-7708].
