...
1. **Fresh mixed-joint lifecycle coverage remains unresolved (mission 2).** The uninterrupted resumed walk succeeded; the fresh crank-slider invocation was attempted and refused by claude-opus-5 before geometry. Both offline units have landed; short now leads with one conditional fresh walk and permits the detached receipt unit only after completion or an explicit refusal [rec: restless-wave-7376] [rec: idle-pond-4961] [rec: clear-shade-1084]. A readiness success is permission to attempt, not proof of completion; an explicit refusal is not a geometry defect. ADR-274's silence follow-up remains something to observe in an eligible walk, not a separate scheduled rerun. No extra seeds or repeated refusal-only dispatch follows [rec: sage-glacier-2165] [rec: northern-quartz-8401] [rec: silent-anchor-7513].

2. **Preserve the four seeded criteria's evidence and the human's decision.** All four remain working in the projection. The walk has an uninterrupted resumed success and earlier from-nothing successes; the requested current mixed-joint run remains unevidenced. All four headless review calls exist and have run; ADR-275 delivered standalone derived sections (unsupported 2/10 at explicit zero versus ok 6/10 derived on ot4-swing2). ADR-277 has delivered the missed-object/movement join (five stored swing misses: three moving, two stationary), with 285 CLI passes; ADR-276 has delivered project model continuity. These leave dispatch, without becoming new prerequisites for acknowledging delivery [rec: idle-pond-4961] [rec: restless-wave-7376]. Three modes already have the scripted remote handoff and documented GUI shape allowed here; actual remote and GUI runs remain forbidden. The second-mechanism evidence remains valid. No checkbox, state status or charter gap is retired by planning [rec: neat-grotto-9232] [rec: sage-glacier-2165] [rec: northern-quartz-8401] [rec: cool-eagle-6400] [rec: falling-willow-7995] [rec: silent-anchor-7513].

3. **Finish the existing detached walk handoff after the pending receipt is proven (mission 2).** Short promotes only the launch/receipt boundary after the fresh walk completes or is blocked. The dependent unit here carries that locator through the walk and documents/scripts completion through existing dispatcher watch/pull operations, then the ordinary policy verification, declaration, rollout and review. Pending is distinct from success; completion must not repeat design or training, consume stale policy bytes, or lose warm-start provenance. Reuse existing run artifacts and policy checks, with local transport stand-ins pinning pending, failed and completed paths; no live remote or GUI execution and no background scheduler. Do not promote this continuation until the receipt unit lands. This remains the existing --detach direction, not a new requirement for the already-working Three modes criterion [rec: idle-falcon-3004] [rec: clear-shade-1084].

4. **Delivered work leaves dispatch.** Seeds 0–3 completed with fixed exported model/task and objective/action identities, training seeds varied and rollout seed 7 held; reward 170.952826823611–175.935971673601, translation 30.078469436328–31.421759939733 mm, rotation zero, every review succeeding and prior artifacts surviving. History updates replace complete files only after successful writing. Recent DECISIONS entries survive the bounded prompt on fresh and resumed turns, and the lifecycle guide no longer promises unconditional repository initialization. The section offset is derived from the accepted snapshot's own bounds and since ADR-273 ranked by objects actually cut, and ADR-275 now exposes derivation when standalone --offset-mm is omitted (explicit zero remains explicit); the remote dispatcher carries and re-points a warm start; and a second walk of a project no longer locks it out (ADR-272). Protection is per-file, not concurrency, multi-document or power-loss safety; context availability is not model understanding [rec: honest-banner-0821] [rec: solar-snow-8543] [rec: western-cliff-6876] [rec: idle-falcon-3004] [rec: rising-rain-0117]. [rec: neat-grotto-9232] [rec: silent-anchor-7513].

5. **Keep comparison and retention limits.** Four-seed ranges are descriptive; the 30 mm action midpoint remains a confound and neither matching objectives nor favorable travel proves equivalent control difficulty or learned improvement. Root review exclusions and policy exclusions after default negations govern untracked files; existing tracked/staged paths retain ordinary Git semantics. Legacy identity rows remain unavailable and numeric comparison references may differ from same-kind identity references. A derived section offset changes the artifact path for future runs and proves no motion clearance. No automatic migration, untracking or standalone-rollout reporting work follows [rec: honest-banner-0821] [rec: easy-beacon-3803] [rec: modest-prairie-3356] [rec: steady-chart-0544].

6. **Keep the charter boundary.** Four seeded criteria are working in the projection. Remote handoff remains scripted/documented and GUI attachment documented under execution constraints. Keep mode rehearsals, inherited-removal campaigns, catalog, fleet, report rendering and gait training parked. Kernel rollout clearance remains unselected on long; and the quill overlap that ADR-005 called deliberate was disputed by a live design turn and designed out to zero, which is a warning about inferring a cause from a project's own record as much as from arithmetic. No charter gap is deleted, retired or marked done by this pass [rec: restless-cabin-9629] [rec: western-grotto-7499] [rec: sleepy-hollow-9498] [rec: humble-star-3313].
