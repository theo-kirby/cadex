"""One bounded real GPU training run on Finch, the buildable MG90S biped (D6,
...
retained final video -- both videos in the dark reference look the CLI's
recorder now has (ADR-331, ADR-332) and naming what they show (ADR-333).

This is the Lark driver (``../../lark-fresh/train.py``) with two Finch facts:
the accepted view carries 29 components (five printed parts and 24 catalog
parts, ADR-334), and the authorship default names Finch. The model, task and
...
authored_by = sys.argv[4] if len(sys.argv) > 4 else 'Finch, authored for the D5 unit and entered with cadex script --set (ADR-334); the stand task declared the same way in the D6 unit'
...
assert components == 29, components   # five printed parts and 24 catalog parts (ADR-334)
base = dict(project_root=p, mode='offboard-checkpoint-experiment', accepted_revision=e['accepted_revision'], digest=e['digest'], params=e['params'], param_specs=m['param_specs'], specs_source='successful CLI export', identity_source='successful params export envelope', requested={'iterations': 240, 'envs': 1024, 'seed': 0, 'checkpoint_every': 20, 'timeout_seconds': 3600, 'authored_by': authored_by}, task_bundle=task_json, task_sha256=sha(task_json), model_xml=model_xml)
write_run_record(run, status='running', snapshot_docs=True, **base)
unit = 'cadex-' + name
cmd = ['systemd-run', '--user', '--scope', '--unit=' + unit, '-p', 'MemoryMax=20G', 'timeout', '--signal=TERM', '--kill-after=20s', '3600', str(Path.home() / 'cadex-train-venv/bin/python'), 'training/cadex_train.py', str(task_json), '--out', str(train / (name + '.cxpolicy')), '--iterations', '240', '--envs', '1024', '--seed', '0', '--checkpoint-every', '20']
