...
1. **Run the documented walk on an unwalked mechanism (missions 1, 2 and 6).** Fresh scratch project outside the checkout; ordinary installed engine by explicit `--engine` with the environment overrides unset, as the refresh did. One `cadex walk` with one `--prompt` naming a two-servo mechanism from catalog parts whose joint topology the walk has not carried (a pan-tilt head on two MG90S is the suggested shape; another unwalked topology is fine if the record says why), local toy training with `--iterations 1 --envs 4 --seed 0 --timeout 600`, the existing training venv, `JAX_PLATFORMS=cpu`, under the existing process-tree monitor at 2.9 GB and 850 s. Inspect the four named views, the section and review.json. Record per leg: clean, needed a guess, or needed a person, with the walk's verbatim error where one occurs; timing and sampled peak RSS are observations. Compact evidence only; no product source change, build, GUI launch or remote dispatch. Declare impacts on crisp-reef-5607, swift-dusk-2951 and damp-moon-9297 only for evidence actually gained [rec: salty-nest-8235] [rec: strong-raven-3067].
2. **Close the leg the run names (conditional).** Only if rank 1 records a leg that needed a person or a guess: close that one leg in one unit under the zone gate AGENTS.md names, with the ADR line and ROADMAP convention, then rerun the same walk once to show it. A clean rank 1 ends the direction and empties this rung; the next planner selects from the standing rungs [rec: salty-nest-8235].

Both schema-guidance units landed: the resolver probe confirmed the schema-only boundary with 10 existing resolution passes, and the guidance correction removed the payload-coherence overclaim with unchanged runtime and 195 CLI passes without skips. That direction is exhausted. Signals: 31 iterations, 4.9h elapsed, 10.1h left; one bounded walk and at most one closing unit are selected. GUI stays unlaunched and Later criteria parked [rec: tiny-haven-0347] [rec: keen-pebble-3574] [rec: salty-nest-8235].
