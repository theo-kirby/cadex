...
- **Phase 8 — metatype preservation and complete disable landed; deletion remains open (ADR-213).** `App/MetaTypes.h` preserves the contract, the Gui header forwards and eighteen retained includes now use App; release dependencies contain zero audited GUI paths [rec: sharp-pond-0087]. All nine public presets select OFF; the shared initializer defaults OFF and rejects explicit/cached ON. Debug/release configuration and release build pass, with retained Qt components and GUI sources. Engine tests pass (2021 passed / 52 skipped); the final rpm correction received a focused 6/6 run, without a repeat full suite/build or cross-platform configuration. Both cadex ctests pass with their installed-engine preference. Inherited ctest retains 162 baseline failures / 1537 enabled tests, unchanged inventory/skips/disabled cases and all 35 Material tests passing; no fresh staging or packaged gate was run [rec: fair-cabin-5280]. The thirteen-tree deletion boundary includes `src/Gui` (1,960 files / 65,331,470 bytes). Keep Assembly `CommandCreateView`, `JointObject`, `Preferences` and `UtilsAssembly`: the publisher still creates native `ExplodedView`/`ExplodedViewStep` through the unconditionally installed module. Follow the residual-registration and delete/build/ctest/install/stage gates in `docs/PHASE8-AUDIT.md`; prerequisite success does not close deletion [rec: wise-isle-1725] [rec: fair-cabin-5280].
