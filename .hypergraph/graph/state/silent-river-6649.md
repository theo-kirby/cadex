...
**The Wren lifecycle report now links its own missing/partial/failed video receipt and reproducible browser recovery probe**, closing the stated Wren-only D8 evidence gap. Isolated-copy fault injection preserves historical playback/download, restores current output on the same page and leaves all 3,769 original files unchanged. It does not establish a new real encoder crash or whole-goal acceptance. The fresh report guard passes four tests; that iteration's full CLI process had loaded the old guard and reported 418 passed/1 skipped/1 failed, with no full rerun after the report edit. Engine validation passed 2,110/53 skipped [rec: fresh-timber-6139].

**Wren now has a user-facing D1–D11 lifecycle report at `docs/probes/wren-fresh/LIFECYCLE.md`.** It traces creation, reopen, training, review, copy, interruption, product-agent revision, retraining and restart; six historical/current browser views preserve exact model/document/video identities and all 703 run/asset files. The report separates original agent-authored 85 mm, caller-authored 105/110 mm and agent-revised 90 mm designs. Historical five-seed comparisons are distinct from the current `wren71` seed-0 restart repeat (+61.241 mm). The 110 mm retry trained 12 updates versus 240 for `wren66`; matching evaluation seeds and episode limits do not make this a controlled geometry comparison. Reconcile judgement: retain `working` with the Wren report now present; no walking or whole-goal claim follows. CLI 419 passed/1 skipped; isolated engine rerun 2110 passed/53 skipped after an earlier invocation ended on signal 15 with cause unestablished [rec: crisp-stream-4743].
