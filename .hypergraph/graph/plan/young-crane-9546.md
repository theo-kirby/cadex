...
1. **Rerun the documented local toy walk (mission 2; damp-moon-9297).** Both former short units landed, so the charter's empty-short rule applies before new implementation [rec: happy-dune-3481] [rec: falling-birch-1503] [rec: modest-summit-8554]. Use the documented hinged-arm recipe and public `cadex script` then `cadex walk` entry points against the built engine, with one iteration, four environments, seed zero and local CPU training in the training venv. Bound each training run to 15 minutes and 3 GB; record commands, exit status, wall time, peak memory, policy verification, rollout numbers and project-relative artifact paths. Check inventory remains in review and the project commit; missing clearance wiring is expected and must be named. Compare whole-walk cost with the recorded 14.39 s arm run, separately from rebuild medians. Run the CLI zone gate and report skips honestly. This is one evidence unit with its own commit/record; do not bundle wiring into it. Any actual failed leg becomes the next must-fix before ranks 2–3 [rec: happy-dune-3481] [rec: falling-birch-1503] [rec: humble-stream-3878].
2. **Wire clearance into walk review (missions 6 and 2; damp-moon-9297), after a clean rerun or its repair.** Reuse the complete clearance reader and existing thresholds without tokens or an extra rebuild. Land `docs/clearance.md` with the walk's project commit; add `review.json` availability, initial-pose scope, thresholds, pairs checked, offending pairs by label/catalog id, unknown-pair count and project-relative report path; put comparable offending-pair counts in `PROGRESS.md`. Preserve unavailable and unknown outcomes distinctly from clear, and keep genuine inspection failures visible. An offending pair is a finding, not an automatic walk failure. Do not imply swept-motion coverage or large-assembly qualification [rec: falling-birch-1503]. Update scaffold/`project_docs.py`, shared mode-artifact table, CLI/walk docs, ADR and ROADMAP as required in the same commit. Real arm/carriage walks must assert report contents and commits, including the arm's below-clearance finding and the carriage's clear result at existing defaults; extend local/remote-flag CPU stand-in parity and unavailable/unknown reader-boundary cases. Run the full CLI suite with built engine and toy CPU venv, reporting resource measurements and skips. No engine/protocol or shell change is needed [rec: happy-dune-3481] [rec: gilded-basin-9946] [rec: humble-stream-3878].
3. **Probe named-angle rendering (mission 6; damp-moon-9297), after clearance wiring lands or blocks.** One evidence unit establishes whether the existing blueprint renderer can run in the bundle's Blender background process, without a GUI or copying GPL shell code into LGPL CLI. Use an accepted recipe revision to produce front/top/right/iso artifacts locally and inspect their contents, recording command, dependencies, revision/placement fidelity, wall time and memory. If this route fails, assess rendering accepted tessellation without Blender and record the failure and selected alternative. Explain compatibility with headless engine-only projects and what section views would reuse. Record the chosen route before product implementation; leave render delivery on medium. No replacement shell/engine, GUI launch, new dependency installation or remote dispatch. Run gates for any touched zone; an evidence-only probe honestly states no product code changed [rec: flat-river-8853] [rec: humble-stream-3878].

Inventory wiring and the clearance call are landed, not redispatches; the headless-review criterion remains open [rec: happy-dune-3481] [rec: falling-birch-1503]. Three-mode parity is already evidenced under the charter's documented-only constraints [rec: gilded-basin-9946]. No further medium promotion until these short units land or block; any critic must-fix precedes them. Whenever short empties again, rerun the documented local toy walk first [rec: modest-summit-8554] [rec: humble-stream-3878].

Parked Later criteria remain outside dispatch, and inherited reduction remains standing work rather than an iteration target [rec: modest-summit-8554].
